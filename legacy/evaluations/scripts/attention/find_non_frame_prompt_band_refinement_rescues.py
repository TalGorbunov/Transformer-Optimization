"""
Refine a selected non-frame prompt band by splitting only that focus region into
finer contiguous subgroups and measuring which subgroup rescues the answer most.
...
- choose one focus region inside the non-frame prompt tokens
- at layers [starting_layer, starting_layer + transition_layers):
  - keep frame-token transition masking identical to the non-frame rescue script
  - keep carrier transition masking identical to the non-frame rescue script
  - block all non-frame prompt tokens to self-plus-optional-spared-token attention
  - reopen exactly one contiguous focus subgroup at a time
...
_RUNS_KEY = "band_refinement_runs"
_GROUPS_KEY = "focus_subgroups"
_PLOT_COLOR = "#ff7f0e"
...
_PER_SAMPLE_FIELDS = [
    ...
    "carrier_token",
    "focus_mode",
    "focus_positions_json",
    "focus_size",
    "num_groups",
    "run_type",
    "subgroup_index",
    "subgroup_start_position",
    "subgroup_end_position",
    "subgroup_size",
    "subgroup_token_positions_json",
    "subgroup_token_texts_json",
...
def parse_focus_positions(raw: str) -> List[int]:
    parts = [part.strip() for part in str(raw).split(",") if part.strip()]
    if not parts:
        raise ValueError("--focus_positions must not be empty")
    ...
    for part in parts:
        try:
            ...
            raise ValueError(f"Invalid integer in --focus_positions: {part!r}") from exc
    return positions
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Refine a selected non-frame prompt band by reopening one finer contiguous "
            "focus subgroup at a time during one fixed transition window."
        )
    )
    ...
    ap.add_argument(
        "--output",
        type=str,
        default="outputs/find_non_frame_prompt_band_refinement_rescues",
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Print lightweight transition-mask sanity checks plus representative focus-band "
            "rows for one retained sample."
        ),
    )
    ap.add_argument(
    ...
    has_focus_range = args.focus_start is not None or args.focus_end is not None
    has_focus_positions = args.focus_positions is not None
    if has_focus_range == has_focus_positions:
        raise ValueError(
            "Exactly one focus selection mode is required: either pass both --focus_start/--focus_end "
            "or pass --focus_positions."
        )
    if has_focus_range:
        if args.focus_start is None or args.focus_end is None:
            raise ValueError("--focus_start and --focus_end must be provided together")
        if int(args.focus_end) < int(args.focus_start):
            raise ValueError("--focus_end must be >= --focus_start")
        args.focus_mode = "range"
        args.focus_positions_list = None
    else:
        args.focus_mode = "explicit_positions"
        args.focus_positions_list = parse_focus_positions(args.focus_positions)
    return args
...
def classify_focus_position(
    ...
    if int(position) not in set(int(value) for value in non_frame_prompt_positions):
        return "not_non_frame_prompt_token"
    return None


def resolve_focus_positions(
    layout: af1_utils.TokenLayout,
    frame_group_by_token: Dict[int, Tuple[int, ...]],
    non_frame_prompt_positions: Sequence[int],
    focus_mode: str,
    ...
    if str(focus_mode) == "range":
        assert focus_start is not None and focus_end is not None
        selected_positions = list(range(int(focus_start), int(focus_end) + 1))
    elif str(focus_mode) == "explicit_positions":
        if focus_positions_list is None:
            raise ValueError("focus_positions_list must be provided for explicit_positions mode")
        selected_positions = [int(position) for position in focus_positions_list]
    else:
        raise ValueError(f"Unsupported focus_mode: {focus_mode!r}")

    invalid_details: List[str] = []
    for position in selected_positions:
        reason = classify_focus_position(
        ...
        if reason is not None:
            invalid_details.append(f"{int(position)}:{reason}")
    if invalid_details:
        raise RuntimeError(
            f"focus selection contains invalid positions for sample_id={layout.sample_id}: {json.dumps(invalid_details)}"
        )

    return sorted(set(int(position) for position in selected_positions))


def build_subgroup_records(
    layout: af1_utils.TokenLayout,
    focus_positions: Sequence[int],
    num_groups: int,
) -> List[Dict[str, Any]]:
    subgroups = split_positions_into_groups(focus_positions, num_groups)
    records: List[Dict[str, Any]] = []
    for subgroup_index, subgroup_positions in enumerate(subgroups):
        token_texts = [
            af1_utils.sanitize_token_text(str(layout.prompt_decoded_tokens[int(position)]))
            for position in subgroup_positions
        ]
        records.append(
            {
                "subgroup_index": int(subgroup_index),
                "token_positions": [int(position) for position in subgroup_positions],
                "token_texts": token_texts,
                "subgroup_size": int(len(subgroup_positions)),
                "subgroup_start_position": (
                    None if not subgroup_positions else int(subgroup_positions[0])
                ),
                "subgroup_end_position": (
                    None if not subgroup_positions else int(subgroup_positions[-1])
                ),
            }
        )
    return records
...
def run_model_with_last_token_mask(
    ...
    prepare_attention_backend_for_forward(
        path_name="find_non_frame_prompt_band_refinement_rescues_masked_window",
...
def format_transition_run_debug(
    ...
    reopened_subgroup_index: Optional[int],
    reopened_non_frame_positions: Sequence[int],
) -> str:
    blocked_set = {int(position) for position in non_frame_prompt_positions}
    reopened_set = {int(position) for position in reopened_non_frame_positions}
    grouped_positions = [
        [int(position) for position in subgroup_record["token_positions"]]
        for subgroup_record in subgroup_records
    ]
    focus_position_set = {int(position) for position in focus_positions}
    non_reopened_focus_positions = [
        int(position)
        for position in focus_positions
        if int(position) not in reopened_set
    ]
    ...
    if non_reopened_focus_positions:
        representative_queries.append(int(non_reopened_focus_positions[0]))
    elif focus_positions:
        representative_queries.append(int(focus_positions[0]))
    ...
    lines = [
        ...
        f"selected_focus_positions={json.dumps([int(position) for position in focus_positions])}",
        f"grouped_focus_positions={json.dumps(grouped_positions)}",
        (
            "reopened_subgroup_index=baseline_blocked"
            if reopened_subgroup_index is None
            else f"reopened_subgroup_index={int(reopened_subgroup_index)}"
        ),
    ]
    for query_idx in unique_queries[:3]:
        query_role = "carrier"
        if int(query_idx) in focus_position_set and int(query_idx) in reopened_set:
            query_role = "focus_reopened"
        elif int(query_idx) in focus_position_set:
            query_role = "focus_blocked"
        ...
        lines.append(
            f"transition_query_role={query_role} "
            f"query_idx={int(query_idx)} token={token_text!r} "
...
def compute_run_metrics(
    ...
    focus_positions: Sequence[int],
    subgroup_records: Sequence[Dict[str, Any]],
    subgroup_record: Optional[Dict[str, Any]],
    non_frame_prompt_self_only: bool,
    prompt_len: int,
    ...
    reopened_subgroup_index = None if subgroup_record is None else int(subgroup_record["subgroup_index"])
    reopened_non_frame_positions = [] if subgroup_record is None else list(subgroup_record["token_positions"])
    if debug_masks and emit_detailed_debug:
        debug_prefix = (
            "  [debug] "
            f"run_type={run_type} "
            f"subgroup_index={'none' if reopened_subgroup_index is None else int(reopened_subgroup_index)}"
        )
        print(
            f"{debug_prefix} "
        ...
        for line in format_transition_run_debug(
            ...
            reopened_subgroup_index=reopened_subgroup_index,
            reopened_non_frame_positions=reopened_non_frame_positions,
        ).splitlines():
    ...
    try:
        ...
        print(
            ...
            f"failed non-frame prompt band refinement masking ({exc}); "
    ...
    return {
        ...
        "subgroup_index": None if subgroup_record is None else int(subgroup_record["subgroup_index"]),
        "subgroup_start_position": (
            None if subgroup_record is None else subgroup_record["subgroup_start_position"]
        ),
        "subgroup_end_position": (
            None if subgroup_record is None else subgroup_record["subgroup_end_position"]
        ),
        "subgroup_size": 0 if subgroup_record is None else int(subgroup_record["subgroup_size"]),
        "subgroup_token_positions": [] if subgroup_record is None else list(subgroup_record["token_positions"]),
        "subgroup_token_texts": [] if subgroup_record is None else list(subgroup_record["token_texts"]),
...
def format_per_sample_results_table(sample_payload: Dict[str, Any]) -> str:
    ...
    for run in sample_payload.get(_RUNS_KEY, []):
        rows.append(
            [
                str(run["run_type"]),
                "baseline" if run.get("subgroup_index") is None else str(int(run["subgroup_index"])),
                str(int(run["subgroup_size"])),
    ...
    title = (
        f"Per-sample subgroup rescues: sample_id={sample_payload['sample_id']} "
...
def compute_sample_payload(
    ...
    try:
        focus_positions = resolve_focus_positions(
            ...
            focus_end=focus_end,
            focus_positions_list=focus_positions_list,
        )
    except Exception as exc:
        print(
            f"[{sample_index}/{total_samples}] sample_id={sample_id} skipped: "
            f"focus selection invalid ({exc})"
        )
        return None
    subgroup_records = build_subgroup_records(
        layout=layout,
        focus_positions=focus_positions,
    ...
    print(
        ...
        f"focus_mode={str(focus_mode)} "
        f"focus_positions={json.dumps([int(position) for position in focus_positions])} "
    ...
    for subgroup_record in subgroup_records:
        ...
        print(
            "  "
            f"run_type=subgroup_reopened "
            f"subgroup_index={int(subgroup_record['subgroup_index'])} "
            f"subgroup_size={int(subgroup_record['subgroup_size'])} "
    ...
    best_candidate = max(
        [
            ...
            if run.get("run_type") == "subgroup_reopened" and _is_present_number(run.get("rescue_amount"))
        ],
        key=lambda run: (float(run["rescue_amount"]), -int(run["subgroup_index"])),
        default=None,
    )
    if best_candidate is None:
        print("  best_subgroup_index=none best_rescue_amount=n/a")
    else:
        print(
            "  "
            f"best_subgroup_index={int(best_candidate['subgroup_index'])} "
    ...
    sample_payload = {
        ...
        "focus_mode": str(focus_mode),
        "focus_positions": [int(position) for position in focus_positions],
        "focus_size": int(len(focus_positions)),
        "num_groups": int(num_groups),
        "starting_layer": int(starting_layer),
        "transition_layers": int(transition_layers),
            ...
            rows.append(
                {
                    ...
                    "focus_mode": sample["focus_mode"],
                    "focus_positions_json": json.dumps(sample.get("focus_positions", [])),
                    "focus_size": int(sample["focus_size"]),
                    "num_groups": int(sample["num_groups"]),
                    "run_type": run["run_type"],
                    "subgroup_index": run.get("subgroup_index"),
                    "subgroup_start_position": run.get("subgroup_start_position"),
                    "subgroup_end_position": run.get("subgroup_end_position"),
                    "subgroup_size": int(run["subgroup_size"]),
                    "subgroup_token_positions_json": json.dumps(run.get("subgroup_token_positions", [])),
                    "subgroup_token_texts_json": json.dumps(run.get("subgroup_token_texts", [])),
...
def build_aggregate_rows(
    ...
    rows: List[Dict[str, Any]] = []
    for subgroup_index in range(int(num_groups)):
        candidate_rows = [
            run
            for sample in sample_payloads
            for run in sample.get(_RUNS_KEY, [])
            if run.get("run_type") == "subgroup_reopened"
            and int(run.get("subgroup_index")) == int(subgroup_index)
        ]
        subgroup_sizes = [float(run["subgroup_size"]) for run in candidate_rows]
        score_drop_values = [
        ...
        rows.append(
            {
                ...
                "mean_subgroup_size": _mean_or_none(subgroup_sizes),
...
def format_aggregate_group_table(rows: Sequence[Dict[str, Any]]) -> str:
    ...
    values = [
        [
            str(int(row["subgroup_index"])),
            str(int(row["n_samples"])),
            _format_float_or_na(row.get("mean_subgroup_size")),
...
def build_group_metric_summary(
    ...
    per_group_values: Dict[int, List[float]] = {int(subgroup_index): [] for subgroup_index in range(int(num_groups))}
    for sample in sample_payloads:
        for run in sample.get(_RUNS_KEY, []):
            if run.get("run_type") != "subgroup_reopened":
                continue
            ...
            if not _is_present_number(value):
                continue
            per_group_values[int(subgroup_index)].append(float(value))

    if not any(per_group_values[subgroup_index] for subgroup_index in per_group_values):
        return None
    ...
    for subgroup_index in range(int(num_groups)):
        values = per_group_values[int(subgroup_index)]
        if not values:
            continue
        ...
        summary["group_indices"].append(int(subgroup_index))
...
def plot_per_sample_rescue_summary(
    ...
    bar_colors: List[str] = []
    heights: List[float] = []
    for subgroup_index, rescue_value in zip(x_values, y_values):
        if rescue_value is None:
            bar_colors.append(_MISSING_BAR_COLOR)
            heights.append(0.0)
            continue
        plotted_y_values.append(float(rescue_value))
        if best_subgroup_index is not None and int(subgroup_index) == int(best_subgroup_index):
...
def write_outputs(
    ...
    per_sample_csv_path = output_dir / "non_frame_prompt_band_refinement_rescues_per_sample.csv"
    aggregate_csv_path = output_dir / "non_frame_prompt_band_refinement_rescues_aggregate.csv"
    ...
    if not args.disable_plots:
        if aggregate_rescue_summary is not None:
            aggregate_rescue_plot_path = plot_group_bar_summary(
                ...
                output_dir
                / (
                    f"mean_rescue_amount_by_subgroup_index"
                    f"{f'_{seq_len_label}' if seq_len_label else ''}.png"
                ),
                title=(
                    "Mean rescue amount by focus subgroup "
        ...
        if aggregate_score_drop_summary is not None:
            aggregate_score_drop_plot_path = plot_group_bar_summary(
                ...
                output_dir
                / (
                    f"mean_score_drop_by_subgroup_index"
                    f"{f'_{seq_len_label}' if seq_len_label else ''}.png"
                ),
                title=(
                    "Mean score drop by focus subgroup "
                    f"(start_layer={int(args.starting_layer)}, transition_layers={int(args.transition_layers)})"
                ),
    ...
    selected_focus_definition = (
        {
            "focus_mode": "range",
            "focus_start": int(args.focus_start),
            "focus_end": int(args.focus_end),
        }
        if str(args.focus_mode) == "range"
        else {
            "focus_mode": "explicit_positions",
            "focus_positions": [int(position) for position in args.focus_positions_list],
        }
    )
    ...
    summary_payload = {
        "cli_args": {
            ...
            if key != "focus_positions_list"
        },
        "selected_focus_definition": selected_focus_definition,
        ...
        "grouping_definition": (
            "For each sample, validate one selected focus region inside the non-frame prompt tokens, preserve "
            "those positions in token order, and split only that focus region into num_groups contiguous "
            "subgroups, allowing empty subgroups when num_groups exceeds the focus size."
        ),
        "masking_definition": (
    ...
    print("Aggregate subgroup summary across all retained samples:")
