...
EXPERIMENT_NAME = "visual_fixed8_iid_carrier_slots_lora"
...
CARRIER_SLOTS_K1 = "carrier_slots_k1"
CARRIER_SLOTS_K2 = "carrier_slots_k2"
CARRIER_SLOTS_K4 = "carrier_slots_k4"
CARRIER_SLOTS_K8 = "carrier_slots_k8"
SLOT_VARIANT_TO_K = {
    CARRIER_SLOTS_K1: 1,
    CARRIER_SLOTS_K2: 2,
    CARRIER_SLOTS_K4: 4,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Real-frame fixed-8 IID all-count LoRA carrier-slot capacity sweep."
    )
...
class VisualCarrierSlotAdapter(VisualCarrierMixingAdapter):
    def __init__(
        self,
        *,
        hidden_size: int,
        ...
        self.method = "carrier_slots"
        self.hidden_size = int(hidden_size)
        self.inject_layers = [int(layer) for layer in inject_layers]
        self.layer_to_pos = {layer: pos for pos, layer in enumerate(self.inject_layers)}
        self.message_mode = str(message_mode)
        self.num_slots = int(num_slots)
        self.score_norms = nn.ModuleList([nn.LayerNorm(self.hidden_size) for _ in self.inject_layers])
        self.value_norms = nn.ModuleList([nn.LayerNorm(self.hidden_size) for _ in self.inject_layers])
        self.read_norms = nn.ModuleList(
            [nn.LayerNorm(self.num_slots * self.hidden_size) for _ in self.inject_layers]
        )
        self.read_projections = nn.ModuleList(
            [
                nn.Linear(self.num_slots * self.hidden_size, self.hidden_size, bias=False)
                for _ in self.inject_layers
            ]
        )
        self.slot_queries = nn.Parameter(
            torch.empty(len(self.inject_layers), self.num_slots, self.hidden_size, dtype=torch.float32)
        )
        self.value_scales = nn.Parameter(
            torch.ones(len(self.inject_layers), self.hidden_size, dtype=torch.float32)
        )
        self.value_biases = nn.Parameter(
            torch.zeros(len(self.inject_layers), self.hidden_size, dtype=torch.float32)
        )
        self.gate_logits = nn.Parameter(torch.full((len(self.inject_layers),), float(gate_init), dtype=torch.float32))
        ...
        self._handles: List[Any] = []
        self._last_stats: Dict[str, Dict[str, Any]] = {}
        self._last_slot_states_by_layer: Dict[int, List[torch.Tensor]] = {}
        self.hook_fire_counts: Dict[int, int] = defaultdict(int)
        self.message_mode_counts: Dict[str, int] = defaultdict(int)
        self.exact_failure_counts: Dict[str, int] = defaultdict(int)
        self.exact_failure_examples: List[str] = []
    ...
    def set_context(self, batch: VisualBatch) -> None:
        ...
        self._last_slot_states_by_layer = {}
        self._last_stats = {
            "gate_values_by_layer": {},
            "residual_norm_by_layer": {},
            "hidden_norm_by_layer": {},
            ...
            "visual_message_norm_by_layer": {},
            "slot_norm_by_layer": {},
            "slot_assignment_entropy_by_layer": {},
            "message_mode_by_layer": {},
        }
    ...
    def inject_before_layer(
        ...
        slot_queries = self.score_norms[layer_pos](self.slot_queries[layer_pos].float())
        score_scale = float(self.hidden_size) ** -0.5
        for batch_idx in range(int(hidden_states.shape[0])):
            carriers = self._carrier_positions[batch_idx]
            sample_residuals: List[float] = []
            sample_hidden: List[float] = []
            ...
            sample_masses: List[float] = []
            sample_messages: List[float] = []
            sample_slot_norms: List[float] = []
            sample_entropies: List[float] = []
            sample_slots: List[torch.Tensor] = []
            for carrier_row, carrier_pos in enumerate(carriers):
                z = z_by_batch[batch_idx][carrier_row].float()
                masses = mass_by_batch[batch_idx][carrier_row].float()
                expected_frames = len(self._frame_groups[batch_idx])
                ...
                scored_messages = self.score_norms[layer_pos](z)
                scores = torch.matmul(scored_messages, slot_queries.transpose(0, 1)) * score_scale
                assignments = torch.softmax(scores, dim=-1)
                values = self.value_norms[layer_pos](z)
                values = values * self.value_scales[layer_pos].float() + self.value_biases[layer_pos].float()
                slots = torch.einsum("fk,fh->kh", assignments, values)
                flat_slots = slots.reshape(self.num_slots * self.hidden_size)
                projected = self.read_projections[layer_pos](self.read_norms[layer_pos](flat_slots)).float()
                residual = gate * projected
                before = hidden_states[batch_idx, carrier_pos, :].float()
                out[batch_idx, carrier_pos, :] = (
                    out[batch_idx, carrier_pos, :] + residual.to(dtype=out.dtype)
                ...
                residual_norm = residual.norm()
                hidden_norm = before.norm()
                entropy = -(assignments * assignments.clamp_min(EPS).log()).sum(dim=-1).mean()
                sample_residuals.append(float(residual_norm.detach().cpu().item()))
                sample_hidden.append(float(hidden_norm.detach().cpu().item()))
                sample_ratios.append(float((residual_norm / hidden_norm.clamp_min(EPS)).detach().cpu().item()))
                sample_masses.append(float(masses.sum().detach().cpu().item()))
                sample_messages.append(float(z.sum(dim=0).norm().detach().cpu().item()))
                sample_slot_norms.append(float(slots.norm(dim=-1).mean().detach().cpu().item()))
                sample_entropies.append(float(entropy.detach().cpu().item()))
                sample_slots.append(slots.detach().cpu())
            residual_norms.append(finite_mean(sample_residuals, default=0.0))
            hidden_norms.append(finite_mean(sample_hidden, default=0.0))
            ratios.append(finite_mean(sample_ratios, default=0.0))
            attention_masses.append(finite_mean(sample_masses, default=0.0))
            visual_message_norms.append(finite_mean(sample_messages, default=0.0))
            slot_norms.append(finite_mean(sample_slot_norms, default=0.0))
            assignment_entropies.append(finite_mean(sample_entropies, default=0.0))
            if sample_slots:
                slot_rows.append(torch.stack(sample_slots, dim=0))
            else:
                slot_rows.append(torch.empty((0, self.num_slots, self.hidden_size), dtype=torch.float32))
        self._last_slot_states_by_layer[int(layer_idx)] = slot_rows
        key = str(int(layer_idx))
        self._last_stats["gate_values_by_layer"][key] = [float(gate.detach().cpu().item())] * len(residual_norms)
        self._last_stats["residual_norm_by_layer"][key] = residual_norms
        self._last_stats["hidden_norm_by_layer"][key] = hidden_norms
        ...
        self._last_stats["visual_attention_mass_by_layer"][key] = attention_masses
        self._last_stats["visual_message_norm_by_layer"][key] = visual_message_norms
        self._last_stats["slot_norm_by_layer"][key] = slot_norms
        self._last_stats["slot_assignment_entropy_by_layer"][key] = assignment_entropies
        self._last_stats["message_mode_by_layer"][key] = [mode] * len(residual_norms)
        return out

    def probe_features_for_row(self, row: int, layer_idx: int = 17) -> torch.Tensor:
        by_row = self._last_slot_states_by_layer.get(int(layer_idx))
        if by_row is None or int(row) >= len(by_row):
            raise RuntimeError(f"No slot probe states captured for layer {layer_idx}")
        slots = by_row[int(row)].float()
        if slots.numel() == 0:
            return torch.empty((0, self.hidden_size), dtype=torch.float32)
        return slots.reshape(slots.shape[0] * slots.shape[1], slots.shape[2]).contiguous()

    def stats_for_row(self, row: int) -> Dict[str, Any]:
        out = super().stats_for_row(row)
        out["num_slots"] = int(self.num_slots)
        out["slot_norm"] = finite_mean(out.get("slot_norm_by_layer", {}).values(), default=0.0)
        out["slot_assignment_entropy"] = finite_mean(
            out.get("slot_assignment_entropy_by_layer", {}).values(),
            default=0.0,
        )
        return out


class LayerCarrierStateCollector:
    def __init__(self, layer_idx: int) -> None:
        self.layer_idx = int(layer_idx)
        self._carrier_positions: Optional[List[List[int]]] = None
        self._features_by_row: List[torch.Tensor] = []
        self._handles: List[Any] = []

    def set_context(self, batch: VisualBatch) -> None:
        self._carrier_positions = [[int(position) for position in row] for row in batch.carrier_positions]
        self._features_by_row = []

    def clear_context(self) -> None:
        self._carrier_positions = None

    @staticmethod
    def _output_hidden(output: Any) -> Optional[torch.Tensor]:
    ...
    def attach(self, model: Any) -> None:
        ...
        if self.layer_idx < 0 or self.layer_idx >= len(layers):
            raise ValueError(f"probe layer={self.layer_idx} outside [0, {len(layers) - 1}]")
        ...
        def hook(
            ...
            hidden = self._output_hidden(output)
            if hidden is None or self._carrier_positions is None:
                return
            rows: List[torch.Tensor] = []
            seq_len = int(hidden.shape[1])
            for batch_idx, carriers in enumerate(self._carrier_positions):
                valid = [position for position in carriers if 0 <= int(position) < seq_len]
                if valid:
                    index = torch.tensor(valid, device=hidden.device, dtype=torch.long)
                    rows.append(hidden[batch_idx, index, :].detach().float().cpu())
                else:
                    rows.append(torch.empty((0, int(hidden.shape[-1])), dtype=torch.float32))
            self._features_by_row = rows

        self._handles.append(layers[self.layer_idx].register_forward_hook(hook, with_kwargs=True))
    ...
    def features_for_row(self, row: int) -> torch.Tensor:
        if int(row) >= len(self._features_by_row):
            raise RuntimeError(f"No carrier probe states captured for layer {self.layer_idx}")
        return self._features_by_row[int(row)].float()
...
def collect_probe_feature_records(
    *,
    variant: str,
    split_name: str,
    model: Any,
    ...
    model.eval()
    adapter.eval()
    collector: Optional[LayerCarrierStateCollector] = None
    if variant == LORA_BASELINE:
        collector = LayerCarrierStateCollector(layer_idx)
    ...
    try:
        with torch.no_grad():
            for step, idx in enumerate(indices, start=1):
                ...
                _outputs = model(**batch.inputs, use_cache=False)
                if collector is not None:
                    features = collector.features_for_row(0)
                    feature_kind = "layer17_room_char_carrier_hidden"
                    element_count = int(features.shape[0])
                else:
                    if not isinstance(adapter.mixer, VisualCarrierSlotAdapter):
                        raise RuntimeError(f"{variant}: slot probe features require VisualCarrierSlotAdapter")
                    features = adapter.mixer.probe_features_for_row(0, layer_idx=layer_idx)
                    feature_kind = f"layer17_internal_slots_k{adapter.mixer.num_slots}"
                    element_count = int(features.shape[0])
                if features.ndim != 2:
                    raise RuntimeError(f"{variant} {split_name}: expected [elements, hidden], got {tuple(features.shape)}")
                if element_count <= 0:
                    raise RuntimeError(f"{variant} {split_name}: no probe feature elements captured")
                records.append(
                    {
                        "variant": variant,
                        "split": split_name,
                ...
                if step == 1 or step % 25 == 0:
                    print(f"  probe collect {variant} {split_name}: {step}/{len(indices)}")
    finally:
        if collector is not None:
            collector.detach()
    return records
...
def pad_probe_records(
    records: Sequence[Dict[str, Any]],
    *,
    max_elements: int,
) -> Dict[str, Any]:
    if not records:
        raise RuntimeError("Cannot pad empty probe feature records")
    hidden_size = int(records[0]["features"].shape[-1])
    features = torch.zeros((len(records), int(max_elements), hidden_size), dtype=torch.float32)
    mask = torch.zeros((len(records), int(max_elements)), dtype=torch.bool)
    labels = torch.empty((len(records),), dtype=torch.long)
    example_ids: List[str] = []
    element_counts: List[int] = []
    carrier_positions: List[List[int]] = []
    for row_idx, record in enumerate(records):
        tensor = record["features"].float()
        if int(tensor.shape[-1]) != hidden_size:
            raise RuntimeError("Probe hidden size changed within one variant")
        count = min(int(tensor.shape[0]), int(max_elements))
        features[row_idx, :count, :] = tensor[:count]
        mask[row_idx, :count] = True
        labels[row_idx] = int(record["gold_count"])
        example_ids.append(str(record["example_id"]))
        element_counts.append(int(record["element_count"]))
        carrier_positions.append([int(x) for x in record.get("carrier_token_positions", [])])
    return {
        "features": features,
        "mask": mask,
        "labels": labels,
        ...
        "carrier_token_positions": carrier_positions,
        "max_elements": int(max_elements),
        "hidden_size": hidden_size,
    }


def collect_and_save_probe_features(
    *,
    variant: str,
    run_dir: Path,
    model: Any,
    processor: Any,
    adapter: VariantAdapter,
    examples_by_split: Dict[str, List[VisualExample]],
    train_indices: Sequence[int],
    ...
    split_to_indices = {
        TRAIN_SPLIT: list(train_indices),
        VAL_SPLIT: list(eval_indices[VAL_SPLIT]),
        TEST_SPLIT: list(eval_indices[TEST_SPLIT]),
    }
    for split, indices in split_to_indices.items():
        raw_by_split[split] = collect_probe_feature_records(
            variant=variant,
            split_name=split,
            model=model,
            processor=processor,
    ...
    max_elements = max(int(record["features"].shape[0]) for rows in raw_by_split.values() for record in rows)
    payloads: Dict[str, Dict[str, Any]] = {}
    feature_dir = run_dir / "probe_features"
    feature_dir.mkdir(parents=True, exist_ok=True)
    for split, records in raw_by_split.items():
        payload = pad_probe_records(records, max_elements=max_elements)
        payload.update(
            {
                "variant": variant,
                "split": split,
                "probe_layer": 17,
                "feature_kind": str(records[0]["feature_kind"]) if records else "",
            }
        )
        torch.save(
            {
                **payload,
                "features": payload["features"].to(dtype=torch.float16),
            },
            feature_dir / f"{split}.pt",
        )
        write_jsonl(
            feature_dir / f"{split}_index.jsonl",
            [
                {
                    "variant": variant,
                    "split": split,
                    ...
                    "feature_kind": str(record["feature_kind"]),
                    "element_count": int(record["element_count"]),
                    "carrier_token_positions": list(record["carrier_token_positions"]),
                }
...
def probe_inputs(payload: Dict[str, Any], mode: str) -> Tuple[torch.Tensor, torch.Tensor]:
    features = payload["features"].float()
    mask = payload["mask"].bool()
    labels = payload["labels"].long()
    if mode == "pooled":
        weights = mask.float().unsqueeze(-1)
        x = (features * weights).sum(dim=1) / weights.sum(dim=1).clamp_min(1.0)
    elif mode == "flattened":
...
def probe_prediction_rows(
    *,
    ...
    preds = logits.argmax(dim=-1).detach().cpu().long()
    gold = labels.detach().cpu().long()
    rows: List[Dict[str, Any]] = []
    for idx in range(int(gold.numel())):
        pred = int(preds[idx].item())
        label = int(gold[idx].item())
        rows.append(
            {
                "variant": variant,
                "probe": probe_name,
...
def summarize_probe_rows(rows: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "n": len(rows),
        "probe_accuracy": accuracy(gold, pred),
        "accuracy": accuracy(gold, pred),
        "probe_mae": mae(gold, pred),
        "mae": mae(gold, pred),
        "high_count_probe_accuracy": high_accuracy,
        "high_count_5_8_accuracy": high_accuracy,
    }


def build_probe_metrics(
    rows: Sequence[Dict[str, Any]],
    variant: str,
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    split_rows: List[Dict[str, Any]] = []
    count_rows: List[Dict[str, Any]] = []
    for probe_name in ("pooled", "flattened"):
        for split in (TRAIN_SPLIT, VAL_SPLIT, TEST_SPLIT):
            selected = [row for row in rows if row["probe"] == probe_name and row["split"] == split]
            split_rows.append({"variant": variant, "probe": probe_name, "split": split, **summarize_probe_rows(selected)})
            for count in COUNT_VALUES:
                count_selected = [row for row in selected if int(row["gold_count"]) == count]
                count_rows.append(
                    {
                        ...
                        "split": split,
                        "gold_count": count,
                        **summarize_probe_rows(count_selected),
                    }
                )
    return split_rows, count_rows


def plot_probe_accuracy_by_count(
    path: Path,
    rows: Sequence[Dict[str, Any]],
    *,
    ...
    for variant in sorted({str(row["variant"]) for row in filtered}):
        for probe_name in ("pooled", "flattened"):
            selected = sorted(
                [row for row in filtered if row["variant"] == variant and row["probe"] == probe_name],
                key=lambda row: int(row["gold_count"]),
            )
            xs = [int(row["gold_count"]) for row in selected]
            ys = [float(row["probe_accuracy"]) for row in selected]
            if xs:
                ax.plot(xs, ys, marker="o", label=f"{variant} {probe_name}")
    ax.set_xlabel("Gold count")
...
def plot_high_count_probe_bar(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    selected = [row for row in rows if row["split"] == TEST_SPLIT]
    plot_rows = [
        {
            "label": f"{row['variant']} {row['probe']}",
            "value": row.get("high_count_probe_accuracy", math.nan),
        }
        for row in selected
    ]
    fig, ax = plt.subplots(figsize=(max(9, len(plot_rows) * 1.1), 5))
    labels = [row["label"] for row in plot_rows]
    values = [float(row["value"]) if finite_float(row.get("value")) is not None else math.nan for row in plot_rows]
    ax.bar(range(len(labels)), values, color="#59a14f")
    ax.set_xticks(range(len(labels)))
    ax.set_xticklabels(labels, rotation=30, ha="right")
    ax.set_ylabel("Gold 5..8 probe accuracy")
    ax.set_ylim(0, 1)
    ax.grid(axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(path, dpi=160)
...
def train_and_evaluate_probes(
    *,
    ...
    probe_device = device if str(device).startswith("cuda") and torch.cuda.is_available() else "cpu"
    prediction_rows: List[Dict[str, Any]] = []
    histories: List[Dict[str, Any]] = []
    for probe_name in ("pooled", "flattened"):
        x_train, y_train = probe_inputs(feature_payloads[TRAIN_SPLIT], probe_name)
        mean = x_train.mean(dim=0, keepdim=True)
        std = x_train.std(dim=0, unbiased=False, keepdim=True).clamp_min(1e-5)
        x_train = ((x_train - mean) / std).to(probe_device)
        y_train = y_train.to(probe_device)
        classifier = nn.Linear(int(x_train.shape[-1]), len(COUNT_VALUES)).to(probe_device)
        optimizer = torch.optim.AdamW(
            classifier.parameters(),
            lr=float(args.probe_lr),
            weight_decay=float(args.probe_weight_decay),
        )
        batch_size = max(1, int(args.probe_batch_size))
        for epoch in range(1, int(args.probe_epochs) + 1):
            order = torch.randperm(int(x_train.shape[0]), device=probe_device)
            total_loss = 0.0
            total_correct = 0
            total_n = 0
            classifier.train()
            for start in range(0, int(order.numel()), batch_size):
                ...
                logits = classifier(x_train[batch_idx])
                loss = F.cross_entropy(logits, y_train[batch_idx])
                optimizer.zero_grad(set_to_none=True)
                loss.backward()
                optimizer.step()
                total_loss += float(loss.detach().cpu().item()) * int(batch_idx.numel())
                total_correct += int((logits.argmax(dim=-1) == y_train[batch_idx]).sum().detach().cpu().item())
                total_n += int(batch_idx.numel())
            if epoch == 1 or epoch == int(args.probe_epochs) or epoch % 50 == 0:
                histories.append(
                    {
                        "variant": variant,
                        "probe": probe_name,
                        "epoch": epoch,
                        "train_ce": total_loss / max(1, total_n),
                        "train_accuracy": total_correct / max(1, total_n),
                    }
                )
                print(
                    f"  probe {variant} {probe_name} epoch={epoch}/{int(args.probe_epochs)} "
                    f"train_ce={total_loss / max(1, total_n):.4f} "
                    f"train_acc={total_correct / max(1, total_n):.4f}"
                )
        classifier.eval()
        for split, payload in feature_payloads.items():
            x, labels = probe_inputs(payload, probe_name)
            x = ((x - mean) / std).to(probe_device)
            with torch.no_grad():
        ...
        torch.save(
            ...
            run_dir / f"linear_probe_{probe_name}.pt",
        )
    split_rows, count_rows = build_probe_metrics(prediction_rows, variant)
    write_jsonl(run_dir / "probe_predictions.jsonl", prediction_rows)
    write_csv_dynamic(run_dir / "probe_metrics_by_split.csv", split_rows, ["variant", "probe", "split"])
    write_csv_dynamic(run_dir / "probe_metrics_by_count.csv", count_rows, ["variant", "probe", "split", "gold_count"])
    write_csv_dynamic(run_dir / "probe_train_history.csv", histories, ["variant", "probe", "epoch"])
    if not bool(args.no_plots):
        plot_probe_accuracy_by_count(run_dir / "probe_accuracy_by_count.png", count_rows)
        plot_high_count_probe_bar(run_dir / "high_count_probe_accuracy_bar.png", split_rows)
    return split_rows, count_rows
...
def write_summary(
    ...
    plot_lines(
        summary_dir / "final_answer_accuracy_vs_gold_count.png",
        all_count_rows,
        value_key="accuracy",
        ylabel="Final answer accuracy",
        variant_lines=True,
    )
    plot_lines(
    ...
    plot_lines(
        summary_dir / "mean_predicted_count_vs_gold_count.png",
        all_count_rows,
        value_key="mean_predicted_count",
        ylabel="Mean predicted count",
        variant_lines=True,
    ...
    plot_bar(
        summary_dir / "high_count_final_accuracy_comparison.png",
        high_rows,
        label_key="variant",
        value_key="high_count_5_8_accuracy",
        ylabel="Gold 5..8 final accuracy",
    )


def write_probe_summary(
    summary_dir: Path,
    probe_split_rows: Sequence[Dict[str, Any]],
    probe_count_rows: Sequence[Dict[str, Any]],
    no_plots: bool,
) -> None:
    summary_dir.mkdir(parents=True, exist_ok=True)
    write_csv_dynamic(summary_dir / "probe_summary_metrics.csv", probe_split_rows, ["variant", "probe", "split"])
    write_json(summary_dir / "probe_summary_metrics.json", {"rows": list(probe_split_rows)})
    write_csv_dynamic(
        summary_dir / "probe_summary_metrics_by_count.csv",
        probe_count_rows,
        ["variant", "probe", "split", "gold_count"],
    )
    if no_plots or not probe_split_rows:
        return
    plot_probe_accuracy_by_count(summary_dir / "probe_accuracy_vs_gold_count.png", probe_count_rows)
    plot_high_count_probe_bar(summary_dir / "high_count_probe_accuracy_comparison.png", probe_split_rows)
...
def main() -> int:
    ...
    if any(variant in SLOT_VARIANT_TO_K for variant in variants) and 17 not in carrier_layers:
        raise ValueError("carrier slot probes require layer 17 in --carrier-layers")
    ...
    all_probe_split_rows: List[Dict[str, Any]] = []
    all_probe_count_rows: List[Dict[str, Any]] = []
    for variant in variants:
        ...
        try:
            ...
            config = {
                ...
                "carrier_slots": {
                    "enabled": variant in SLOT_VARIANT_TO_K,
                    "layers": carrier_layers,
                    "num_slots_per_carrier": int(SLOT_VARIANT_TO_K[variant]) if variant in SLOT_VARIANT_TO_K else 0,
                    "source": "one alpha*v message per real visual frame",
                    "target": "queried room and character carrier tokens only",
                    "external_sequence_unchanged": True,
                    "prompt_visible_memory_tokens": 0,
                ...
                "probes": {
                    "enabled": not bool(args.no_probes),
                    "layer": 17,
                    "epochs": int(args.probe_epochs),
                    "lr": float(args.probe_lr),
                    "weight_decay": float(args.probe_weight_decay),
                    "batch_size": int(args.probe_batch_size),
                    "baseline_features": "room+character carrier hidden states after layer 17",
                    "slot_features": "internal slot states captured from the layer-17 carrier-slot adapter",
                },
            ...
            if not bool(args.no_probes):
                feature_payloads = collect_and_save_probe_features(
                    variant=variant,
                    run_dir=run_dir,
                    model=model,
                    processor=processor,
                ...
                probe_split_rows, probe_count_rows = train_and_evaluate_probes(
                    variant=variant,
                    run_dir=run_dir,
                ...
                all_probe_split_rows.extend(probe_split_rows)
                all_probe_count_rows.extend(probe_count_rows)
    ...
    if not bool(args.no_probes):
        write_probe_summary(summary_dir, all_probe_split_rows, all_probe_count_rows, bool(args.no_plots))
