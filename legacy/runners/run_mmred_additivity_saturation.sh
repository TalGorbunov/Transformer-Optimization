#!/bin/bash
#SBATCH --job-name=mmred-additivity
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
...
CORRUPTED_ROOT="${CORRUPTED_ROOT:-data/mmred_corrupted}"
METADATA_PATH="${METADATA_PATH:-data/mmred}"

EXPERIMENT_DIR="${EXPERIMENT_DIR:-outputs/mmred_additivity_saturation}"
RUN_NAME="${RUN_NAME:-$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-${EXPERIMENT_DIR}/${RUN_NAME}}"
CACHE_DIR="${CACHE_DIR:-${EXPERIMENT_DIR}/cache}"

SEQ_LENS="${SEQ_LENS:-4 8}"
SPLIT="${SPLIT:-all_uniform}"
EVIDENCE_COUNTS="${EVIDENCE_COUNTS:-all}"
MAX_SAMPLES_PER_SEQ_LEN="${MAX_SAMPLES_PER_SEQ_LEN:-20}"
LAYERS="${LAYERS:-36 38 40 41 42 43 44 45 46 48 50 52 56 60 63}"
...
CMD=(
  python -u
  evaluations/scripts/eval_mmred_additivity_saturation.py
  --model-id "${MODEL_ID}"
  --data-root "${DATA_ROOT}"
  --corrupted-root "${CORRUPTED_ROOT}"
