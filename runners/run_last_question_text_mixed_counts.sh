#!/bin/bash
#SBATCH --job-name=last-qtext-mixed-counts
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
...
EXPERIMENT_DIR="${EXPERIMENT_DIR:-outputs/last_question_text_mixed_counts_T30}"
RUN_NAME="${RUN_NAME:-$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-${EXPERIMENT_DIR}/${RUN_NAME}}"
CACHE_DIR="${CACHE_DIR:-${EXPERIMENT_DIR}/cache}"

SEQ_LENS="${SEQ_LENS:-2 4 6}"
...
CMD=(
  python -u
  evaluations/scripts/eval_mmred_last_question_text_mixed_counts.py
  --model-id "${MODEL_ID}"
  --data-root "${DATA_ROOT}"
