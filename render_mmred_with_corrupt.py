...
def main():
    ...
    # Corruption mode: create corrupted samples from already-rendered samples
    ap.add_argument("--corrupt_from", default=None, help="Path to rendered samples root (e.g., data/mmred_images/seq_len_16/train)")
    ap.add_argument("--corrupt_out", default="data/mmred_corrupted", help="Output root for corrupted samples")
    ap.add_argument("--corrupt_limit", type=int, default=None, help="Max number of sample dirs to corrupt (after filtering)")
    args = ap.parse_args()

    # If --corrupt_from is provided, we do NOT load the dataset.
    # We iterate existing rendered sample dirs and create per-evidence-frame corruptions.
    if args.corrupt_from is not None:
        in_root = args.corrupt_from
        # infer split from path if possible, otherwise fall back to --split
        split = args.split
        # common layout: .../<config>/<split>
        tail = os.path.basename(os.path.normpath(in_root))
        if tail in ("train", "val", "test"):
            split = tail

        # Collect sample directories (any directory that contains qa.txt), robust to extra nesting.
        sample_dirs = []
        for dirpath, dirnames, filenames in os.walk(in_root):
            if "qa.txt" in filenames:
                sample_dirs.append(dirpath)
        sample_dirs = sorted(set(sample_dirs))

        print(f"Found {len(sample_dirs)} rendered samples under: {os.path.abspath(in_root)}")

        if args.corrupt_limit is not None:
            sample_dirs = sample_dirs[: args.corrupt_limit]

        total_created = 0
        for sd in sample_dirs:
            try:
                total_created += create_all_corruptions_for_sample(sd, args.corrupt_out, split)
