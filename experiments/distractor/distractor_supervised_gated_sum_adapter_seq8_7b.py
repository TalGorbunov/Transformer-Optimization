...
EXPERIMENT_NAME = "distractor_supervised_gated_sum_adapter_seq8_7b"
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "outputs" / EXPERIMENT_NAME
DEFAULT_DATASET_ROOT = PROJECT_ROOT / "data" / "mmred_images_park"

BASELINE = "baseline"
SUPERVISED_GATED_SUM = "supervised_gated_sum_adapter"
SUPERVISED_GATED_SUM_READOUT = "supervised_gated_sum"
NUM_FRAMES = 8
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Distractor seq_len=8 experiment: frozen Qwen baseline vs a learned "
            "supervised sigmoid evidence selector that gates exact frame messages "
            "before additive injection into the last prompt token."
        )
    )
    ...
    parser.add_argument("--run-supervised-gated-sum", action="store_true", default=False)
...
def default_output_dir(args: argparse.Namespace) -> Path:
    name = str(args.run_name).strip() or f"supervised_gated_sum_{time.strftime('%Y%m%d_%H%M%S')}"
...
def evidence_mask_for_record(record: prev.SampleRecord, *, seq_len: int = NUM_FRAMES) -> List[int]:
    ...
    if not mask:
        raise RuntimeError(f"sample_id={record.sample_id}: evidence_frame_mask is missing")
    if len(mask) != int(seq_len):
        raise RuntimeError(
            f"sample_id={record.sample_id}: evidence_frame_mask length {len(mask)} != seq_len {seq_len}"
        )
    clean = [int(x) for x in mask]
    if any(value not in (0, 1) for value in clean):
        raise RuntimeError(f"sample_id={record.sample_id}: evidence mask is not binary: {clean}")
    selected = int(sum(clean))
    if selected != int(record.gold_count):
        raise RuntimeError(
            f"sample_id={record.sample_id}: evidence mask selects {selected} frames but gold_count={record.gold_count}"
        )
    if selected != int(record.evidence_count):
        raise RuntimeError(
            f"sample_id={record.sample_id}: evidence mask selects {selected} frames but evidence_count={record.evidence_count}"
        )
    return clean


def validate_evidence_masks(records: Sequence[prev.SampleRecord], seq_len: int = NUM_FRAMES) -> Dict[str, Any]:
...
def evidence_masks_for_records(records: Sequence[prev.SampleRecord], device: str) -> torch.Tensor:
...
class SupervisedGatedSumEvidenceAdapter(sum_base.SimpleSumEvidenceAdapter):
    """Learn independent sigmoid evidence gates, sum gated messages, and inject."""
    ...
    def __init__(
        ...
        *,
        ...
        self.readout_mode = SUPERVISED_GATED_SUM_READOUT
    ...
    def set_context(
        ...
        *,
        ...
        query_positions: Sequence[Sequence[int]],
        ...
        if int(evidence_frame_masks.shape[1]) != self._num_frames():
            raise ValueError(
                f"evidence mask frame dimension {int(evidence_frame_masks.shape[1])} != frame groups {self._num_frames()}"
            )
        self._query_positions = [[int(pos) for pos in positions] for positions in query_positions]
        ...
        self._last_stats["gate_bce_by_layer"] = {}
        self._last_stats["gate_count_error_by_layer"] = {}
        self._last_stats["gate_count_loss_by_layer"] = {}
        self._last_stats["gated_message_norm_by_layer"] = {}
    ...
    def inject_before_layer(
        ...
        gated_messages = projected_messages.float() * alpha.unsqueeze(-1).float()
        summed = gated_messages.sum(dim=1)
        ...
        target = self._evidence_frame_masks.to(device=alpha.device, dtype=alpha.dtype)
        gate_bce = F.binary_cross_entropy(alpha.clamp(1e-6, 1.0 - 1e-6), target, reduction="none").mean(dim=-1)
        gold_counts = self._gold_counts.to(device=alpha.device, dtype=alpha.dtype) if self._gold_counts is not None else alpha.sum(dim=-1)
        gate_count_error = alpha.sum(dim=-1) - gold_counts
        ...
        self._last_stats["gated_message_norm_by_layer"][layer_key] = (
            gated_messages.detach().float().norm(dim=-1).cpu().tolist()
        )
        ...
        self._last_stats["gate_bce_by_layer"][layer_key] = gate_bce.detach().float().cpu().tolist()
        self._last_stats["gate_count_error_by_layer"][layer_key] = gate_count_error.detach().float().cpu().tolist()
        self._last_stats["gate_count_loss_by_layer"][layer_key] = gate_count_error.detach().float().pow(2).cpu().tolist()
        self._last_stats["message_mode_by_layer"][layer_key] = [mode for _ in range(batch)]
        return out


def adapter_set_context(adapter: SupervisedGatedSumEvidenceAdapter, batch: carrier.MemoryBatch, records: Sequence[prev.SampleRecord]) -> None:
    adapter.set_context(
        ...
        evidence_frame_masks=evidence_masks_for_records(records, str(batch.prompt_last_indices.device)),
...
def blank_diagnostics(layers: Sequence[int]) -> Dict[str, Any]:
    ...
    payload["gate_values_by_layer"] = {str(int(layer)): [] for layer in layers}
    payload["gate_sum_by_layer"] = {str(int(layer)): 0.0 for layer in layers}
    payload["gate_bce_by_layer"] = {str(int(layer)): 0.0 for layer in layers}
    payload["gate_count_error_by_layer"] = {str(int(layer)): 0.0 for layer in layers}
    payload["gate_count_loss_by_layer"] = {str(int(layer)): 0.0 for layer in layers}
    payload["gated_message_norm_by_layer"] = {str(int(layer)): [] for layer in layers}
    return payload
...
def mean_gate_stats(gate_by_layer: Dict[str, Any], evidence_mask: Sequence[int]) -> Dict[str, float]:
    evidence_values: List[float] = []
    distractor_values: List[float] = []
    gate_sums: List[float] = []
    mask = [int(x) for x in evidence_mask]
    if not isinstance(gate_by_layer, dict):
        return {"mean_gate_evidence": math.nan, "mean_gate_distractor": math.nan, "gate_sum": math.nan}
    for values in gate_by_layer.values():
        if not isinstance(values, list):
            continue
        gates = [float(v) for v in values if base.finite_float(v) is not None]
        if not gates:
            continue
        gate_sums.append(float(sum(gates)))
        for frame_idx, gate in enumerate(gates[: len(mask)]):
            if int(mask[frame_idx]) == 1:
                evidence_values.append(float(gate))
            else:
                distractor_values.append(float(gate))
    return {
        "mean_gate_evidence": base.finite_mean(evidence_values),
        "mean_gate_distractor": base.finite_mean(distractor_values),
        "gate_sum": base.finite_mean(gate_sums),
    }
...
def f1_at_threshold(labels: Sequence[int], scores: Sequence[float], threshold: float = 0.5) -> float:
    ...
    tp = sum(1 for y, pred in zip(labels, preds) if int(y) == 1 and int(pred) == 1)
    fp = sum(1 for y, pred in zip(labels, preds) if int(y) == 0 and int(pred) == 1)
    fn = sum(1 for y, pred in zip(labels, preds) if int(y) == 1 and int(pred) == 0)
    denom = 2 * tp + fp + fn
    return float(2 * tp / denom) if denom else math.nan


def gate_metrics_from_rows(
    rows: Sequence[Dict[str, Any]],
    *,
    split_name: str,
    layers: Sequence[int],
) -> List[Dict[str, Any]]:
    gated_rows = [row for row in rows if row.get("method") == SUPERVISED_GATED_SUM]
    out: List[Dict[str, Any]] = []
    for layer_key in ["overall"] + [str(int(layer)) for layer in layers]:
        labels: List[int] = []
        scores: List[float] = []
        ...
        for row in gated_rows:
            mask = base.parse_json_field(row, "evidence_frame_mask_json", [])
            gates_by_layer = base.parse_json_field(row, "gate_values_by_layer_json", {})
            if not isinstance(mask, list) or not isinstance(gates_by_layer, dict):
                continue
            selected_layers = (
                [values for values in gates_by_layer.values() if isinstance(values, list)]
                if layer_key == "overall"
                else [gates_by_layer.get(layer_key, [])]
            )
            sample_gate_sums: List[float] = []
            for values in selected_layers:
                if not isinstance(values, list):
                    continue
                gates = [float(v) for v in values if base.finite_float(v) is not None]
                if not gates:
                    continue
                sample_gate_sums.append(float(sum(gates)))
                for frame_idx, gate in enumerate(gates[: len(mask)]):
                    label = int(mask[frame_idx])
                    labels.append(label)
                    scores.append(float(gate))
                    if label == 1:
                        evidence_scores.append(float(gate))
                    else:
                        distractor_scores.append(float(gate))
            if sample_gate_sums:
                gate_sums.append(base.finite_mean(sample_gate_sums))
                gold_counts.append(int(row.get("gold_count", 0)))
        out.append(
            {
                "split": str(split_name),
                "method": SUPERVISED_GATED_SUM,
                "layer": str(layer_key),
                "n_samples": len(gated_rows),
                "n_frame_scores": len(scores),
                "evidence_gate_auroc": prev.auroc_binary(labels, scores) if labels else math.nan,
                "evidence_gate_f1_at_0_5": f1_at_threshold(labels, scores) if labels else math.nan,
                "mean_alpha_evidence": base.finite_mean(evidence_scores),
                "mean_alpha_distractor": base.finite_mean(distractor_scores),
                "mean_gate_sum": base.finite_mean(gate_sums),
                "gate_count_mae": base.finite_mean(
                    [abs(float(gate_sum) - float(gold)) for gate_sum, gold in zip(gate_sums, gold_counts)]
                ),
                "corr_gate_sum_gold_count": carrier.correlation(gate_sums, gold_counts),
            }
        )
    return out
...
def evaluate_model(
    *,
    ...
    model: Any,
    processor: Any,
    adapter: Optional[SupervisedGatedSumEvidenceAdapter],
    records: Sequence[prev.SampleRecord],
    ...
    try:
        ...
        for batch_num, batch_indices in enumerate(batches, start=1):
            ...
            for row_idx, sample_idx in enumerate(batch_indices):
                ...
                evidence_mask = evidence_mask_for_record(record, seq_len=NUM_FRAMES)
                ...
                gate_values_by_layer = diag.get("gate_values_by_layer", {})
                gate_bce_by_layer = diag.get("gate_bce_by_layer", {})
                gate_count_error_by_layer = diag.get("gate_count_error_by_layer", {})
                gate_count_loss_by_layer = diag.get("gate_count_loss_by_layer", {})
                ...
                gate_stats = mean_gate_stats(gate_values_by_layer, evidence_mask)
                gate_sum = gate_stats["gate_sum"]
                gate_count_error = float(gate_sum) - float(gold) if base.finite_float(gate_sum) is not None else math.nan
                ...
                rows.append(
                    {
                        ...
                        "evidence_frame_mask_json": base.json_compact(evidence_mask),
                        "gate_values_by_layer_json": base.json_compact(gate_values_by_layer) if adapter is not None else "",
                        "mean_gate_evidence": float(gate_stats["mean_gate_evidence"]) if adapter is not None else "",
                        "mean_gate_distractor": float(gate_stats["mean_gate_distractor"]) if adapter is not None else "",
                        "gate_sum": float(gate_sum) if adapter is not None else "",
                        "gate_count_error": float(gate_count_error) if adapter is not None else "",
                        ...
                        "gated_message_norm_by_layer_json": base.json_compact(
                            diag.get("gated_message_norm_by_layer", {})
                        )
                        if adapter is not None
                        else "",
                        "gate_bce_by_layer_json": base.json_compact(gate_bce_by_layer)
                        if adapter is not None
                        else "",
                        "gate_count_error_by_layer_json": base.json_compact(gate_count_error_by_layer)
                        if adapter is not None
                        else "",
                        "gate_count_loss_by_layer_json": base.json_compact(gate_count_loss_by_layer)
                        if adapter is not None
                        else "",
...
def train_adapter(
    *,
    ...
    inject_layers: Sequence[int],
    device: str,
) -> Tuple[SupervisedGatedSumEvidenceAdapter, List[Dict[str, Any]], Dict[str, Any], Path]:
    adapter = SupervisedGatedSumEvidenceAdapter(
        hidden_size=int(hidden_size),
    ...
    checkpoint_path = ckpt_dir / "supervised_gated_sum_adapter_best.pt"
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        try:
            ...
            for step, batch_indices in enumerate(train_batches, start=1):
                ...
                mask_loss, count_loss = adapter.gate_supervision_loss(count_logits.device)
                update_energy = adapter.update_energy_for_loss(count_logits.device)
                loss = (
                    ce
                    + float(args.lambda_mask) * mask_loss
                    + float(args.lambda_count) * count_loss
                ...
                train_mask_total += float(mask_loss.detach().cpu().item())
                train_count_total += float(count_loss.detach().cpu().item())
                ...
                if step == 1 or step % 25 == 0:
                    print(
                        f"  {SUPERVISED_GATED_SUM} epoch={epoch} step={step}/{len(train_batches)} "
                        f"train_ce={train_ce_total / max(1, train_steps):.4f} "
                        f"mask_bce={train_mask_total / max(1, train_steps):.4f} "
                        f"count_loss={train_count_total / max(1, train_steps):.4f} "
        ...
        row = {
            "method": SUPERVISED_GATED_SUM,
            "readout_mode": SUPERVISED_GATED_SUM_READOUT,
            ...
            "train_gate_bce": train_mask_total / max(1, train_steps),
            "train_gate_count_loss": train_count_total / max(1, train_steps),
        ...
        history.append(row)
        print(
            f"  {SUPERVISED_GATED_SUM} epoch={epoch} train_ce={row['train_ce']:.4f} "
            f"mask_bce={row['train_gate_bce']:.4f} count_loss={row['train_gate_count_loss']:.4f} "
...
def summarize_method(
    ...
    *,
    ...
    return {
        ...
        "query_token_group": infer_column(rows, "query_token_group", default=canonical_group("all_question")),
        ...
        "mean_gate_evidence": base.finite_mean(row.get("mean_gate_evidence") for row in rows)
        if method == SUPERVISED_GATED_SUM
        else 0.0,
        "mean_gate_distractor": base.finite_mean(row.get("mean_gate_distractor") for row in rows)
        if method == SUPERVISED_GATED_SUM
        else 0.0,
        "mean_gate_sum": base.finite_mean(row.get("gate_sum") for row in rows)
        if method == SUPERVISED_GATED_SUM
        else 0.0,
        "mean_gate_count_error": base.finite_mean(row.get("gate_count_error") for row in rows)
        if method == SUPERVISED_GATED_SUM
        else 0.0,
        ...
        "mean_summed_message_norm": base.finite_mean(
            (row.get("summed_message_norm") for row in rows), default=0.0
        )
        if method == SUPERVISED_GATED_SUM
        else 0.0,
        ...
        out.append(
            {
                ...
                "mean_gate_evidence": base.finite_mean(row.get("mean_gate_evidence") for row in count_rows),
                "mean_gate_distractor": base.finite_mean(row.get("mean_gate_distractor") for row in count_rows),
                "mean_gate_sum": base.finite_mean(row.get("gate_sum") for row in count_rows),
                "gate_count_mae": base.finite_mean(
                    [abs(float(row["gate_count_error"])) for row in count_rows if base.finite_float(row.get("gate_count_error")) is not None]
                ),
                ...
                "mean_summed_message_norm": base.finite_mean(
                    (row.get("summed_message_norm") for row in count_rows),
                    default=0.0,
                )
                if method == SUPERVISED_GATED_SUM
                else 0.0,
...
def comparison_by_evidence_count(accuracy_rows: Sequence[Dict[str, Any]], counts: Sequence[int]) -> List[Dict[str, Any]]:
    ...
    for count in counts:
        ...
        gated_row = by_key.get((SUPERVISED_GATED_SUM, int(count)), {})
        base_acc = base.finite_float(base_row.get("accuracy"))
        gated_acc = base.finite_float(gated_row.get("accuracy"))
        out.append(
            {
                "evidence_count": int(count),
                "gold_count": int(count),
                "baseline_accuracy": "" if base_acc is None else float(base_acc),
                "supervised_gated_sum_accuracy": "" if gated_acc is None else float(gated_acc),
                "delta_accuracy": ""
                if base_acc is None or gated_acc is None
                else float(gated_acc) - float(base_acc),
                "baseline_mean_pred": base_row.get("mean_pred_count", ""),
                "supervised_gated_sum_mean_pred": gated_row.get("mean_pred_count", ""),
                "baseline_mean_margin": base_row.get("mean_margin", ""),
                "supervised_gated_sum_mean_margin": gated_row.get("mean_margin", ""),
                "supervised_gated_sum_mean_gate_sum": gated_row.get("mean_gate_sum", ""),
                "supervised_gated_sum_gate_count_mae": gated_row.get("gate_count_mae", ""),
            }
        )
    return out
...
def save_combined_line_plot(
    ...
    *,
    ...
    for method in [BASELINE, SUPERVISED_GATED_SUM]:
...
def save_combined_confusions(path: Path, rows: Sequence[Dict[str, Any]], counts: Sequence[int]) -> None:
    ...
    for ax, method in zip(axes, [BASELINE, SUPERVISED_GATED_SUM]):
...
def make_plots(
    ...
    gate_metric_rows: Sequence[Dict[str, Any]],
    counts: Sequence[int],
) -> None:
    ...
    gated_rows = [row for row in metrics_rows if row["method"] == SUPERVISED_GATED_SUM]
    save_confusion(plots_dir / "predicted_count_confusion_matrix_baseline.png", base_rows, counts, "Baseline Confusion Matrix")
    save_confusion(
        plots_dir / "predicted_count_confusion_matrix_supervised_gated_sum_adapter.png",
        gated_rows,
        counts,
        "Supervised Gated Sum Adapter Confusion Matrix",
    )
    ...
    plt.ylabel("Supervised gated sum minus baseline accuracy")
    plt.title("Delta Accuracy")
    plt.xticks(counts)
    plt.grid(axis="y", alpha=0.25)
    plt.tight_layout()
    plt.savefig(plots_dir / "delta_accuracy_supervised_gated_sum_minus_baseline.png", dpi=180, bbox_inches="tight")
    plt.close()

    by_count = {int(row["evidence_count"]): row for row in accuracy_rows if row["method"] == SUPERVISED_GATED_SUM}
    diagnostic_specs = [
        ("mean_update_norm", "update_norm_vs_evidence_count.png", "Mean update norm", "Supervised Gated Sum Update Norm"),
        ("mean_message_norm", "message_norm_vs_evidence_count.png", "Mean message norm", "Projected Message Norm"),
        (
            "mean_summed_message_norm",
            "summed_message_norm_vs_evidence_count.png",
            "Mean summed message norm",
            "Summed Gated Message Norm",
        ),
    ]
    ...
    plt.figure(figsize=(7.2, 4.3))
    gate_sums = [float(by_count.get(int(count), {}).get("mean_gate_sum", math.nan)) for count in counts]
    plt.plot([0, max(counts)], [0, max(counts)], linestyle="--", color="black", linewidth=1.1, label="perfect y=x")
    plt.plot(counts, gate_sums, marker="o", linewidth=1.8, label=SUPERVISED_GATED_SUM)
    plt.xlabel("Gold count")
    plt.ylabel("Mean gate sum")
    plt.title("Gate Sum vs Gold Count")
    plt.xticks(counts)
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
    plt.savefig(plots_dir / "gate_sum_vs_gold_count.png", dpi=180, bbox_inches="tight")
    plt.close()

    plt.figure(figsize=(7.2, 4.3))
    maes = [float(by_count.get(int(count), {}).get("gate_count_mae", math.nan)) for count in counts]
    plt.plot(counts, maes, marker="o", linewidth=1.8)
    plt.xlabel("Gold count")
    plt.ylabel("Gate count MAE")
    plt.title("Gate Count MAE vs Gold Count")
    plt.xticks(counts)
    plt.grid(alpha=0.25)
    plt.tight_layout()
    plt.savefig(plots_dir / "gate_count_mae_vs_gold_count.png", dpi=180, bbox_inches="tight")
    plt.close()

    plt.figure(figsize=(7.2, 4.3))
    ev = [float(by_count.get(int(count), {}).get("mean_gate_evidence", math.nan)) for count in counts]
    dist = [float(by_count.get(int(count), {}).get("mean_gate_distractor", math.nan)) for count in counts]
    plt.plot(counts, ev, marker="o", linewidth=1.8, label="evidence")
    plt.plot(counts, dist, marker="o", linewidth=1.8, label="distractor")
    plt.xlabel("Gold count")
    plt.ylabel("Mean alpha")
    plt.title("Mean Gate: Evidence vs Distractor")
    plt.xticks(counts)
    plt.ylim(-0.02, 1.02)
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
    plt.savefig(plots_dir / "mean_gate_evidence_vs_distractor.png", dpi=180, bbox_inches="tight")
    plt.close()

    for metric_key, filename, ylabel, title in [
        ("evidence_gate_auroc", "gate_auroc_by_layer.png", "AUROC", "Gate AUROC by Layer"),
        ("evidence_gate_f1_at_0_5", "gate_f1_by_layer.png", "F1 at 0.5", "Gate F1 by Layer"),
    ]:
        plt.figure(figsize=(7.2, 4.3))
        for split_name in sorted({str(row.get("split")) for row in gate_metric_rows if row.get("layer") != "overall"}):
            split_rows = [row for row in gate_metric_rows if str(row.get("split")) == split_name and row.get("layer") != "overall"]
            xs = [int(row["layer"]) for row in split_rows if str(row.get("layer", "")).isdigit()]
            ys = [float(row.get(metric_key, math.nan)) for row in split_rows if str(row.get("layer", "")).isdigit()]
            if xs:
                plt.plot(xs, ys, marker="o", linewidth=1.8, label=split_name)
        plt.xlabel("Layer")
        plt.ylabel(ylabel)
        plt.title(title)
        plt.xticks([int(layer) for layer in sorted({row["layer"] for row in gate_metric_rows if str(row.get("layer", "")).isdigit()}, key=int)])
        plt.ylim(-0.02, 1.02)
        plt.grid(alpha=0.25)
        plt.legend()
        plt.tight_layout()
    ...
    save_candidate_logit_curves(
        plots_dir / "candidate_logit_curves_by_evidence_count_supervised_gated_sum_adapter.png",
        metrics_rows,
        SUPERVISED_GATED_SUM,
        counts,
    )
...
def write_diagnostics(
    *,
    ...
    gated_rows = method_rows(metrics_rows, SUPERVISED_GATED_SUM)
    localization_rows = gated_rows or metrics_rows
    message_counts = [len(base.parse_json_field(row, "message_target_positions_json", [])) for row in localization_rows]
    query_counts = [len(base.parse_json_field(row, "query_positions_json", [])) for row in localization_rows]
    inject_counts = [len(base.parse_json_field(row, "inject_positions_json", [])) for row in localization_rows]
    update_values = numeric_values_from_json_field(gated_rows, "update_norm_by_layer_json")
    ...
    gate_values = numeric_values_from_json_field(gated_rows, "gate_values_by_layer_json")
    gate_bce_values = numeric_values_from_json_field(gated_rows, "gate_bce_by_layer_json")
    gate_count_loss_values = numeric_values_from_json_field(gated_rows, "gate_count_loss_by_layer_json")
    finite_gate_values = bool(gate_values) and all(math.isfinite(float(value)) for value in gate_values)
    gate_values_in_range = bool(gate_values) and all(0.0 <= float(value) <= 1.0 for value in gate_values)
    finite_gate_losses = bool(gate_bce_values or gate_count_loss_values) and all(
        math.isfinite(float(value)) for value in list(gate_bce_values) + list(gate_count_loss_values)
    )
    train_loss_values = [
        value
        for row in train_history
        for value in (row.get("train_gate_bce"), row.get("train_gate_count_loss"))
        if value != ""
    ]
    finite_train_gate_losses = bool(train_loss_values) and all(base.finite_float(value) is not None for value in train_loss_values)
    score_fields = sorted(
        {
            key
            for row in metrics_rows
            for key in row.keys()
            if "matrix_score" in str(key) or "key_query" in str(key) or "raw_matrix" in str(key)
        }
    )
    ...
    forbidden_param_names = [
        ...
        if any(part in name for part in ("w_q", "w_k", "w_v", "readout", "key_query", "raw_matrix"))
    ]
    ...
    for name in trainable_param_names:
        if not (
            name == "gamma"
            or name.startswith("message_to_memory.")
            or name.startswith("w_o.")
            or name.startswith("selector_mlp.")
        ):
    ...
    query_localization_ok = all(
        len(base.parse_json_field(row, "query_positions_json", [])) > 0
        for row in localization_rows
    )
    ...
    mask_counts = [int(sum(int(x) for x in base.parse_json_field(row, "evidence_frame_mask_json", []))) for row in mask_rows]
    mask_count_equals_gold = all(int(mask_count) == int(row.get("gold_count", -2)) for mask_count, row in zip(mask_counts, mask_rows))
    mean_selected = base.finite_mean(mask_counts, default=0.0)
    ...
    payload = {
        ...
        "only_adapter_expected_parameters_trainable": int(bool(adapter is not None and allowed_trainable_ok)),
        "message_token_group": canonical_group(str(args.message_token_group)),
        "query_token_group": canonical_group(str(args.query_token_group)),
        "inject_token_group": canonical_group(str(args.inject_token_group)),
        ...
        "evidence_mask_validation": evidence_validation,
        "evidence_masks_found_for_all_samples": int(len(mask_rows) == len(metrics_rows) and bool(mask_rows)),
        ...
        "per_sample_evidence_mask_count_equals_gold_count": int(mask_count_equals_gold and bool(mask_rows)),
        "evidence_masks_binary": int(masks_binary and bool(mask_rows)),
        "distractor_frames_have_mask_zero": int(distractor_frames_zero and bool(mask_rows)),
        "gate_values_finite": int(finite_gate_values),
        "gate_values_in_0_1": int(gate_values_in_range),
        "gate_bce_count_losses_finite": int(finite_gate_losses and finite_train_gate_losses),
        "gate_metrics": [dict(row) for row in gate_metric_rows],
        "finite_update_norms": int(finite_updates),
        "nonzero_updates": int(nonzero_updates),
        "raw_matrix_query_key_readout_scores_used": int(bool(score_fields)),
        "raw_matrix_query_key_readout_score_fields": score_fields,
        "raw_matrix_query_key_readout_parameters_present": int(bool(forbidden_param_names)),
        "raw_matrix_query_key_readout_parameter_names": forbidden_param_names,
        "no_raw_matrix_query_key_readout_used": int(not score_fields and not forbidden_param_names),
        "all_question_localization_ok": int(bool(all_question_localization_ok)),
        "query_token_localization_ok": int(bool(query_localization_ok)),
...
def write_readme(
    ...
    gated_acc = base.finite_float(summary.get(SUPERVISED_GATED_SUM, {}).get("accuracy"))
    improved = base_acc is not None and gated_acc is not None and float(gated_acc) > float(base_acc)
    base_mae = mean_pred_mae(accuracy_rows, BASELINE)
    gated_mae = mean_pred_mae(accuracy_rows, SUPERVISED_GATED_SUM)
    better_diagonal = base.finite_float(base_mae) is not None and base.finite_float(gated_mae) is not None and gated_mae < base_mae
    update_norm = base.finite_mean(
        (row.get("update_norm") for row in metrics_rows if row.get("method") == SUPERVISED_GATED_SUM), default=0.0
    )
    ...
    gate_overall = {
        ...
        for row in diagnostics.get("gate_metrics", [])
        if str(row.get("layer")) == "overall"
    }
    test_gate = gate_overall.get("test", {})
    test_auroc = base.finite_float(test_gate.get("evidence_gate_auroc"))
    test_f1 = base.finite_float(test_gate.get("evidence_gate_f1_at_0_5"))

    lines = [
        "# Distractor Supervised Gated Sum Adapter seq_len=8 7B",
        "",
        "This experiment tests whether the oracle-mask result can be approached by a learned evidence selector.",
        "",
        "For each injection layer, the adapter extracts exact frame-to-all-question-token messages, pools a question/query vector, predicts independent sigmoid gates alpha_f with direct evidence-mask supervision, sums alpha_f * W_m m_f, and injects the result into the last prompt token.",
        "",
        "h_last^l <- h_last^l + gamma_l * W_o s^l",
        "",
        "There is no raw-matrix readout, no key/value query attention, and no softmax over frames.",
        "",
        "## Automatic Interpretation",
        "",
        (
            f"- Did supervised gating improve over baseline? {bool(improved)} "
            f"(baseline={base_acc if base_acc is not None else math.nan:.4f}, "
            f"supervised-gated-sum={gated_acc if gated_acc is not None else math.nan:.4f})."
        ),
        (
            f"- Does mean predicted count follow y=x better than baseline? {bool(better_diagonal)} "
            f"(baseline mean-pred MAE={base_mae:.4f}, supervised-gated-sum={gated_mae:.4f})."
        ),
        f"- Test gate AUROC={test_auroc if test_auroc is not None else math.nan:.4f}; F1@0.5={test_f1 if test_f1 is not None else math.nan:.4f}.",
        (
        ...
        f"- Were evidence masks found for all samples? {bool(diagnostics.get('evidence_masks_found_for_all_samples'))}.",
        f"- Are gate values finite and in [0, 1]? {bool(diagnostics.get('gate_values_finite')) and bool(diagnostics.get('gate_values_in_0_1'))}.",
        f"- Are gate BCE/count losses finite? {bool(diagnostics.get('gate_bce_count_losses_finite'))}.",
        f"- Was raw-matrix/query-key readout avoided? {bool(diagnostics.get('no_raw_matrix_query_key_readout_used'))}.",
        ...
        "",
        "- If gate AUROC/F1 are high but answer accuracy is low, injection compatibility is still the problem.",
        "- If gate AUROC/F1 are low, evidence selection is the problem.",
        "- If gate_sum tracks gold_count and answer accuracy improves, supervised gating is a strong candidate method.",
        "- Compare final test accuracy and gate quality against the oracle-mask sum adapter.",
        "",
        "## Files",
        "",
        "- `metrics.csv`: per-sample logits, predictions, evidence masks, gate values, token positions, and additive-sum diagnostics.",
        "- `summary.csv`: overall frozen baseline and supervised-gated-sum summary.",
        "- `accuracy_by_evidence_count.csv`: accuracy and prediction histograms by evidence/gold count.",
        "- `comparison_by_evidence_count.csv`: baseline vs supervised-gated-sum deltas.",
        "- `train_history.csv`: adapter training and validation history.",
        "- `gate_metrics.csv`: selector AUROC/F1, mean evidence/distractor alpha, gate-sum MAE, and gate-count correlation over val/test.",
        "- `diagnostics.json`: frozen-model, trainability, hook, exact-message, evidence-mask, gate, update-norm, and no-query/key checks.",
        "- `plots/`: requested comparison, confusion, candidate-logit, update/message-norm, and gate plots.",
    ]
    ...
    *,
    ...
    base.write_csv_dynamic(
        ...
        [
            ...
            "gated_message_norm_by_layer_json",
            "gate_bce_by_layer_json",
            "gate_count_error_by_layer_json",
            "gate_count_loss_by_layer_json",
    ...
    base.write_csv_dynamic(
        ...
        [
            ...
            "gold_count",
            "baseline_accuracy",
            "supervised_gated_sum_accuracy",
            "delta_accuracy",
            "baseline_mean_pred",
            "supervised_gated_sum_mean_pred",
            "baseline_mean_margin",
            "supervised_gated_sum_mean_margin",
            "supervised_gated_sum_mean_gate_sum",
            "supervised_gated_sum_gate_count_mae",
        ],
    )
    base.write_csv_dynamic(
    ...
    base.write_csv_dynamic(
        ...
        [
            ...
            "n_frame_scores",
            "evidence_gate_auroc",
            "evidence_gate_f1_at_0_5",
            "mean_alpha_evidence",
            "mean_alpha_distractor",
            "mean_gate_sum",
            "gate_count_mae",
            "corr_gate_sum_gold_count",
        ],
    )
...
def main() -> int:
    ...
    if not (args.run_baseline or args.run_supervised_gated_sum or args.run_all):
        args.run_all = True

    should_run_baseline = bool(args.run_baseline or args.run_all)
    should_run_gated = bool(args.run_supervised_gated_sum or args.run_all)
    ...
    try:
        ...
        run_config = {
            ...
            "run_supervised_gated_sum": bool(should_run_gated),
            "d_mem": int(args.d_mem),
        ...
        evidence_validation = validate_evidence_masks(records, seq_len=NUM_FRAMES)
        base.write_json(output_dir / "evidence_mask_manifest.json", evidence_validation)
        print(f"Evidence mask validation: {base.json_compact(evidence_validation)}")
        ...
        if should_run_gated and (not train_indices or not val_indices):
            raise RuntimeError("Supervised gated adapter training requires non-empty train and val splits")
        if not test_indices:
        ...
        backward_diag: Dict[str, Any] = {}
        adapter: Optional[SupervisedGatedSumEvidenceAdapter] = None
        ...
        if should_run_gated:
            print("Training supervised gated sum adapter")
            ...
            base.write_json(
                ...
                {
                    "supervised_gated_sum_best_checkpoint": os.fspath(checkpoint_path),
                    "readout_mode": SUPERVISED_GATED_SUM_READOUT,
                    "message_token_group": str(args.message_token_group),
                    "query_token_group": str(args.query_token_group),
                    "inject_token_group": str(args.inject_token_group),
            ...
            print("Evaluating supervised gated sum adapter on val split for gate diagnostics")
            gated_val_eval = evaluate_model(
                args=args,
                method=SUPERVISED_GATED_SUM,
                split_name="val",
                model=model,
            ...
            gate_eval_rows.extend(gated_val_eval["rows"])
            print("Evaluating supervised gated sum adapter on test split")
            gated_test_eval = evaluate_model(
                args=args,
                method=SUPERVISED_GATED_SUM,
                split_name="test",
                model=model,
            ...
            metrics_rows.extend(gated_test_eval["rows"])
            gate_eval_rows.extend(gated_test_eval["rows"])
        ...
        if should_run_gated:
            summary_rows.append(
                summarize_method(
                    method_rows(metrics_rows, SUPERVISED_GATED_SUM),
                    method=SUPERVISED_GATED_SUM,
        ...
        for split_name in ["val", "test"]:
            split_rows = [row for row in gate_eval_rows if row.get("split") == split_name]
            gate_metric_rows.extend(gate_metrics_from_rows(split_rows, split_name=split_name, layers=inject_layers))
        write_outputs(
            output_dir=output_dir,
            ...
            train_history=train_history,
            gate_metric_rows=gate_metric_rows,
        )
        if not bool(args.no_plots):
            make_plots(output_dir, metrics_rows, accuracy_rows, comparison_rows, gate_metric_rows, COUNT_VALUES)
