# RESULTS.md — Research progress log (live: the fencing / carrier / readout era)
...
> - Cited output paths that no longer resolve → [ARCHIVE_MAP.md](ARCHIVE_MAP.md).
>   Cited code paths (`experiments/…`, `evaluations/…`, `models/…`, `runners/…`) → `legacy/<same path>`.
> - Clean method implementation: `gnnformer/` + `scripts/` (each script docstring names the
>   anchor entries it must reproduce). Canonical checkpoints: `checkpoints/`.

## Executive summary (updated 2026-07-30)
