"""Independent fixed N16 fresh factor audit; no fitting or native-head replay.
...
import os
import subprocess
from scripts import report_native_identity_join_factor_native_v2 as native_audit
...
def driver():
    from scripts import evaluate_native_identity_join_factor_fresh as module
    return module
...
def inputs(torch,plan):
    ...
    need(manifest['native_report']['file']==native_ref['file'] and manifest['native_report']['sha256']==native_ref['sha256']
         and manifest['native_report']['native_identity']==native_plan['native_identity'],'Fresh images must bind the same completed native model/report')
    ...
    need(Counter(r['matches_training_orientation'] for r in rows if r['panel']=='A')=={True:54,False:54},'Panel A orientation denominator differs')
...
def audit_run(torch,out,directory,plan_path,plan,rows,prepared,layouts,stats,weights,tokenizer):
    ...
    for key in ('protocol','policy','source_sha256','inherited_source_sha256','cached_report','cached_plan','native_identity','native_identity_sha256','conditioning_identity','stats_file','stats_sha256','native_report','stage_report','timing_certificate'):
    ...
    for j,(case,record) in enumerate(zip(cases,manifest['rows'])):
        ...
        outcomes.append(dict(**{key:sample[key] for key in ('sid','contrast_id','pair_id','variant','n_frames','question','gold','target_ids','panel','orientation','matches_training_orientation','trio','room_pair','family_instance_sha256')},state=state,
            generated_ids=ids,text=result['text'],raw_text=result['raw_text'],first_query_nll=nll,
            noncanonical_answer=tuple(ids) not in canonical_targets,early_eos_relative_to_target=scored['completed'] and len(ids)<len(sample['target_ids']),**scored))
...
def timing_audit(plan,native_result):
    observations=[]
    for phase,key,count in (('profile','profiles',4),('main','runs',108)):
    ...
    need(len(setups)==4 and len(times)==224 and all(math.isfinite(x) and x>0 for x in setups+times),'Complete positive native timings are required')
    setup=max(setups);observed=max(times);floor=2.30838355794549;bound=max(floor,observed);seconds=setup+1.25*270*bound+60.
    expected=dict(passed=seconds<=1200,observations=observations,setup_observations=4,trajectory_observations=224,pooled_setup_seconds=setup,
...
def envelope_audit(torch,plan,rows,prepared,layouts):
    ...
    need(len(original)==54 and len(fresh)==270,'Complete measured native/fresh N16 input envelope required')
    ...
    expected=dict(passed=True,original=original,fresh=fresh,limits=limits,old_contexts=54,fresh_contexts=270,actual_owner_bound_native_layout=True,no_additional_GPU_profile=True)
    need(saved==expected,'Independent actual input envelope differs from released CPU certificate');return dict(passed=True,old_contexts=54,fresh_contexts=270,limits=limits)
...
def resources(out,runs):
    ...
    names=set(p.JOBS.values());allocations=factor.allocation_rows(raw,names|{'identity_join_factor_fresh_eval_run'});expected={p.JOBS[a]:runs[a] for a in ARMS}
...
def report(args,out,frozen):
    ...
    analysis=dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,policy=p.POLICY,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        plan_file=str(plan_path),plan_sha256=sha(plan_path),native_report=plan['native_report'],stage_report=plan['stage_report'],timing_certificate=timing,
        ...
        strata={a:strata(runs[a]['outcomes']) for a in ARMS},descriptive_paired_transitions=transitions,
        ordinary_joint_baseline_present=False,reasoning_model_evidence_present=False)
    save(out/'analysis.json',analysis)
    lines=['# Fixed fresh N16 factor evaluation','',
