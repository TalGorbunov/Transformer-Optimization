"""All-selected-checkpoint V8 native state/cache audit, without model selection.
...
OUT = REPO/'outputs/native_aggregation_vlm/v8/checkpoint_audit'
MAIN = REPO/'outputs/native_aggregation_vlm/v8'
DATA = Path('/mnt/data/gabriele/gnn_transformer/v8_checkpoint_audit')
CKPT = Path('/mnt/ckpts/gabriele/gnn_transformer/native_aggregation_vlm_v8')
STEPS = [972, 1944, 2916, 3888, 4860]
FORCED = ['Therefore', ':']
OWN = ('scripts/audit_native_vision_v8_checkpoint.py', 'slurm/native_vision_v8_checkpoint_selftest.sbatch',
       'slurm/native_vision_v8_checkpoint_check.sbatch', 'slurm/native_vision_v8_checkpoint.sbatch')
DEPENDENCIES = tuple(dict.fromkeys((*OWN, *profile.OWN, 'scripts/native_vision_v8_pairs.py', 'gnnformer/aggregation_consistency.py')))
...
def unit(args):
    ...
    profile.local.index(out, 'V8 checkpoint audit software check', [('Summary', 'summary.json'), ('Frozen sources', 'source_hashes.json')])
    print(json.dumps(dict(passed=True, directory=str(out))), flush=True)


def verify_v8_pairing(config, train_plan, cache, directory):
    from scripts.native_vision_v8_pairs import load_pairs
    policy = config['policy']
    expected = dict(protocol='v8_paired_residual_consistency', native_arm='parallel',
        conditions=['ce', 'consistency'], seeds=[10, 11], pair_slots_per_epoch=972,
        scene_slots_per_epoch=1944, scene_presentations=77760, pair_presentations=38880,
        pairs_per_batch=8, consistency_coefficients={'ce': 0., 'consistency': 1.},
        ...
        native_dtype='torch.float16', branch_dtype='torch.float32', exact_requires_eos=True)
    need(all(policy.get(key) == value for key, value in expected.items()), 'V8 paired objective/weighting differs')
    coefficient = {'ce': 0., 'consistency': 1.}[config['condition']]
    need(config['consistency_coefficient'] == coefficient, 'Condition coefficient differs')
    ...
    for step, row in enumerate(history, 1):
        ...
        need(all(math.isfinite(row[key]) and row[key] >= 0 for key in
                 ('loss', 'ce_loss', 'consistency_loss', 'weighted_consistency_loss', 'gradient_norm')),
             'Nonfinite or negative loss/gradient statistic')
        need(row['consistency_coefficient'] == coefficient
             and math.isclose(row['weighted_consistency_loss'], coefficient*row['consistency_loss'], rel_tol=2e-6, abs_tol=2e-6)
             and math.isclose(row['loss'], row['ce_loss']+row['weighted_consistency_loss'], rel_tol=2e-6, abs_tol=2e-6)
             and row['clipped'] == (row['gradient_norm'] > 1.), 'Condition objective or clipping accounting differs')
    ...
    need(d['passed'] and d['diagnostic_only'] and d['no_fit'] and d['vlm_forward_calls'] == 0
         and d['synthetic_points'] == 216, 'Diagnostic completion binding is invalid')
    ...
    need(release['passed'] and release['plan_sha256'] == config['plan_sha256']
         ...
         and set(release['profiles']) == {'ce', 'consistency'}, 'Main condition release differs')
    ...
    for condition, binding in release['profiles'].items():
        ...
        need(p['profile'] and p['condition'] == condition and p['arm'] == 'parallel'
             and p['seed'] == 10 and p['steps'] == 32 and p['passed'] and p['computational_integrity_passed']
...
def verify_run(torch, directory):
    ...
    need(directory.parent == MAIN, 'Require a canonical V8 main run directory')
    ...
    need(arm == 'parallel' and condition in ('ce', 'consistency') and seed in (10, 11) and config['profile'] is False
    ...
    pair_artifacts = verify_v8_pairing(config, train_plan, cache, directory)
...
def check(args):
    ...
    need({(x['condition'], x['seed']) for x in runs} == {(a, s) for a in ('ce', 'consistency') for s in (10, 11)},
    ...
    plan = dict(schema_version=1, protocol='all_selected_v8_native_checkpoint_audit', source_sha256=sources(), tests=tests,
    ...
    profile.local.index(out, 'All four selected V8 checkpoints: CPU freeze', [('Plan', 'plan.json'), ('Summary', 'summary.json')])
...
def verify(path):
    ...
    plan = read(path)
    need(plan['schema_version'] == 1 and plan['protocol'] == 'all_selected_v8_native_checkpoint_audit'
...
def run(args):
    ...
    profile.local.index(out, 'All-selected V8 native checkpoint audit', [('Frozen plan', 'plan.json'),
    ...
    lines = ['# V8 selected-checkpoint software audit', '',
    ...
    profile.local.index(data, 'Selected V8 native state evidence', [('Summary', str(out/'summary.json')), ('Forward inventory', str(out/'forwards.json'))])
