"""Stage fresh V8 evaluation only; run generation/rendering/audits on Slurm CPU.
...
DESTINATION = DATA_BASE / 'v8_fresh'
OUTPUT_ROOT = REPO / 'outputs/native_aggregation_vlm/v8/data_staging'
SEED = 20260929
SOURCE_FILES = ('scripts/stage_native_vision_v8_test.py',
...
def load_inventory():
    ...
    paths += [DATA_BASE / 'v7_fresh' / f'{p}_manifest.json' for p in ('main', 'count')]
           ...
           'V8 must exclude the exact V7 prior19 plus both V7 fresh manifests')
    ...
    inventory = dict(schema_version=1, entries=entries, excluded_content_sha256=ordered,
        ...
        scope='All21 canonical manifests: the exact V7 prior19 plus V7-fresh main/count; V7 balanced training and development remain protected',
    ...
    for purpose, (lengths, golds, repeats, axis) in SPEC.items():
        ...
        for gold in golds:
            for replicate in range(repeats):
                label = f'v8_{purpose}_K{gold}_{replicate:03d}'
...
def main():
    ...
    plan = dict(schema_version=1, purpose='v8_fresh_evaluation_only', slurm_job_id=job,
        ...
        for purpose, rows in groups.items():
            ...
            for n in SPEC[purpose][0]:
                ...
                for record, sample in zip(records, selected):
                    record.update(question=sample['question'], origin='generated_v8_fresh', fresh_test_seed=SEED)
            ...
            manifests[purpose] = dict(schema_version=1, purpose=purpose, dataset_root=str(DESTINATION),
                ...
                main_count_content_disjoint=True, evaluation_status='Fresh complete context/question content, fixed before V8 model outcomes; V7 results informed the research direction',
    ...
    (output / 'REPORT.md').write_text('# V8 fresh evaluation staging\n\n'
    ...
    if not index.exists():
        index.write_text('# V8 fresh evaluation staging\n\n')
