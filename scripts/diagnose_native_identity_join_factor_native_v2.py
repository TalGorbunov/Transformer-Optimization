...
from scripts import diagnose_native_identity_join_factor_native as original
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_factor_native_v2'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_factor_native_v2')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_factor_native_v2')
PROTOCOL='identity_join_fixed_factor_native_trainability_v2';ARMS=('product','additive')
JOBS={phase:{arm:f'identity_join_factor_native_v2_{phase}_{arm}' for arm in ARMS} for phase in ('profile','main')}
...
REPAIR_NOTE='docs/paper/NATIVE_AGGREGATION_FACTOR_NATIVE_V2_REPAIR.md'
PRIOR_FAILURE=original.OUT/'check_443567/failure.json'
PRIOR_FAILURE_SHA='742887a78d7d597fa96feb9cf5ab12d92f80e681de9562fe23ede8a37e024760'
PRIOR_RELEASE=original.OUT/'source_release.json'
PRIOR_RELEASE_SHA='9a5ccccaa240355f0dc53ea1db445c3d4b828fbff814733d4ad4e3fff631cd3b'
PRIOR_GPU_NAMES={name for group in original.JOBS.values() for name in group.values()}|{'identity_join_factor_native_profile','identity_join_factor_native_main'}
OWN=('scripts/diagnose_native_identity_join_factor_native_v2.py','scripts/report_native_identity_join_factor_native_v2.py',
     'slurm/native_identity_join_factor_native_v2_check.sbatch','slurm/native_identity_join_factor_native_v2_profile.sbatch',
     'slurm/native_identity_join_factor_native_v2_main.sbatch','slurm/native_identity_join_factor_native_v2_release.sbatch',
     'slurm/native_identity_join_factor_native_v2_report.sbatch',REPAIR_NOTE)


def inherited_sources():
    need(sha(PRIOR_RELEASE)==PRIOR_RELEASE_SHA,'Original source release changed');release=read(PRIOR_RELEASE)
    values={**original.inherited_sources(),**original.sources()}
    need(len(values)==115 and values=={**release['inherited_source_sha256'],**release['source_sha256']},'Original115-source closure differs')
    for name,digest in values.items():need(sha(REPO/name)==digest,'Inherited source changed: '+name)
    return values


def verify_prior_failure(bindings):
    bind(PRIOR_FAILURE,bindings,PRIOR_FAILURE_SHA);bind(PRIOR_RELEASE,bindings,PRIOR_RELEASE_SHA)
    failure=read(PRIOR_FAILURE);release=read(PRIOR_RELEASE)
    need(failure['type']=='TypeError' and failure['message']=="Qwen2_5_VLModel.get_rope_index() missing 1 required positional argument: 'self'"
         and failure['source_sha256']==release['source_sha256']==original.sources() and failure['partial_outputs_retained'] is True
         and not (PRIOR_FAILURE.parent/'plan.json').exists() and not (PRIOR_FAILURE.parent/'summary.json').exists(), 'Preserved pre-plan CPU failure differs')
    for name,digest in failure['source_sha256'].items():bind(PRIOR_FAILURE.parent/'source'/name.replace('/','_'),bindings,digest)
    need(read(PRIOR_FAILURE.parent/'inherited_sources.json')==dict(source_sha256=release['inherited_source_sha256']), 'Original failed-run ancestry differs')
    return dict(file=str(PRIOR_FAILURE),sha256=PRIOR_FAILURE_SHA,source_release_file=str(PRIOR_RELEASE),source_release_sha256=PRIOR_RELEASE_SHA,
        original_cpu_job='443567',failed_before_plan_and_fixtures=True,no_original_gpu_release=True)


def prior_allocation_check(out,bindings):
    ...
    raw=subprocess.run(command,capture_output=True,text=True,check=True).stdout;file=out/'prior_sacct.psv';file.write_text(raw);bind(file,bindings)
    need(not factor_audit.allocation_rows(raw,PRIOR_GPU_NAMES),'Any original native GPU attempt forbids this CPU-only repair route')
    cpu=factor_audit.allocation_rows(raw,{'identity_join_factor_native_check'})
    need(len(cpu)==1 and cpu[0]['job_id']=='443567' and cpu[0]['partition']=='cpu' and cpu[0]['state']=='FAILED'
         and cpu[0]['exit_code']=='1:0' and cpu[0]['gpus']==0,'Original failed CPU allocation differs')
    return dict(passed=True,file=str(file),sha256=sha(file),command=command,original_cpu=cpu[0],original_gpu_allocations=[],original_gpu_seconds=0)
...
def check(args,out,frozen):
    ...
    torch.set_num_threads(4);bindings={};prior=verify_prior_failure(bindings);prior_accounting=prior_allocation_check(out,bindings);path=args.cached_report.resolve();need(path==CACHED_REPORT,'Only the fixed registered cached report is eligible');bind(path,bindings,CACHED_REPORT_SHA);proof=read(path)
    ...
    owner,rope,api=software.backend.native_api(processor);need(api==parent['native_identity']['native_api'],'Installed native position/API source differs');layouts={}
    for sample,row in zip(rows,compact):
        ...
        layout=native.audit_layout(lambda **kw:rope(owner,**kw),bundle);prepared[sample['sid']]=dict(item,layout=layout['metadata']);layouts[sample['sid']]=layout
    ...
    from scripts.report_native_identity_join_factor_native_v2 import self_test
    ...
    plan=dict(schema_version=1,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),tests=tests,
        ...
        rows_file=str(out/'rows.json'),prepared_file=str(out/'prepared.json'),layout_file=str(layout_file),layout_sha256=sha(layout_file),profile_cases=cases,checkpoints=checkpoints,prior_cpu_failure=prior,prior_allocation_check=prior_accounting,
...
def verify_plan(path,ancestors=False):
    ...
    need(plan['prior_cpu_failure']==verify_prior_failure({}),'Preserved original failure binding differs')
    bind(plan['prior_allocation_check']['file'],{},plan['prior_allocation_check']['sha256'])
    need(plan['prior_allocation_check']['passed'] is True and plan['prior_allocation_check']['original_gpu_allocations']==[],'Original GPU-zero proof differs')
...
def single_attempt_guard(out,phase,arm):
    ...
    need(not factor_audit.allocation_rows(raw,PRIOR_GPU_NAMES),'Original native GPU attempt after repair preparation')
