"""Fixed-budget orientation-coverage training of the unchanged factor pair.

The original unfitted weights, conditioning, CE objective and6000-update schedule
are retained. One orientation is chosen for each unchanged base-pair visit.
"""
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_factor_orientation_training'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_factor_orientation_training')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_factor_orientation_training')
PROTOCOL='identity_join_factor_orientation_fixed_budget'
RUN_JOBS={arm:'identity_join_factor_orientation_training_'+arm for arm in ARMS}
PARENT=previous.PARENT;PARENT_SHA=previous.PARENT_SHA
PROPOSAL='docs/paper/NATIVE_AGGREGATION_FACTOR_ORIENTATION_TRAINING_PROPOSAL.md'
...
POLICY.update(protocol=PROTOCOL,training_contexts=216,pairs=108,length_pairs=108,base_pairs=54,base_families=18,
    schedule='parent48000 base-pair visits; alternate original/flipped for each base pair from original',
    horizon_and_annealing_calibration=False,orientation_coverage_only=True,original_conditioning_unchanged=True,
    training_head_rows=213330,final_head_rows=216,total_head_rows={arm:213546 for arm in ARMS},
    core_calls={arm:6014 for arm in ARMS},norm_calls={arm:6014 for arm in ARMS},head_calls={arm:6014 for arm in ARMS},
    cpu_final_head_calls=14,cpu_final_head_rows=216,evaluation_steps=[6000],earlier_endpoints_descriptive_only=False,
    final_checkpoint_only=True,thresholds=dict(pooled=206,per_orientation=103,complete_families=16),
    first_token_fit=dict(correct=206,contexts=216,per_orientation_correct=103,per_orientation_contexts=108,complete_families=16,families=18),
    fresh_confirmation_seed=91726342,confirmation_manifest_required_before_fitting=True,
    gpu_seconds_cap=180,campaign_gpu_seconds_cap=360,cpu_check_seconds=300,cpu_report_seconds=300,
    maximum_concurrent_gpus=2)
OWN=('scripts/diagnose_native_identity_join_factor_orientation_training.py',
     'scripts/report_native_identity_join_factor_orientation_training.py',
     'slurm/native_identity_join_factor_orientation_training_check.sbatch',
     'slurm/native_identity_join_factor_orientation_training_run.sbatch',
     'slurm/native_identity_join_factor_orientation_training_report.sbatch',PROPOSAL)


def inherited_sources():
    result={}
    for group in (previous.sources(),previous.inherited_sources(),bridge.sources(),bridge.inherited_sources(),confirmation.source_hashes()):
...
def self_test(torch):
    ...
    return dict(passed=True,criterion_groups=4,call_inventory=True,unchanged_factor=original.self_test(torch))
...
def check(args,out,frozen):
    ...
    bank=bridge.verify_cache(args.cache_summary);manifest=confirmation.verify_stage(args.confirmation_stage)
    bind(args.cache_summary,bindings);bind(args.confirmation_stage,bindings)
    cacheproof=read(args.cache_summary);confirmationproof=read(args.confirmation_stage)
    need(manifest['seed']==91726342 and manifest['manifest_required_before_fitting'] is True
         and manifest['new_model_trainability_established'] is False,'Prospectively frozen new confirmation required')
    need(bank['native_identity']==parent['native_identity'] and bank['native_identity_sha256']==parent['native_identity_sha256'],
         'Training feature/native identity changed')
    need(bank['plan']['training_stage']==manifest['training_stage'], 'Training/confirmation stage identity differs')
    reference_rows=read(parent['rows_file']);question_indices={r['question']:r['question_index'] for r in reference_rows}
    need(len(question_indices)==6 and all(question_indices[r['question']]==r['question_index'] for r in reference_rows),'Original question metadata differs')
    rows=[dict(r,first_token_id=r['target_ids'][0],question_index=question_indices[r['question']]) for r in bank['rows']];scenes=bank['scenes'];pairs=bank['pairs'];order=bank['order']
    need(len(rows)==len(scenes)==216 and len(pairs)==108 and len(order)==48000,'Complete orientation training bank required')
    need({r['sid'] for r in rows}==set(scenes) and all(scenes[r['sid']]['target_ids']==r['target_ids'] for r in rows),'Target ownership changed')
    counts=[sum(len(scenes[sid]['target_ids']) for pair in order[i:i+8] for sid in pair['sids']) for i in range(0,48000,8)]
    need(len(counts)==6000 and sum(counts)==213330 and max(counts)==44,'Exact unchanged training head-row budget required')
    for name,value in (('rows.json',rows),('scenes.json',scenes),('pairs.json',pairs),('order.json',order),
        ('head_rows.json',dict(by_update=counts,total=sum(counts),maximum=max(counts),endpoint_rows=216,all_head_rows=sum(counts)+216))):save(out/name,value)
    ...
    need({k:tensor_info(v) for k,v in stats.items() if isinstance(v,torch.Tensor)}==parent['stats_tensors']
         ...
         'Original108-scene conditioning statistics must remain unchanged')
    packet=torch.load(bank['features_file'],map_location='cpu',weights_only=True)
    need(tensor_info(packet['states'])==bank['feature_tensor'] and packet['feature_ids']==bank['feature_ids']
         and packet['states'].dtype==torch.float16 and packet['states'].shape==(len(packet['feature_ids']),3584)
         and torch.isfinite(packet['states']).all(),'Complete finite nativeFP16 bank required');del packet,stats
    ...
    need(sha(initial_file)==parent['initial_sha256'],'Original unfitted full checkpoint required')
    ...
    for arm in ARMS:
        ...
        need(base.v7.state_info(core)==parent['initial_state'] and mask==parent['trainability_mask']
             and core.interaction==arm and core.factor_derangement is False,'Original unfitted state/mask differs')
    tests=self_test(torch)
    from scripts.report_native_identity_join_factor_orientation_training import self_test as reporter_self_test
    ...
    runtime={}
    for key in ('stats_file','stats_inventory_file','native_model_file'):bind(parent[key],runtime,parent['runtime_bindings'][parent[key]])
    bind(initial_file,runtime);bind(bank['features_file'],runtime,bank['features_sha256'])
    for name in ('rows.json','scenes.json','pairs.json','order.json','head_rows.json'):bind(out/name,runtime)
    bind(args.cache_summary,runtime);bind(args.confirmation_stage,runtime);bind(confirmationproof['manifest_file'],runtime,confirmationproof['manifest_sha256'])
    plan=dict(parent,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),tests=tests,
        input_bindings=bindings,runtime_bindings=runtime,factor_plan=dict(file=str(PARENT),sha256=PARENT_SHA),
        cache_report=dict(file=str(args.cache_summary.resolve()),sha256=sha(args.cache_summary)),
        confirmation_report=dict(file=str(args.confirmation_stage.resolve()),sha256=sha(args.confirmation_stage),
            manifest_file=confirmationproof['manifest_file'],manifest_sha256=confirmationproof['manifest_sha256']),
        training_stage=manifest['training_stage'],features_file=bank['features_file'],features_sha256=bank['features_sha256'],feature_tensor=bank['feature_tensor'],
        rows_file=str(out/'rows.json'),scenes_file=str(out/'scenes.json'),pairs_file=str(out/'pairs.json'),order_file=str(out/'order.json'),order_object_sha256=object_sha(order),
        head_rows_by_update=counts,head_rows_file=str(out/'head_rows.json'),training_head_rows=sum(counts),total_head_rows=sum(counts)+216,
        ...
        decision_step=6000,evaluation_steps=[6000],no_new_statistics=True,orientation_coverage_only=True)
    ...
    return dict(passed=True,completed=True,phase='check',protocol=PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        training_head_rows=sum(counts),maximum_training_head_rows=max(counts),evaluation_head_rows=216,total_head_rows=sum(counts)+216,no_pretrained_head_forward=True,
        confirmation_report=plan['confirmation_report'],cache_report=plan['cache_report'])
...
def run(args,out,frozen,started):
    ...
    try:
        ...
        need(time.perf_counter()-started<=180,'Single orientation-training job exceeded180 seconds');return result
    finally:
...
def main():
    ...
    parser.add_argument('--arm',choices=ARMS);parser.add_argument('--cache-summary',type=Path);parser.add_argument('--confirmation-stage',type=Path)
    args=parser.parse_args();native.require_slurm(gpu=args.run)
    if args.check:need(os.environ.get('SLURM_JOB_PARTITION')=='cpu' and not os.environ.get('SLURM_JOB_GPUS') and args.cache_summary is not None and args.confirmation_stage is not None,'CPU-only cache/confirmation check')
    ...
    try:
        ...
        need(sources()==frozen and inherited_sources()==read(out/'inherited_sources.json')['source_sha256'],'Source changed during orientation-training execution')
