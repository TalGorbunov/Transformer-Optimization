...
CPU preparation binds already audited checkpoints, fresh orientation image bundles and
...
from scripts import diagnose_native_identity_join_factor_native_v2 as original
from scripts import diagnose_native_identity_join_factor_orientation_training as cached
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_factor_orientation_native'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_factor_orientation_native')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_factor_orientation_native')
PROTOCOL='identity_join_factor_orientation_native_trainability';ARMS=('product','additive')
JOBS={phase:{arm:f'identity_join_factor_orientation_native_{phase}_{arm}' for arm in ARMS} for phase in ('profile','main')}
PROPOSAL='docs/paper/NATIVE_AGGREGATION_FACTOR_ORIENTATION_NATIVE_PROPOSAL.md'
PROPOSAL_SHA='8ea55afc7d42f1b25c229c63b1e83e42744309236a37facfb54c5666049b5cb8'
POLICY=dict(protocol=PROTOCOL,arms=list(ARMS),checkpoint_step=6000,maximum_new_tokens=4,
    profile_trajectories=8,profile_model_cap=32,profile_vision_calls=8,main_trajectories=216,main_model_cap=864,main_vision_calls=216,
    profile_order='first Sandra base pair; original,flipped then N8,N16 then zero,fitted',
    zero='fixed fitted state with only up.weight zeroed',paired_only=True,no_fitting=True,training_only=True,
    profile_seconds=120,main_seconds=900,campaign_seconds=2040,max_concurrent_gpus=2,
    cpu_check_seconds=300,cpu_release_seconds=300,cpu_report_seconds=300,
    ...
    whole_answer_threshold=206,per_orientation_threshold=103,complete_family_threshold=16,families=18,contexts=216,
    no_output_filter=True,no_answer_prefix=True,no_fresh_or_dev=True)
OWN=('scripts/diagnose_native_identity_join_factor_orientation_native.py','scripts/report_native_identity_join_factor_orientation_native.py',
     'slurm/native_identity_join_factor_orientation_native_check.sbatch','slurm/native_identity_join_factor_orientation_native_profile.sbatch',
     'slurm/native_identity_join_factor_orientation_native_main.sbatch','slurm/native_identity_join_factor_orientation_native_release.sbatch',
     'slurm/native_identity_join_factor_orientation_native_report.sbatch',PROPOSAL)


def inherited_sources():
    values={}
    for group in (original.sources(),original.inherited_sources(),cached.sources(),cached.inherited_sources()):
        for name,digest in group.items():
            need(name not in values or values[name]==digest,'Inherited source conflict: '+name);values[name]=digest
...
def criterion(rows):
    need(len(rows)==len({r['sid'] for r in rows})==216,'Native training denominator differs')
    groups={};orientations=('original','flipped')
    for row in rows:groups.setdefault(row['base_contrast_id'],[]).append(row)
    need(len(groups)==18 and all({(r['variant'],r['n_frames'],r['orientation_version']) for r in values}
         =={(i,n,o) for i in range(3) for n in (8,16) for o in orientations} and len(values)==12 for values in groups.values()),
         'Complete native12-context family inventory differs')
    by_orientation={o:sum(r['exact'] for r in rows if r['orientation_version']==o) for o in orientations}
    need(all(sum(r['orientation_version']==o for r in rows)==108 for o in orientations),'Native orientation denominator differs')
    exact=sum(by_orientation.values());complete=sum(all(r['exact'] for r in values) for values in groups.values())
    return dict(passed=exact>=206 and min(by_orientation.values())>=103 and complete>=16,whole_correct=exact,
        first_correct=sum(r['first_token_correct'] for r in rows),orientation_correct=by_orientation,
        contexts=216,complete_families=complete,families=18,thresholds=dict(pooled=206,per_orientation=103,complete_families=16),
        natural_whole_answer_and_eos=True,training_only=True)


def profile_cases(rows):
    first=next(r for r in rows if r['gold']=='Sandra');base=first['base_pair_id']
    pair=[r for o in ('original','flipped') for n in (8,16) for r in rows
          if r['base_pair_id']==base and r['orientation_version']==o and r['n_frames']==n]
    need(len(pair)==4 and len({r['sid'] for r in pair})==4 and all(r['gold']=='Sandra' for r in pair),
         'First frozen-row Sandra base-pair orientation/length coverage differs')
    return [dict(sid=r['sid'],n_frames=r['n_frames'],state=state) for r in pair for state in ('zero','fitted')]


def projection(setups,timings):
    need(len(setups)==2 and len(timings)==16 and all(math.isfinite(x) and x>0 for x in setups+timings),'Incomplete positive timing inventory')
    setup=max(setups);observed=max(timings);bound=max(POLICY['historical_four_token_floor'],observed)
    seconds=setup+1.25*216*bound+60.
    return dict(passed=seconds<=900,pooled_setup_seconds=setup,maximum_observed_four_token_seconds=observed,
        historical_floor_seconds=POLICY['historical_four_token_floor'],pooled_scene_seconds=bound,multiplier=1.25,
        contexts=216,reserve_seconds=60.,projected_seconds=seconds,cap_seconds=900.)


def check(args,out,frozen):
    import torch
    from transformers import AutoProcessor
    torch.set_num_threads(4);bindings={};path=args.cached_report.resolve();bind(path,bindings);proof=read(path)
    need(proof['passed'] is proof['completed'] is True and proof['phase']=='report' and proof['protocol']==cached.PROTOCOL
         and proof['primary_endpoint_step']==6000,'Passed independent orientation cached report required')
    bind(proof['analysis_file'],bindings,proof['analysis_sha256']);analysis=read(proof['analysis_file'])
    need(analysis['passed'] is analysis['completed'] is True and analysis['protocol']==cached.PROTOCOL
         and analysis['primary_endpoint_step']==6000 and set(analysis['runs'])==set(ARMS)
         and proof['first_token_screen']==analysis['first_token_screen']
         and any(proof['first_token_screen'][a]['passed'] for a in ARMS),
         'Either fixed cached216 screen must pass; both arms must remain present')
    need(proof['plan_file']==analysis['plan_file'] and proof['plan_sha256']==analysis['plan_sha256'],'Cached report plan differs')
    bind(analysis['plan_file'],bindings,analysis['plan_sha256']);parent=cached.verify_plan(analysis['plan_file'],ancestors=True)
    need(proof['source_sha256']==analysis['source_sha256']==parent['source_sha256']
    ...
    for name,digest in parent['source_sha256'].items():bind(path.parent/'source'/name.replace('/','_'),bindings,digest)
    need(read(path.parent/'inherited_sources.json')==dict(source_sha256=parent['inherited_source_sha256']),'Cached report source archive differs')
    bind(REPO/PROPOSAL,bindings,PROPOSAL_SHA)
    stage_ref=parent['training_stage'];stage_plan=cached.confirmation.check_training_binding(stage_ref)
    for key in ('file','plan_file','independent_audit_file'):
        bind(stage_ref[key],bindings,stage_ref['sha256' if key=='file' else key.replace('_file','_sha256')])
    bind(stage_plan['rows_file'],bindings,stage_plan['files'][stage_plan['rows_file']]);stage_rows=read(stage_plan['rows_file'])
    bind(parent['rows_file'],bindings,parent['runtime_bindings'][parent['rows_file']]);rows=read(parent['rows_file'])
    need(len(rows)==len(stage_rows)==216 and [{k:v for k,v in r.items() if k not in ('first_token_id','question_index')} for r in rows]==stage_rows,
         'Exact published216 training rows and cached target ownership required')
    need(stage_ref['native_identity_sha256']==parent['native_identity_sha256']
         and analysis['training_stage']==stage_ref and analysis['confirmation_report']==parent['confirmation_report'],
         'Training-stage/confirmation provenance differs')
    confirmation_ref=parent['confirmation_report']
    bind(confirmation_ref['file'],bindings,confirmation_ref['sha256']);confirmation_proof=read(confirmation_ref['file'])
    bind(confirmation_ref['manifest_file'],bindings,confirmation_ref['manifest_sha256']);confirmation_manifest=read(confirmation_ref['manifest_file'])
    need(confirmation_proof['passed'] is confirmation_proof['completed'] is True
         and confirmation_proof['manifest_file']==confirmation_ref['manifest_file'] and confirmation_proof['manifest_sha256']==confirmation_ref['manifest_sha256']
         and confirmation_manifest['training_stage']==stage_ref and confirmation_manifest['seed']==91726342
         and confirmation_manifest['manifest_required_before_fitting'] is True,'Frozen pre-fit confirmation identity differs')
    ...
    owner,rope,api=software.backend.native_api(processor);need(api==parent['native_identity']['native_api'],'Installed native position/API source differs')
    data=DATA/out.name;data.mkdir(parents=True,exist_ok=False);(data/'bundles').mkdir();prepared={};layouts={}
    for sample in rows:
        need(sample['split']=='train' and sample['n_frames'] in (8,16) and sample['orientation_version'] in ('original','flipped')
             and sample['first_token_id']==sample['target_ids'][0]
             and processor.tokenizer.encode(sample['gold'],add_special_tokens=False)+[151645]==sample['target_ids']
             and 1<=len(sample['target_ids'])<=4,'Native full-name/EOS target ownership differs')
        bind(Path(sample['path'])/'qa.txt',bindings,sample['qa_sha256'])
        for image in sample['image_files']:bind(image['path'],bindings,image['sha256'])
        bundle=runtime.prepare_scene(processor,model_view(sample));runtime.validate_bundle(bundle);meta=bundle['metadata']
        need(meta['prefix_ids']==[] and meta['sid']==sample['sid'] and meta['question']==sample['question']
             and meta['n_frames']==sample['n_frames'] and meta['processor_parity_checked'] is True
             and meta['image_paths']==[x['path'] for x in sample['image_files']] and meta['image_sha256']==[x['sha256'] for x in sample['image_files']],
             'Fresh prepared four-key image/question bundle differs')
        layout=native.audit_layout(lambda **kw:rope(owner,**kw),bundle);file=data/'bundles'/f"{sample['sid']}.pt";torch.save(bundle,file)
        prepared[sample['sid']]=dict(file=str(file),sha256=sha(file),metadata=meta,layout=layout['metadata']);layouts[sample['sid']]=layout
    cases=profile_cases(rows)
    ckpt=CKPT/out.name;ckpt.mkdir(parents=True,exist_ok=False);checkpoints={}
    bind(parent['initial_file'],bindings,parent['initial_sha256']);initial=torch.load(parent['initial_file'],map_location='cpu',weights_only=True)['branch']
    for arm in ARMS:
        run=analysis['runs'][arm];endpoint=run['endpoint_audits']['6000'];source=Path(endpoint['checkpoint'])
        need(run['primary_endpoint_step']==6000 and run['first_token_fit']==analysis['first_token_screen'][arm]
             and endpoint['endpoint_step']==6000 and endpoint['passed'] is True,'Fixed cached endpoint/report ownership differs')
        bind(source,bindings,endpoint['checkpoint_sha256']);packet=torch.load(source,map_location='cpu',weights_only=True)
        need(packet['step']==6000 and packet['config']['arm']==arm and packet['config']['protocol']==cached.PROTOCOL
             and packet['config']['plan_sha256']==analysis['plan_sha256'],'Fixed cached checkpoint ownership differs')
        ...
        need(all(torch.equal(zero[k],weights[k]) for k in weights if k!='up.weight') and bool((zero['up.weight']==0).all()),'Zero-U copy changed other tensors')
    ...
    conditioning={k:tensor_info(stats[k]) for k in ('global_mean','scale')}
    need(conditioning=={k:parent['stats_tensors'][k] for k in conditioning},'Frozen original global conditioning changed')
    tests=dict(runtime=runtime.self_test(torch),layout_owner_binding=dict(passed=True,bundles=len(layouts),actual_native_API=True,extra_layout_calls=0))
    from scripts.report_native_identity_join_factor_orientation_native import self_test
    tests['reporter']=self_test(torch)
    save(out/'rows.json',rows);save(out/'prepared.json',prepared)
    layout_file=data/'layouts.pt';torch.save(layouts,layout_file);bind(layout_file,bindings)
    runtime_bindings={}
    ...
    plan=dict(schema_version=1,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),tests=tests,
        ...
        training_stage=stage_ref,confirmation_report=confirmation_ref,
        original_initial_file=parent['initial_file'],original_initial_sha256=parent['initial_sha256'],
        ...
        native_software_plan=native_plan,input_bindings=bindings,runtime_bindings=runtime_bindings,no_new_preparation=False,
        new_four_key_preparation=True,training_contexts=216,no_confirmation_model_inputs=True)
    ...
    return dict(passed=True,completed=True,phase='check',protocol=PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        plan_file=str(out/'plan.json'),plan_sha256=sha(out/'plan.json'),tests=tests,training_contexts=216,profile_cases_per_arm=8,no_model_loaded=True)
...
def verify_release(path,plan_path,plan):
    ...
    need(summary['passed'] is summary['completed'] is True and summary['phase']=='release' and summary['release_file']==str(path)
         ...
         and value['projection']['passed'] is True and value['projection']['projected_seconds']<=900
         and value['main_reservation_seconds']==1800 and set(value['profiles'])==set(ARMS),'Passed measured two-arm release required')
...
def run(args,out,frozen,started):
    ...
    config=dict(protocol=PROTOCOL,policy=POLICY,phase=phase,arm=args.arm,run_id=out.name,slurm_job_id=os.environ['SLURM_JOB_ID'],
        ...
        cached_report=plan['cached_report'],cached_plan=plan['cached_plan'],training_stage=plan['training_stage'],confirmation_report=plan['confirmation_report'],native_identity=plan['native_identity'],native_identity_sha256=plan['native_identity_sha256'],
    ...
    for number,case in enumerate(cases):
        ...
        need(result['metadata']['layout']==item['layout'],'Actual native positions differ from CPU-bound staged layout')
    ...
    expected_scenes=8 if args.profile else 216;cap=32 if args.profile else 864
...
def main():
    ...
    parser.add_argument('--cached-report',type=Path);parser.add_argument('--plan',type=Path);parser.add_argument('--arm',choices=ARMS);parser.add_argument('--main-release',type=Path)
    args=parser.parse_args();native.require_slurm(gpu=not args.check)
    if args.check:need(args.cached_report is not None and os.environ.get('SLURM_JOB_PARTITION')=='cpu' and not os.environ.get('SLURM_JOB_GPUS'),'CPU-only preparation and completed cached report required')
