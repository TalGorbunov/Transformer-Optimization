...
from scripts import report_native_identity_join_orientation_joint_code as previous
from scripts import report_native_identity_join_factor_orientation_training as orientation
OWN=('scripts/report_native_identity_join_orientation_paired_code.py',)
...
def driver():
    from scripts import diagnose_native_identity_join_orientation_paired_code as module
    return module
...
def audit_inputs(torch,plan):
    p=driver();ref=plan['joint_code_orientation_plan'];bind(ref['file'],ref['sha256'])
    parent=previous.driver().verify_plan(ref['file'],ancestors=True)
    rows,scenes,old_order,states,index,by_sid,initial=previous.audit_inputs(torch,parent)
    for key in ('rows_file','scenes_file','pairs_file','training_samples_file','code_inventory_file'):
        need(read(plan[key])==read(parent[key]),'Unchanged original216 input differs: '+key)
    for key in ('codes_sha256','code_tensor','features_sha256','feature_tensor','global_feature_ids','initial_sha256','initial_state',
                ...
                'capacity_witness','training_stage','trainability_mask'):
        need(plan[key]==parent[key],'Original code/readout/native identity differs: '+key)
    ...
    need(plan['paired_minibatch_only'] is True and plan['unchanged_presentation_multiset'] is True,'Only minibatch pairing may change')
    order=audit_paired_order(plan,parent,p.paired,parent['orientation_plan'])
...
def audit_run(torch,out,directory,plan_path,plan,frozen,rows,scenes,order,states,index,by_sid,initial,norm,head):
    ...
    inherited=dict(plan_file=plan['joint_code_orientation_plan']['file'],plan_sha256=plan['joint_code_orientation_plan']['sha256'],source_sha256=plan['inherited_source_sha256'])
    ...
    for key in ('native_identity','native_identity_sha256','native_module_identity','native_model_file','native_model_sha256',
                ...
                'parent_initial_file','parent_initial_sha256','joint_code_plan','joint_code_orientation_plan','paired_order_stage','negative_evidence','orientation_plan','capacity_witness','training_stage','factor_plan','uniform_plan','architecture_stop_report','decision_step','evaluation_steps'):
...
def report(args,out,frozen):
    ...
    inherited=dict(plan_file=plan['joint_code_orientation_plan']['file'],plan_sha256=plan['joint_code_orientation_plan']['sha256'],source_sha256=plan['inherited_source_sha256'])
    need(read(out/'inherited_sources.json')==inherited and plan['joint_code_orientation_plan']==dict(file=str(p.PARENT),sha256=p.PARENT_SHA)
    ...
    result=dict(passed=run['passed'],completed=True,all_numerical_audits_collected=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        capacity_witness=gate,joint_code_orientation_plan=plan['joint_code_orientation_plan'],paired_order_stage=plan['paired_order_stage'],negative_evidence=plan['negative_evidence'],runs=runs,resources=budget,primary_endpoint_step=6000,first_token_screen={p.ARM:run['first_token_fit']},
        ...
        optimizer_and_exposure_unchanged=True,paired_minibatch_only=True,oracle_local_semantics=True,no_answer_or_intersection_code=True,
    ...
    lines=['# Paired-minibatch local joint-code optimization diagnostic','',
        ...
        'Only the ordering of the same48000 length-pair presentations changes: original/flipped counterparts share minibatches. The6000-update optimizer, objective and final-only decision remain fixed.','',
