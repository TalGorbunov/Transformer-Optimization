"""Held single product ablation: the explicit query enters only the readout.

Reuse the original unfitted factor checkpoint and all frozen data/statistics.
The final cyclic pairing diagnostic changes no fitted parameters or screen.
"""
...
from scripts import analyze_native_identity_join_factor_query_geometry as geometry_proof
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_readout_query'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_readout_query')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_readout_query')
PROTOCOL='identity_join_readout_only_query'
ARMS=('product',)
RUN_JOBS={arm:'identity_join_readout_query_'+arm for arm in ARMS}
PEOPLE=oracle.PEOPLE
PROPOSAL='docs/paper/NATIVE_AGGREGATION_READOUT_QUERY_PROPOSAL.md'
PROPOSAL_SHA='04f45b2748d9150e9d94594bb2ceb6510b49d63f6191b17b4c56c43ca774ef02'
PARENT=original.OUT/'check_443373/plan.json'
PARENT_SHA='aa9a44067f86ce6340d56419b5efe9411ccd84cb52115f0b31d10745b4e68a07'
CONDITIONAL=original.OUT/'report_443383/summary.json'
CONDITIONAL_SHA='de38a1498115bc835955cf264a609ead1a095bacb0c3faa99eba10a1ae962238'
CONDITIONAL_ANALYSIS_SHA='326db6392f648a517e169b206ca2c46b25b28f1d7f2faf3da31a314afcb74f94'
GEOMETRY=original.OUT/'query_geometry_443401/summary.json'
GEOMETRY_SHA='7f2b8b6592c840c459f1edb97a97b257a1cd96339e03334da74c03518c1a7147'
DIAGNOSTIC_STEPS=(1,2,32,128,300,600)
POLICY=dict(original.POLICY,protocol=PROTOCOL,arms=list(ARMS),query_placement='readout_only',
    fresh_shared_initialization=False,reused_unfitted_initialization=True,shared_query_both_factors=False,query_in_readout=True,
    factor_rule=dict(product='tanh(A*x+bias_a)*tanh(B*x+bias_b)'),
    norm_calls=dict(product=614),head_calls=dict(product=614),core_calls=dict(product=614),total_head_rows=dict(product=21550),
    gpu_seconds_cap=90,campaign_gpu_seconds_cap=90,maximum_gpus=1,final_query_placement_ablation=True)
OWN=('gnnformer/parallel_local_readout_query_binding.py','scripts/diagnose_native_identity_join_readout_query.py',
     'scripts/report_native_identity_join_readout_query.py','slurm/native_identity_join_readout_query_check.sbatch',
     'slurm/native_identity_join_readout_query_run.sbatch','slurm/native_identity_join_readout_query_report.sbatch',PROPOSAL)


def ancestor_sources():return {**original.sources(),**{n:sha(REPO/n) for n in geometry_proof.OWN}}


def sources():return {**ancestor_sources(),**{n:sha(REPO/n) for n in OWN}}
...
def conditional_gate(bindings):
    ...
    need(proof['passed'] is True and proof['completed'] is True and proof['phase']=='report'
         ...
         and analysis['passed'] is True and analysis['completed'] is True and analysis['source_sha256']==original.sources()
         and analysis['plan_file']==str(PARENT) and analysis['plan_sha256']==PARENT_SHA
         and proof['first_token_screen']==analysis['first_token_screen'],'Independent completed factor report required')
    need(set(proof['first_token_screen'])=={'product','additive'}
         and all(c['passed'] is False and c['contexts']==108 and c['families']==18 for c in proof['first_token_screen'].values()),
         'Both fixed factor screens must fail before this final query-placement ablation')
    for n,h in proof['source_sha256'].items():need(sha(CONDITIONAL.parent/'source'/n.replace('/','_'))==h,'Factor report source snapshot changed')
    bind(GEOMETRY,bindings,GEOMETRY_SHA);geometry=read(GEOMETRY)
    need(geometry['passed'] is True and geometry['completed'] is True and geometry['tests']['passed'] is True
         and geometry['captures']==26 and geometry['paired_final_scenes_per_arm']==108
         and geometry['source_sha256']==ancestor_sources() and geometry['report_file']==str(CONDITIONAL)
         and geometry['report_sha256']==CONDITIONAL_SHA and geometry['analysis_sha256']==CONDITIONAL_ANALYSIS_SHA
         and all(geometry[k]==0 for k in ('vlm_calls','core_calls','norm_calls','head_calls','optimizer_calls')),
         'Independent all-capture query geometry required; no outcome-dependent numeric threshold')
    for n,h in geometry['source_sha256'].items():need(sha(GEOMETRY.parent/'source'/n.replace('/','_'))==h,'Geometry source snapshot changed')
    for file,h in geometry['files'].items():bind(file,bindings,h)
    bind(REPO/PROPOSAL,bindings,PROPOSAL_SHA)
    return parent,dict(summary_file=str(CONDITIONAL),summary_sha256=CONDITIONAL_SHA,
        analysis_file=proof['analysis_file'],analysis_sha256=CONDITIONAL_ANALYSIS_SHA,first_token_screen=proof['first_token_screen'])
...
def make_core(torch,arm):
    ...
    return ParallelLocalReadoutQueryBinding(interaction=arm)
...
def query_means(torch,stats,scenes,sids,layout=None,*,arm):
    need(arm in ARMS,'Unknown factor-binding arm')
    return original.query_means(torch,stats,scenes,sids,layout,arm='product')


def self_test(torch):
    from gnnformer.parallel_local_readout_query_binding import self_test as core_self_test
    result=core_self_test(torch);need(result['passed'] is True,'Readout-only query core fixtures failed')
    return dict(passed=True,core=result,unchanged_conditioning_and_factor_scope=original.self_test(torch))
...
def check(out,frozen):
    ...
    need(sha(initial_file)==parent['initial_sha256'],'Only exact unfitted parent checkpoint may initialize this ablation')
    packet=torch.load(initial_file,map_location='cpu',weights_only=True)
    need(set(packet)=={'seed','branch'} and packet['seed']==24,'Unfitted initial checkpoint metadata differs')
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(24);core=make_core(torch,'product')
    need(base.v7.state_info(core)==parent['initial_state'],'Subclass factory must reproduce the parent unfitted initialization')
    core.load_state_dict(packet['branch']);mask=freeze_selector(torch,core);initial_state=base.v7.state_info(core)
    need(initial_state==parent['initial_state'] and mask==parent['trainability_mask'],'Unfitted initial state/mask differ')
    tests=self_test(torch)
    from scripts.report_native_identity_join_readout_query import self_test as reporter_self_test
    ...
    plan=dict(schema_version=1,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,tests=tests,trainability_mask=mask,
        ...
        factor_plan=dict(file=str(PARENT),sha256=PARENT_SHA),factor_report=report,query_geometry=dict(file=str(GEOMETRY),sha256=GEOMETRY_SHA),
        conditioning_plan=parent['conditioning_plan'],conditioning_report=parent['conditioning_report'],query_placement='readout_only',
        ...
        fresh_shared_initialization=False,reused_unfitted_initialization=True,no_pretrained_backbone_loaded=True,no_pretrained_head_forward=True)
    ...
    return dict(passed=True,completed=True,phase='check',protocol=PROTOCOL,source_sha256=frozen,
        plan_file=str(out/'plan.json'),plan_sha256=sha(out/'plan.json'),tests=tests,factor_report=report,query_geometry=dict(file=str(GEOMETRY),sha256=GEOMETRY_SHA),
...
def verify_plan(path,ancestors=False):
    ...
    need(plan['protocol']==PROTOCOL and plan['policy']==POLICY and plan['source_sha256']==sources()
         and sha(path)==path.with_suffix('.sha256').read_text().strip(),'Readout-query plan/source changed')
    ...
    if ancestors:
        for file,h in plan['input_bindings'].items():need(sha(file)==h,'Readout-query ancestor changed')
...
def single_run_guard(out,arm):
    ...
    for line in raw.splitlines():
        ...
        if fields[1]==RUN_JOBS[arm] and fields[2]=='gpu':need(fields[0]==os.environ['SLURM_JOB_ID'],'Single readout-query GPU attempt per arm only')
...
def run(args,out,frozen,started):
    ...
    config=dict(protocol=PROTOCOL,run_id=out.name,slurm_job_id=os.environ['SLURM_JOB_ID'],arm=args.arm,seed=24,policy=POLICY,
        ...
        conditioning_plan=plan['conditioning_plan'],conditioning_report=plan['conditioning_report'],fresh_shared_initialization=False,
        factor_plan=plan['factor_plan'],factor_report=plan['factor_report'],query_geometry=plan['query_geometry'],
        reused_unfitted_initialization=True,query_placement='readout_only',
    ...
    try:
        ...
        endpoint=dict(passed=True,query_placement='readout_only',interaction=core.interaction,pairing_restored=True,factor_derangement=core.factor_derangement,before_evaluation=final,after_evaluation=base.v7.state_info(core),parameter_sha256=object_sha(final),
        ...
        need(time.perf_counter()-started<=90,'Single head-only readout-query job exceeded90 seconds')
        return result
    finally:
...
def main():
    ...
    try:
        ...
        need(sources()==frozen,'Source changed during readout-query execution')
