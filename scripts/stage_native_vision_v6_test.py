"""Stage fresh V6 evaluation only; run generation/rendering/audits on Slurm CPU.

V4 training, development, profiles and schedules remain byte-for-byte unchanged.
...
DESTINATION = DATA_BASE / 'v6_fresh'
OUTPUT_ROOT = REPO / 'outputs/native_aggregation_vlm/v6/data_staging'
SEED = 20260919
SOURCE_FILES = ('scripts/stage_native_vision_v6_test.py',
    'slurm/native_aggregation_vision_v6_test_stage.sbatch',
...
def load_inventory():
    ...
    inventory = dict(schema_version=1, entries=entries, excluded_content_sha256=ordered,
        ...
        scope='All16 available canonical V1/V2/V3-binding/V4/reasoning/recoverability manifests found in the staging inventory',
    ...
    for purpose, (lengths, golds, repeats, axis) in SPEC.items():
        ...
        for gold in golds:
            for replicate in range(repeats):
                label = f'v6_{purpose}_K{gold}_{replicate:03d}'
...
def main():
    ...
    plan = dict(schema_version=1, purpose='v6_fresh_evaluation_only', slurm_job_id=job,
        ...
        training_dev_and_schedule='Literal V4 files remain unchanged; runner uses separate fresh-test overrides',
        ...
        for purpose, rows in groups.items():
            ...
            for n in SPEC[purpose][0]:
                ...
                for record, sample in zip(records, selected):
                    record.update(question=sample['question'], origin='generated_v6_fresh', fresh_test_seed=SEED)
            ...
            manifests[purpose] = dict(schema_version=1, purpose=purpose, dataset_root=str(DESTINATION),
                ...
                main_count_content_disjoint=True, evaluation_status='Fresh complete context/question content, fixed before V6 outcomes',
    ...
    semantic = audit_published(manifests, excluded)
           ...
           'Protected V4/prior source changed during staging')
    ...
    (DESTINATION / 'INDEX.md').write_text('# V6 fresh evaluation data\n\n[Main](main_manifest.json) · [Unseen counts](count_manifest.json) · [Audit](stage_audit.json) · [Exclusions](exclusion_inventory.json). V4 training/dev data remain unchanged.\n')
    ...
    (output / 'REPORT.md').write_text('# V6 fresh evaluation staging\n\n'
        ...
        '280 paired-extension audits passed. V4 training/development/schedules unchanged. '
    ...
    if not index.exists():
        index.write_text('# V6 fresh evaluation staging\n\n')
