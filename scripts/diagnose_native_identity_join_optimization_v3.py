...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_optimization_v3'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_optimization_v3')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_optimization_v3')
...
POLICY=dict(protocol=PROTOCOL,arms=list(ARMS),seed=24,steps=600,profile_steps=32,pairs=54,training_contexts=108,
    ...
    campaign_gpu_seconds=5500,maximum_project_gpus=4,final_checkpoint_only=True,dev_evaluated_once=True,
...
OWN=('scripts/diagnose_native_identity_join_optimization_v3.py','scripts/report_native_identity_join_optimization_v3.py',
     ...
     'slurm/native_identity_join_optimization_v3_check.sbatch','slurm/native_identity_join_optimization_v3_profile.sbatch',
     'slurm/native_identity_join_optimization_v3_main.sbatch','slurm/native_identity_join_optimization_v3_report.sbatch')
...
def parse_resources(raw):
    ...
    need(now==0 and peak<=4 and total<=5500,'Pilot cumulative budget/concurrency exceeded')
...
def verify_release(path,plan_path,plan):
    ...
    need(release['protocol']=='identity_join_optimization_main_release' and release['passed'] is True
         ...
         and release['campaign_gpu_seconds_cap']==5500 and set(release['profiles'])==set(ARMS)
...
def main():
    ...
    try:
        if args.check:result=check(out,frozen)
            ...
            from scripts import report_native_identity_join_optimization_v3 as reporter
