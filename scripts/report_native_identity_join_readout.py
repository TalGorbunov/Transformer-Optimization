"""Independent CPU audit of the fixed oracle code diagnostic; no efficacy claim."""
...
def report(args,out,frozen):
    ...
    ids=logits.argmax(-1);targets=torch.tensor([r['first_token_id'] for r in rows]);nll=torch.nn.functional.cross_entropy(logits.float(),targets,reduction='none')
    ...
    return dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,
        ...
        resources=budget,no_generalization_or_deployable_method=True)
