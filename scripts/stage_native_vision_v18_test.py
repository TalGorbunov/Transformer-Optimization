"""Proposed V18 fresh test preparation; exact CPU dry plan before rendering.
...
and exclude30 prior manifests. Fresh means complete context/question freshness,
...
from scripts import stage_native_vision_v14_test as old_fresh
...
DESTINATION=DATA_BASE/'v18_fresh'
OUT=REPO/'outputs/native_aggregation_vlm/v18/data_staging'
SEED=20261118
PROTOCOL='v18_fresh_test_preparation_only'
SPEC=dict(unique_samples=272,unique_content=272,families=136,image_references=13056,
          paired_extensions=136,test_N32=136,test_N64=136,prior_manifests=30)
OWN=tuple(dict.fromkeys(('scripts/stage_native_vision_v18_test.py','slurm/native_vision_v18_test_stage.sbatch',*old_fresh.OWN)))
...
def snapshot(out):
    ...
    (out/'INDEX.md').write_text('# Proposed V18 fresh test preparation\n\n[Plan](plan.json) · [Summary](summary.json) · [Sources](source_hashes.json) · [Report](REPORT.md).\n')
    index=OUT/'INDEX.md'
    if not index.exists():index.write_text('# V18 fresh test data preparation\n\n')
...
def prior_inputs():
    ...
    need(len(inventory['entries'])==len(manifests)==28,'Require exact V14 prior28 inventory')
    prior28=dict(inventory['source_manifest_sha256'])
    expected={
        'v14_fresh':'ed900cc26eb99575325583de8ef7d18beeac506c4baedd5a2c3f67ae70adf909',
        'v16_step_wrap':'f28d86b497a85c72d39826eda01c532d3af8bb15db91dac8895c0c1be089b590'}
    for name,h in expected.items():
        root=DATA_BASE/name;path=root/'main_manifest.json'
        need(path.is_file() and not path.is_symlink() and digest(path)==h,'Completed historical manifest changed')
        manifest=read(path);need(manifest['dataset_root']==str(root),'Historical dataset root differs')
        if name=='v14_fresh':
            need(manifest['source_manifest_sha256']==prior28 and manifest['source_sha256']==old_fresh.source_hashes()
                 and manifest['data_seed']==manifest['fresh_test_seed']==20261108,'V14 ancestry/seed differs')
        rows=list(content_rows(manifest['splits']));hashes={r['content_sha256'] for r in rows}
        need(len(rows)==len(hashes)==(272 if name=='v14_fresh' else 34) and str(path) not in manifests,
             'Historical canonical context inventory differs')
        need(all(isinstance(x,str) and len(x)==64 and all(c in '0123456789abcdef' for c in x) for x in hashes),
             'Invalid historical context hash')
        if name=='v16_step_wrap':need(hashes<=excluded,'V16 must reuse already excluded V14 semantics')
        ...
        for key,value in manifest.items():
            if key.endswith('_file') and key[:-5]+'_sha256' in manifest:
                target=Path(value);expected_digest=manifest[key[:-5]+'_sha256']
                need(target.is_file() and not target.is_symlink() and digest(target)==expected_digest,
                     'Historical data proof changed: '+key)
                inventory['protected_source_sha256'][str(target)]=expected_digest
        for source,expected_digest in manifest['source_sha256'].items():
            target=REPO/source;need(digest(target)==expected_digest,'Historical renderer/source changed')
            inventory['protected_source_sha256'][str(target)]=expected_digest
        if name=='v14_fresh':
            target=Path(manifest['stage_audit_file']);audit=read(target)
            need(audit['passed'] and audit['completed'] and audit['manifest_sha256']['main']==h,
                 'Historical V14 staging proof failed')
            inventory['protected_source_sha256'][str(target)]=digest(target)
    need(len(inventory['entries'])==len(manifests)==len(inventory['source_manifest_sha256'])==30,
         'Require exactly30 prior canonical manifests')
    ...
    inventory.update(excluded_content_sha256=ordered,excluded_content_count=len(ordered),
        ...
        scope='Exact V14 prior28 plus completed V14 fresh and V16 counterfactual manifests;30 canonical manifests',
        permitted_historical_reuse='None. All proposed V18 contexts are nonsaturated N32/N64 with K0..16.',
...
def generate(excluded):
    ...
    for k in range(17):
        for replica in range(8):
            label=f'v18_test_K{k}_{replica:03d}';seed=stable_seed(SEED,label)
            family=make_family(random.Random(seed),(32,64),k,label,'test','v18_length',used,seed)
            for row in family:
...
def dry(out,frozen):
    need(not DESTINATION.exists() and not DESTINATION.is_symlink(),'Preserve existing/partial proposed V18 data')
    ...
    plan=dict(schema_version=1,protocol=PROTOCOL,dataset_root=str(DESTINATION),data_seed=SEED,fresh_test_seed=SEED,
        ...
        generator_law=LAW,sampling='Canonical V2 family law conditioned on exclusion against30 prior manifests and all new families',
    ...
    (out/'REPORT.md').write_text('# V18 prospective test dry plan\n\nPASS: 272 distinct contexts,136 N32→N64 families, eight/K0–16. All30 prior manifests and literal V10 training/dev/schedule/pairing are protected. No images rendered or training released.\n')
    return summary


def load_plan(path,frozen):
    path=Path(path).resolve();need(path.is_relative_to(OUT),'Require a canonical V18 dry plan')
...
def stage(args,out,frozen):
    ...
    need(current==inventory and current_excluded==excluded,'Prior30 inventory changed after dry gate')
    ...
    for record,sample in zip(records,rows):record.update(question=sample['question'],origin='generated_v18_fresh',fresh_test_seed=SEED)
    cells={}
    for n in (32,64):
    ...
    (DESTINATION/'INDEX.md').write_text('# V18 fresh test data\n\n[Manifest](main_manifest.json) · [Semantic audit](semantic_audit.json) · [Completed audit](stage_audit.json) · [Dry plan](stage_plan.json) · [Exclusions](exclusion_inventory.json).\n')
    (out/'REPORT.md').write_text('# V18 fresh test staging\n\nPASS:272 contexts,136 N32→N64 families, eight/K0–16. Independent QA, image, renderer-link, exclusion and extension audits passed. All30 prior manifests and literal V10 training/dev/schedule/pairing are unchanged. No model calls or training release.\n')
    return result
