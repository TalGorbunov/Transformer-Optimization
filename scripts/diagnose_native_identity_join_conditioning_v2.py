...
from scripts import diagnose_native_identity_join_conditioning as original
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_conditioning_v2'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_conditioning_v2')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_conditioning_v2')
PROTOCOL='identity_join_fixed_local_conditioning'
ARMS=('global','question')
RUN_JOBS={arm:'identity_join_conditioning_v2_'+arm for arm in ARMS}
...
FAILURE=original.OUT/'check_443321/failure.json'
FAILURE_SHA='e270ccf7df158e2acbe1b63d694f6173f2e68ce7d6b54a6fcc0f888922d8ba95'
OWN=('scripts/diagnose_native_identity_join_conditioning_v2.py','scripts/report_native_identity_join_conditioning_v2.py',
     'slurm/native_identity_join_conditioning_v2_check.sbatch','slurm/native_identity_join_conditioning_v2_run.sbatch',
     'slurm/native_identity_join_conditioning_v2_report.sbatch')
...
def tensor_info(value):
    """Original tensor identity, with byte flattening also defined for 0D values."""
...
def tensor_info_self_test(torch):
    arrays=[torch.arange(6,dtype=dtype).reshape(2,3) for dtype in
            (torch.float16,torch.float32,torch.float64,torch.int64)]
    arrays += [arrays[1].t(),torch.tensor([[True,False],[False,True]])]
    need(all(tensor_info(value)==base.v7.tensor_info(value) for value in arrays),
         'Non-scalar identities differ from the frozen helper')
    scalar_records=[]
    for dtype,code in ((torch.float32,'f'),(torch.float64,'d')):
        for value in (1.25,-.5):
            scalar=torch.tensor(value,dtype=dtype)
            expected=dict(shape=[],dtype=str(dtype),sha256=hashlib.sha256(struct.pack('='+code,value)).hexdigest())
            need(tensor_info(scalar)==expected and tensor_info(scalar.reshape(1))==dict(expected,shape=[1]),
                 'Scalar bytes or original shape metadata differ')
            scalar_records.append(expected)
    return dict(passed=True,non_scalar_cases=len(arrays),scalar_cases=len(scalar_records),
        scalar_identities=scalar_records,original_shape_preserved=True,no_statistic_value_or_shape_change=True)
...
def conditional_gate(bindings):
    bind(FAILURE,bindings,FAILURE_SHA);failed=read(FAILURE)
    need(failed['type']=='RuntimeError' and failed['message']=='self.dim() cannot be 0 to view Double as Byte (different element sizes)'
         and failed['source_sha256']==original.sources() and failed['partial_outputs_retained'] is True
         and not (FAILURE.parent/'plan.json').exists(), 'Preserved pre-fit scalar-hash failure differs')
    for name,digest in failed['source_sha256'].items():
        need(sha(FAILURE.parent/'source'/name.replace('/','_'))==digest,'Original failed-check source snapshot changed')
...
def check(out,frozen):
    ...
    need(tensor_info(packet['states'])==parent['feature_tensor'] and packet['feature_ids']==sorted(packet['feature_ids']),
    ...
    tests=self_test(torch);tests['scalar_tensor_info']=tensor_info_self_test(torch)
    from scripts.report_native_identity_join_conditioning_v2 import self_test as report_self_test
    ...
    plan=dict(schema_version=1,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,tests=tests,trainability_mask=mask,
        ...
        stats_tensors={k:tensor_info(v) for k,v in stats.items() if isinstance(v,torch.Tensor)},
