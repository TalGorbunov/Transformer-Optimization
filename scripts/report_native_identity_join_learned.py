...
def source_hashes():
    from scripts import train_native_identity_join_learned as training
    own=('scripts/report_native_identity_join_learned.py','scripts/analyze_native_identity_join_learned.py',
         'tests/test_native_identity_join_learned_analysis.py','tests/test_native_identity_join_learned_report.py',
         'slurm/native_identity_join_learned_report.sbatch')
...
def audit_profile(torch,directory,plan_path,tokenizer):
    ...
    return dict(directory=str(Path(directory).resolve()),summary_sha256=sha(Path(directory)/'summary.json'),
        ...
        projected_seconds=projected,passed=projected<=2700,formula='setup+1.25*(first4+4532*steady+108*T16+216*T32+216*T64)+120'),config


JOB_CAPS={'v19_selection_software':300,'identity_join_learned_feature_profile':300,
    'identity_join_learned_feature_shard':1200,'identity_join_learned_train_profile':300,'identity_join_learned_main':2700}
...
def check(args,out,frozen):
    tests=[]
    for name in ('tests/test_native_identity_join_learned_report.py','tests/test_native_identity_join_learned_analysis.py'):
...
def release(args,out,frozen):
    ...
    for p in projections.values():p.update(unpooled_projected_seconds=p['projected_seconds'],projected_seconds=pooled_seconds,pooled_timing_inputs=pooled,passed=pooled_seconds<=2700)
    ...
    budget.update(reserved_main_gpu_seconds=16200,projected_total_with_main_reservations=budget['allocated_gpu_seconds']+16200)
    ...
    result=dict(protocol='identity_join_learned_selection_main_release',passed=passed,completed=True,
        ...
        profiles=profiles,projections=projections,campaign_budget=budget,per_main_seconds_cap=2700,campaign_gpu_seconds_cap=24000,
