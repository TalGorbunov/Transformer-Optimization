...
from scripts import diagnose_native_identity_join_joint_lora as original
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_joint_lora_v2'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_joint_lora_v2')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_joint_lora_v2')
PROTOCOL='identity_join_joint_lora_v2'
...
PROPOSAL='docs/paper/NATIVE_AGGREGATION_JOINT_LORA_V2_REPAIR.md'
PROPOSAL_SHA='80266aea8ff32b329fc8a9692b211b4f07c8fde5ea4f94a3dbef8709e88f4d55'
JOB='identity_join_joint_lora_v2_profile'
ORIGINAL_FAILURE=original.OUT/'profile_443880/failure.json'
ORIGINAL_FAILURE_SHA='deda73bd9174b7de5367bbf8a6ed8138967e49844938e0170ea5813d17fd7445'
ORIGINAL_RELEASE=original.OUT/'source_release.json'
ORIGINAL_RELEASE_SHA='19c56ab55c2f0fc9031413bfefa27eab675ab94500ec650dc44609307a26cf7b'
ORIGINAL_PLAN=original.OUT/'check_443875/plan.json'
ORIGINAL_PLAN_SHA='886da0916e78ab72e0aa1428fe058ab1f465db009c70bdb912b19adaadcacf58'
ORIGINAL_CHECK_SHA='88fc29a38f66762ac09d47de8b5a6b7adf1c987c9363e311bcf73a2a39b071cd'
OWN=('scripts/diagnose_native_identity_join_joint_lora_v2.py',PROPOSAL,
    'slurm/native_identity_join_joint_lora_v2_check.sbatch','slurm/native_identity_join_joint_lora_v2_profile.sbatch')
...
POLICY=dict(protocol=PROTOCOL,contexts=216,image_counts=[8,16],epochs=12,scene_presentations=2592,gradient_accumulation=8,updates=324,
    ...
    prior_failed_gpu_seconds=34,cumulative_gpu_seconds_cap=274,
...
def inherited_sources():
    result={}
    for mapping in (original.sources(),original.inherited_sources()):
        for name,digest in mapping.items():
            need(name not in result or result[name]==digest,'Inherited source conflict');result[name]=digest
    need(len(result)==162 and all(sha(REPO/name)==digest for name,digest in result.items()),'Frozen four plus158 ancestor sources changed')
    return result


def verify_original_failure(bindings):
    bind(ORIGINAL_FAILURE,bindings,ORIGINAL_FAILURE_SHA);bind(ORIGINAL_RELEASE,bindings,ORIGINAL_RELEASE_SHA)
    root=ORIGINAL_FAILURE.parent;failure=read(ORIGINAL_FAILURE);release=read(ORIGINAL_RELEASE)
    need(failure['type']=='ValueError' and failure['message']=='Saved PEFT configuration differs'
         and failure['partial_outputs_retained'] is True and failure['source_sha256']==original.sources()
         and release['status']=='frozen_before_execution' and release['source_sha256']==original.sources()
         and release['inherited_source_sha256']==original.inherited_sources() and not (root/'summary.json').exists(),
         'Preserved failed443880 source/metadata trigger changed')
    request_file=root/'request.json';bind(request_file,bindings);request=read(request_file)
    need(request==dict(phase='profile',plan=str(original.OUT/'check_443875/plan.json'),source_sha256=original.sources()),'Original failed profile request differs')
    bind(root/'source_hashes.json',bindings);bind(root/'inherited_sources.json',bindings)
    need(read(root/'source_hashes.json')==original.sources()
         and read(root/'inherited_sources.json')==dict(source_sha256=original.inherited_sources()),'Original failure source descriptors differ')
    for name,digest in original.sources().items():bind(root/'source'/name.replace('/','_'),bindings,digest)
    return dict(file=str(ORIGINAL_FAILURE),sha256=ORIGINAL_FAILURE_SHA,source_release_file=str(ORIGINAL_RELEASE),
        source_release_sha256=ORIGINAL_RELEASE_SHA,request_file=str(request_file),request_sha256=sha(request_file),prior_gpu_seconds=34)
...
def effective_targets(model,config):
    from peft.tuners.tuners_utils import check_target_module_exists
    return {name for name,_ in model.named_modules() if bool(check_target_module_exists(config,name))}


def require_effective_targets(model,config):
    actual=effective_targets(model,config)
    need(actual==set(TARGETS),'Saved PEFT selector must resolve to exactly the original112 language projections')
    return actual


def condensation_fixture(torch):
    from peft import inject_adapter_in_model
    from peft.tuners.lora.layer import LoraLayer
    from peft.utils.constants import MIN_TARGET_MODULES_FOR_OPTIMIZATION
    class DecoderLayer(torch.nn.Module):
        def __init__(self):
            super().__init__();self.self_attn=torch.nn.Module()
            for key in ('q','k','v','o'):setattr(self.self_attn,key+'_proj',torch.nn.Linear(2,2,bias=False))
    class VisionBlock(torch.nn.Module):
        def __init__(self):
            super().__init__();self.attn=torch.nn.Module();self.attn.qkv=torch.nn.Linear(2,6,bias=False);self.attn.proj=torch.nn.Linear(2,2,bias=False)
    class Tiny(torch.nn.Module):
        def __init__(self):
            super().__init__();self.model=torch.nn.Module();self.model.language_model=torch.nn.Module()
            self.model.language_model.layers=torch.nn.ModuleList([DecoderLayer() for _ in range(28)])
            self.model.visual=torch.nn.Module();self.model.visual.blocks=torch.nn.ModuleList([VisionBlock() for _ in range(2)])
            self.model.visual.merger=torch.nn.Linear(2,2,bias=False);self.lm_head=torch.nn.Linear(2,2,bias=False)
    with torch.random.fork_rng():
        torch.manual_seed(24);model=Tiny().requires_grad_(False);configuration=lora_config()
        need(len(configuration.target_modules)==112>=MIN_TARGET_MODULES_FOR_OPTIMIZATION,'Fixture must exercise actual target condensation')
        inject_adapter_in_model(configuration,model);actual=model.peft_config['default']
        matched=require_effective_targets(model,actual)
        wrapped={name for name,module in model.named_modules() if isinstance(module,LoraLayer)}
        need(len(actual.target_modules)<112 and wrapped==matched==set(TARGETS)
             and len(adapter_parameters(model))==224,'Actual PEFT112-target condensation changed effective scope')
        widened=copy.deepcopy(actual);widened.target_modules=set(actual.target_modules)|{'qkv'}
        try:require_effective_targets(model,widened)
        except ValueError:rejected=True
        else:rejected=False
        need(rejected and effective_targets(model,widened)>set(TARGETS),'Widened visual selector must be rejected')
        return dict(passed=True,requested_targets=sorted(TARGETS),stored_targets=sorted(actual.target_modules),effective_targets=sorted(matched),
            actual_wrapped_modules=sorted(wrapped),adapter_tensors=224,widened_visual_selector_rejected=True,native_model_calls=0)
...
def check(args,out,frozen):
    ...
    torch.set_num_threads(4);bindings={};original_failure=verify_original_failure(bindings);bind(PARENT,bindings,PARENT_SHA);parent=read(PARENT);proof=read(PARENT.parent/'summary.json')
    ...
    bind(ORIGINAL_PLAN,bindings,ORIGINAL_PLAN_SHA);bind(ORIGINAL_PLAN.parent/'summary.json',bindings,ORIGINAL_CHECK_SHA)
    prior=read(ORIGINAL_PLAN);prior_proof=read(ORIGINAL_PLAN.parent/'summary.json')
    need(prior_proof['passed'] is prior_proof['completed'] is True and prior_proof['plan_sha256']==ORIGINAL_PLAN_SHA
         and prior['source_sha256']==original.sources() and prior['inherited_source_sha256']==original.inherited_sources(),'Passed original CPU plan changed')
    for field in ('rows_file','prepared_file','order_file'):bind(prior[field],bindings,prior['runtime_bindings'][prior[field]])
    prior_prepared=read(prior['prepared_file']);semantic=lambda item:{k:v for k,v in item.items() if k not in ('file','sha256')}
    need(rows==read(prior['rows_file']) and order==read(prior['order_file']) and cases==prior['profile_cases']
         and {sid:semantic(v) for sid,v in prepared.items()}=={sid:semantic(v) for sid,v in prior_prepared.items()},
         'Recomputed216 native layouts/targets/positions/order must equal the original CPU plan')
    original_check=dict(plan_file=str(ORIGINAL_PLAN),plan_sha256=ORIGINAL_PLAN_SHA,summary_file=str(ORIGINAL_PLAN.parent/'summary.json'),
        summary_sha256=ORIGINAL_CHECK_SHA,prepared_metadata_exact=True,order_exact=True,old_tensors_loaded=False)
    ...
    tests=cpu_fixtures(torch);tests['target_condensation']=condensation_fixture(torch);packages=installed(bindings)
    ...
    plan=dict(protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),original_failure=original_failure,original_check=original_check,
...
def verify_plan(path,streaming=False):
    ...
    need(plan['original_failure']==verify_original_failure({}),'Bound original failure differs')
...
def adapter_contract(torch,model):
    ...
    need(cfg['r']==16 and cfg['lora_alpha']==32 and cfg['lora_dropout']==.05 and cfg['bias']=='none'
         ...
         and cfg['modules_to_save'] is None and require_effective_targets(model,model.peft_config['default'])==set(TARGETS),'Saved PEFT configuration differs')
...
def install_lora(torch,model,config_observation):
    ...
    save(config_observation,canonical_peft_config(model))
...
def single_attempt_guard(out):
    need(os.environ.get('SLURM_JOB_NAME')==JOB,'Exact registered V2 LoRA profile job name required')
    ...
    for line in raw.splitlines():
        fields=line.split('|')
        if len(fields)!=9 or fields[1] not in (JOB,original.JOB):continue
        ...
        rows.append(dict(job_id=job,name=name,partition=partition,state=state,exit_code=exit_code,seconds=int(seconds),gpus=gpus))
    before=[r for r in rows if r['name']==original.JOB];current=[r for r in rows if r['name']==JOB]
    need(len(before)==len(current)==1 and before[0]['job_id']=='443880' and before[0]['state']=='FAILED'
         and before[0]['exit_code']=='1:0' and before[0]['seconds']==34 and current[0]['job_id']==os.environ['SLURM_JOB_ID']
         and all(r['partition']=='gpu' and r['gpus']==1 for r in rows) and current[0]['seconds']<=240
         and sum(r['seconds']*r['gpus'] for r in rows)<=274,'Exactly priorFAILED34s plus one boundedV2 profile required')
    save(out/'launch_accounting.json',dict(command=command,file=str(out/'launch_sacct.psv'),sha256=sha(out/'launch_sacct.psv'),
        matching_rows=rows,prior_failed_gpu_seconds=34,cumulative_gpu_seconds_cap=274))
...
def profile(args,out,frozen,started):
    ...
    config=dict(protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        original_failure=plan['original_failure'],original_check=plan['original_check'],orientation_plan=plan['orientation_plan'],training_stage=plan['training_stage'],confirmation_report=plan['confirmation_report'],
    ...
    for state in ('base','zero_lora'):
        if state=='zero_lora':
            tick=time.perf_counter();contract=install_lora(torch,model,out/'actual_peft_config_before_contract.json');params=adapter_parameters(model)
    ...
    for step in (1,2):
        for j,sid in enumerate(cases['microbatch_sids']):
            ...
            capture.update(step=step,microbatch=j+1,n_frames=row['n_frames'],counters=calls)
            file=data/f'training_{step}_{j+1:02d}.pt';torch.save(capture,file);digest=sha(file)
