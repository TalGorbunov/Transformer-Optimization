"""V2 canonical HF-symlink repair of the held216-scene orientation stage.
...
from scripts import stage_native_identity_join_factor_orientation as original
need=symbolic.need;read=symbolic.read;save=symbolic.save;oid=symbolic.oid
def sha(path):return symbolic.digest(Path(path))
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_factor_orientation_v2'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_factor_orientation_v2')
...
PROTOCOL='identity_join_factor_orientation_training_stage_v2'
...
REPAIR='docs/paper/NATIVE_AGGREGATION_FACTOR_ORIENTATION_STAGE_V2_REPAIR.md'
REPAIR_SHA='eebd0e67b2b6ceccc91cd0a719a4532ee8d3f1979c08e2a506d6cee36ef76818'
PRIOR_FAILURE=original.OUT/'stage_443650/failure.json'
PRIOR_FAILURE_SHA='3e1f12be71eca8837344bcab543556664121059a516d04ece8158ccaa572d04f'
PRIOR_RELEASE=original.OUT/'source_release.json'
PRIOR_RELEASE_SHA='1256d1c29acce571b6f5a10bbd0edcbd2f8de1be4671d934615ec770d6210366'
OWN=('scripts/stage_native_identity_join_factor_orientation_v2.py',
     'slurm/native_identity_join_factor_orientation_v2_stage.sbatch',REPAIR)
_READ={}


def bind(path,bindings,expected=None):
    path=Path(path).absolute();need(path.is_file(),'Missing bound input: '+str(path));resolved=path.resolve()
    if path.is_symlink():
        snapshot=symbolic.MODEL.absolute();blobs=snapshot.parent.parent/'blobs'
        need(expected is not None and path.is_relative_to(snapshot) and path.parent.resolve().is_relative_to(snapshot.resolve())
             and resolved.is_file() and not resolved.is_symlink() and resolved.is_relative_to(blobs.resolve()),
             'Only expected canonical HF snapshot-to-blob symlinks are allowed: '+str(path))
        targets=(path,resolved)
    else:targets=(resolved,)
    h=sha(resolved);need(expected is None or expected==h,'Changed orientation input: '+str(path))
    for target in targets:
        need(str(target) not in bindings or bindings[str(target)]==h,'Conflicting input hash')
        bindings[str(target)]=h;_READ[str(target)]=h
    return h


def preserved_failure():
    need(sha(PRIOR_FAILURE)==PRIOR_FAILURE_SHA and sha(PRIOR_RELEASE)==PRIOR_RELEASE_SHA,'Preserved V1 failure/release changed')
    failure=read(PRIOR_FAILURE);release=read(PRIOR_RELEASE);own,inherited=original.source_maps()
    need(release['source_sha256']==failure['source_sha256']==own and release['inherited_source_sha256']==inherited
         and len(own)==4 and len(inherited)==104 and failure['type']=='ValueError'
         and failure['message']=='Missing regular bound input: '+str(symbolic.MODEL/'config.json')
         and failure['partial_outputs_retained'] is True and not (PRIOR_FAILURE.parent/'plan.json').exists()
         and not (PRIOR_FAILURE.parent/'summary.json').exists(),'Original CPU-only pre-plan failure differs')
    for name,h in own.items():need(sha(PRIOR_FAILURE.parent/'source'/name.replace('/','_'))==h,'Failed source snapshot changed')
    return dict(failure_file=str(PRIOR_FAILURE),failure_sha256=PRIOR_FAILURE_SHA,source_release_file=str(PRIOR_RELEASE),source_release_sha256=PRIOR_RELEASE_SHA,
        original_source_sha256=own,original_inherited_source_sha256=inherited,original_stage_job='443650',original_allocated_cpu_seconds=55,
        no_original_model_or_head_call=True,no_original_fit=True)


def source_maps():
    need(sha(REPO/REPAIR)==REPAIR_SHA,'Held V2 repair note changed');prior=preserved_failure()
    inherited={**prior['original_inherited_source_sha256'],**prior['original_source_sha256']};need(len(inherited)==108,'All108 original sources required')
...
def binding_self_test(out):
    import tempfile
    config=symbolic.MODEL/'config.json';need(config.is_symlink(),'Regression fixture requires the actual failing HF config symlink')
    bindings={};expected=sha(config.resolve());bind(config,bindings,expected)
    need(bindings=={str(config):expected,str(config.resolve()):expected},'Snapshot and resolved-target hashes must both be retained')
    try:bind(config,{},'0'*64)
    except ValueError as exc:need('Changed orientation input' in str(exc),'Unexpected wrong-hash fixture failure')
    else:raise AssertionError('Changed HF content accepted')
    with tempfile.TemporaryDirectory(prefix='binding_fixture_',dir=out) as directory:
        directory=Path(directory);target=directory/'regular.txt';target.write_text('fixture');alias=directory/'alias.txt';alias.symlink_to(target)
        regular={};bind(target,regular,sha(target));need(regular=={str(target.resolve()):sha(target)},'Regular artifact binding differs')
        try:bind(alias,{},sha(target))
        except ValueError as exc:need('Only expected canonical HF' in str(exc),'Unexpected artifact-symlink fixture failure')
        else:raise AssertionError('Unrelated artifact symlink accepted')
    return dict(passed=True,real_HF_snapshot_and_blob_binding=True,wrong_hash_rejected=True,unrelated_symlink_rejected=True,regular_input_unchanged=True)
...
def render_rows(rows,cp,manifest,root,bindings,out):
    ...
    for row in records:row['origin']='canonical_identity_join_factor_orientation_v2'
...
def stage(out,frozen,inherited):
    bindings={};prior=preserved_failure();bind(PRIOR_FAILURE,bindings,PRIOR_FAILURE_SHA);bind(PRIOR_RELEASE,bindings,PRIOR_RELEASE_SHA)
    repair_test=binding_self_test(out);save(out/'binding_selftest.json',repair_test)
    factor,long,originals,old_pairs,cache,cp,manifest,cache_proof=parents(bindings)
    ...
    tests=self_test();tests['HF_binding_repair']=repair_test
    ...
    for path in (out/'original_rows.json',root/'semantic_rows.json',out/'pairs.json',out/'counterparts.json',out/'exclusion_inventory.json',
                 ...
                 out/'order.json',out/'head_rows.json',out/'selftests.json',out/'binding_selftest.json'):files[str(path)]=sha(path)
    for record in records:
        files[str(Path(record['path'])/'qa.txt')]=record['qa_sha256']
        for image in record['image_files']:files[image['path']]=image['sha256']
    plan=dict(schema_version=1,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited,preserved_v1_failure=prior,
    ...
    return dict(passed=True,completed=True,protocol=PROTOCOL,source_sha256=frozen,inherited_source_sha256=inherited,preserved_v1_failure=prior,
...
def verify_plan(path):
    ...
    need(plan['preserved_v1_failure']==summary['preserved_v1_failure']==preserved_failure(),'V2 failure provenance changed')
