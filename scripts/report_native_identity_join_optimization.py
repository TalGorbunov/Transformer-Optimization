...
def pilot():
    from scripts import diagnose_native_identity_join_optimization as module
    return module
...
def release(args,out,frozen):
    ...
    need(not any(r['name']==p.MAIN_JOB for r in budget['jobs']),'Main attempted before release')
    budget.update(reserved_main_gpu_seconds=2400,projected_total_with_main_reservations=budget['allocated_gpu_seconds']+2400)
    passed=seconds<=600 and budget['projected_total_with_main_reservations']<=3000;budget['passed']=passed
    projection=dict(passed=seconds<=600,projected_seconds=seconds,pooled_inputs=pooled,
    ...
    value=dict(passed=passed,completed=True,protocol='identity_join_optimization_main_release',source_sha256=frozen,
        ...
        pooled_projection=projection,campaign_budget=budget,per_main_seconds_cap=600,campaign_gpu_seconds_cap=3000,
