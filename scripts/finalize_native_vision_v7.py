"""Plot existing V7 metrics and bind final acceptance to both completed audits.
...
OUT = REPO/'outputs/native_aggregation_vlm/v7/finalization'
OWN = ('scripts/finalize_native_vision_v7.py', 'slurm/native_vision_v7_finalize.sbatch')
...
def plotted_rows(analysis):
    rows = []
    need(set(analysis['runs']) == {f'{arm}_s{seed}' for arm in ('joint', 'parallel') for seed in (8, 9)},
         'Require exactly the four registered reported runs')
    for seed in (8, 9):
        for arm in ('joint', 'parallel'):
            run = analysis['runs'][f'{arm}_s{seed}']
            need(run['arm'] == arm and run['seed'] == seed, 'Reported arm/seed differs')
            for cell, denominator in CELLS:
                ...
                rows.append(dict(arm=arm, seed=seed, cell=cell, n=metric['n'], correct=metric['correct'],
...
def match_selected(analysis, post):
    ...
    rows = {(row['arm'], row['seed']): row for row in post['runs']}
    need(set(rows) == {(arm, seed) for arm in ('joint', 'parallel') for seed in (8, 9)},
         'Duplicate or missing post-audit model')
    identities = []
    for arm in ('joint', 'parallel'):
        for seed in (8, 9):
            reported, audited = analysis['runs'][f'{arm}_s{seed}'], rows[arm, seed]
            ...
            identities.append(dict(arm=arm, seed=seed, run_id=audited['run_id'], selected_step=audited['selected_step'],
...
def plot(rows, directory):
    ...
    colors = dict(joint='#66717e', parallel='#1465ac')
    with plt.rc_context({'font.family': 'DejaVu Sans', 'font.size': 10, 'axes.spines.top': False,
        ...
        figure.suptitle('MMReD Vision: V7 native evaluation', fontsize=15, x=.06, ha='left')
        figure.legend(handles=[Patch(facecolor=colors[a], label=label) for a, label in
                      (('joint', 'Joint hidden adaptation'), ('parallel', 'Parallel local computation + fusion'))],
        ...
        for extension in ('png', 'pdf'):
            path = directory/f'native_vision_v7.{extension}'
...
def self_test():
    ...
    for arm in ('joint', 'parallel'):
        for seed in (8, 9):
            name = f'{arm}_s{seed}'
            analysis['runs'][name] = dict(arm=arm, seed=seed, run_directory='/fixture/'+name,
            ...
            post['runs'].append(dict(arm=arm, seed=seed, run_id=name, selected_step=972, checkpoint='/fixture/'+name,
    ...
    changed = copy.deepcopy(analysis); changed['runs']['joint_s8']['metrics']['cells']['main_test_N16']['n'] = 107
...
def finalize(args, out):
    ...
    need(len(stored_replays) == 40 and len(stored_cache) == 336
    ...
    text = ['# V7 final artifact', '',
        ...
        '![Existing per-cell native accuracy](native_vision_v7.png)', '',
        '[PDF figure](native_vision_v7.pdf) · [Acceptance and provenance](final_acceptance.json)', '',
