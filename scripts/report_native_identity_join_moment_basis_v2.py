...
OWN=('scripts/report_native_identity_join_moment_basis_v2.py',)
...
def driver():
    from scripts import diagnose_native_identity_join_moment_basis_v2 as module
    return module
...
def audit_original_failure(plan):
    root=REPO/'outputs/native_aggregation_vlm/identity_join_moment_basis';directory=root/'check_443829'
    expected=dict(file=str(directory/'failure.json'),sha256='d72ac218d5bf9a8d6cd83751442772bc7b5c87d2d1dc8fe3d61a50ff284db33e',
        original_source_release_file=str(root/'source_release.json'),original_source_release_sha256='cee44559ab199588d8ac9ed1393012a862cd61a5c4343cd0ef83bc29c0149fab')
    need(plan['original_failure']==expected,'Exact original metadata-failure descriptor required')
    bind(expected['file'],expected['sha256']);bind(expected['original_source_release_file'],expected['original_source_release_sha256'])
    failure=read(expected['file']);release=read(expected['original_source_release_file']);sources=release['source_sha256']
    need(release['status']=='frozen_before_execution' and failure['type']=='ValueError'
         and failure['message']=='Completed numerical negative evidence required' and failure['partial_outputs_retained'] is True
         and failure['source_sha256']==sources==read(directory/'source_hashes.json') and len(sources)==7
         and not (directory/'plan.json').exists() and not (directory/'summary.json').exists(),
         'Preserved preflight failure/source identity differs')
    request=read(directory/'request.json')
    need(request==dict(phase='check',arm=None,plan=None,source_sha256=sources)
         and read(directory/'inherited_sources.json')==dict(source_sha256=release['inherited_source_sha256']),
         'Original failed CPU request/ancestor archive differs')
    for group in (sources,release['inherited_source_sha256']):
        for name,digest in group.items():
            need(plan['inherited_source_sha256'].get(name)==digest,'Original source missing from V2 closure: '+name)
            bind(REPO/name,digest)
    for name,digest in sources.items():bind(directory/'source'/name.replace('/','_'),digest)
    return expected


def audit_inputs(torch,plan):
    audit_original_failure(plan)
...
def audit_run(torch,out,directory,plan_path,plan,frozen,rows,scenes,order,states,index,stats,initial,norm,head):
    ...
    for key in ('native_identity','native_identity_sha256','native_module_identity','native_model_file','native_model_sha256',
                ...
                'original_failure','paired_visual_plan','paired_visual_report','post_query_code_report','post_query_visual_report','query_placement','fixed_weight','moment_basis_comparison','orientation_plan','paired_order_stage','negative_evidence','decision_step','evaluation_steps'):
...
def resources(out,runs):
    ...
    job_arms={name:arm for arm,name in driver().RUN_JOBS.items()};names=set(job_arms);records=allocation_rows(raw,names|{'identity_join_moment_basis_v2','identity_join_moment_basis_v2_run'})
    ...
    old_gpu=allocation_rows(raw,{'identity_join_moment_basis','identity_join_moment_basis_run','identity_join_moment_basis_within','identity_join_moment_basis_cross'})
    need(not old_gpu,'The preserved original version must have no GPU attempt')
    original_cpu=allocation_rows(raw,{'identity_join_moment_basis_check'})
    need(len(original_cpu)==1 and original_cpu[0]['job_id']=='443829' and original_cpu[0]['partition']=='cpu'
         and original_cpu[0]['state']=='FAILED' and original_cpu[0]['exit_code']=='1:0'
         and original_cpu[0]['seconds']==5 and original_cpu[0]['gpus']==0,'Preserved original CPU allocation differs')
    ...
    return dict(passed=True,jobs=records,allocated_gpu_seconds=total,maximum_concurrent_gpus=peak,
        preserved_original_cpu_job=original_cpu[0],preserved_original_cpu_seconds=5,preserved_original_gpu_seconds=0,
...
def report(args,out,frozen):
    ...
    result=dict(passed=all(r['passed'] for r in runs.values()),completed=True,all_numerical_audits_collected=True,phase='report',protocol=p.PROTOCOL,
        ...
        factor_plan=plan['factor_plan'],cache_report=plan['cache_report'],confirmation_report=plan['confirmation_report'],training_stage=plan['training_stage'],original_failure=plan['original_failure'],
    ...
    lines+=['','Only step6000 determines acceptance:206/216 pooled,103/108 per orientation and16/18 complete12-context families. '
        ...
        'The preserved original metadata failure consumed5 allocated CPU seconds and zero GPU seconds. VLM/vision calls were zero. CPU audit covered44 captures and28 endpoint head batches/432 rows, with exact argmax and TV<=0.02. '
    ...
    return dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        primary_endpoint_step=6000,first_token_screen=result['first_token_screen'],resources=budget,original_failure=plan['original_failure'],bases=list(ARMS),query_placement='pre_silu',fixed_weight=.5,
