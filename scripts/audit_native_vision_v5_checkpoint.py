"""Frozen-checkpoint V5 state/numerical audit and ten software evaluations.
...
PRE=BASE/'profile/mean/mean_seed4_20260910_200035_441450_2903314'
OWN=('scripts/audit_native_vision_v5_checkpoint.py',
     'slurm/native_aggregation_vision_v5_audit_check.sbatch',
     'slurm/native_aggregation_vision_v5_audit.sbatch',
...
def make_plan(torch,phase,directories):
    ...
    ensure((directories==[PRE] if phase=='pre' else len(directories)==4 and len(set(directories))==4),'Wrong fixed run list')
    runs=[]; frozen={}
    for directory in directories:
        ...
        if phase=='post':
                   ...
                   'Post audit requires the canonical V5 main condition/seed directory')
        for name,digest in config['code_sha256'].items():
            ensure(sha(REPO/name)==digest,f'Frozen V5 source changed: {name}')
            frozen[name]=digest
        ...
        del saved
    wanted={('mean',4)} if phase=='pre' else {(arm,seed) for arm in ('sum','mean') for seed in (4,5)}
...
def execute(torch,plan,plan_path):
    ...
    output=OUT/f'{phase}_run_{job}';tensor_root=DATA/'v5_checkpoint_audits'/f'{phase}_{job}'
    ...
    with torch.no_grad():
        for source in plan['runs']:
            ...
            branch=attach_independent_vision_aggregation(model,layer_index=14,rank=96,merge=source['condition'])
    ...
    (output/'INDEX.md').write_text('# V5 frozen-checkpoint audit\n\n[Summary](summary.json) · [Plan](plan.json).\n\n'
...
def main():
    ...
    if args.check:
        ...
        for name,digest in plan['frozen_profile_source_sha256'].items():ensure(sha(REPO/name)==digest,'Frozen V5 source changed')
