"""Native MMReD Vision aggregation pilot, using the ordinary Qwen2.5-VL forward.
...
def parser():
    ...
    p.set_defaults(model="Qwen/Qwen2.5-VL-7B-Instruct", train_ns=[8, 16, 32],
                   dev_ns=[8], eval_ns=[8, 32, 64], limit_train=9, limit_dev=9,
                   limit_eval=9, epochs=1, max_seq_tokens=16000,
                   output=REPO_ROOT / "outputs/native_aggregation_vlm")
...
def run(a) -> int:
    ...
    checkpoint_dir = a.checkpoint_root / "native_aggregation_vlm" / run_id
    code_dir = outdir / "code"
    code_dir.mkdir()
    code_hashes = {}
    for name in ("scripts/native_aggregation_vlm.py", "scripts/native_aggregation.py",
    ...
    config.update(run_id=run_id, slurm_job_id=os.environ["SLURM_JOB_ID"],
                  ...
                  decoding="Ordinary greedy model.generate(use_cache=True), unrestricted vocabulary",
    ...
    def restore(path: Path | str) -> None:
        ...
        if compatible_architecture(saved["architecture"]) != architecture:
    ...
    def evaluate(sets, tag, mode="all"):
        ...
        for n, records in sets.items():
            ...
            for record in records:
                ...
                rows.append(dict(tag=tag, mode=mode, path=record["path"], sid=record["sid"],
            ...
            count = len(records)
            metric = dict(tag=tag, mode=mode, n_frames=n, n=count, correct=correct,
    ...
    write_json(outdir / "summary.json", dict(
        ...
        metric_notes=dict(
            ...
            gold_first_token_nll="Native full-vocabulary NLL at first answer position, not whole-answer likelihood",
            ...
            scope="K<=8; longer N tests distractor-length robustness, not numerical extrapolation",
        ),
    ))
