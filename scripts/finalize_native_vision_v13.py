"""Plot existing V13 metrics and bind final acceptance to both completed audits.
...
OUT = REPO/'outputs/native_aggregation_vlm/v13/finalization'
OWN = ('scripts/finalize_native_vision_v13.py', 'slurm/native_vision_v13_finalize.sbatch')
...
ANCESTORS = ('scripts/finalize_native_vision_v10.py', 'slurm/native_vision_v10_finalize.sbatch')
ACCOUNTING_FIELDS = ('JobIDRaw','JobID','JobName','Partition','State','ElapsedRaw','AllocTRES','ExitCode','Submit','Start','End')
JOB_CAPS = {'v13_native_profile':300, 'v13_null_profile':180, 'v13_null_harvest':600,
            'v13_train_profile':300, 'v13_main':2700, 'v13_selected':300}
    ...
    for seed in SEEDS:
        for condition in CONDITIONS:
            ...
            need(run['condition'] == condition and run['seed'] == seed and run['verified_test_examples'] == 272
                 ...
                 and run['verified_auxiliary_presentations'] == 73440,
...
def acceptance(analysis, report_summary, post, accounting):
    ...
    return dict(report_audit_passed=analysis['passed'] and analysis['audit_passed'] and report_summary['passed'],
        ...
        v13_concurrency_passed=accounting['concurrency_passed'],
...
def plot(rows, directory):
    ...
    with plt.rc_context({'font.family': 'DejaVu Sans', 'font.size': 10, 'axes.spines.top': False,
        ...
        figure.text(.06, .025, 'Maximum training length: 16 frames. All K0–16 values appear in adaptation; this does not test unseen answer values.\n'
                    'Both arms: same core, null predictor, CE, residual consistency and auxiliary calibration. Labels copy existing correct/total.',
                    fontsize=8, color='#424a52')
        figure.subplots_adjust(left=.08, right=.98, top=.85, bottom=.14, hspace=.42, wspace=.17)
        paths = []
        for extension in ('png', 'pdf'):
            path = directory/f'native_vision_v13.{extension}'
...
def parse_accounting(raw):
    ...
    for line in raw.splitlines():
        ...
        if value['Partition']!='gpu' or not value['JobName'].startswith('v13_'): continue
        ...
        need(state in TERMINAL and value['ElapsedRaw'].isdigit(), 'Unfinished or malformed V13 allocation')
        ...
        need(value['JobName'] in JOB_CAPS, 'Unregistered V13 GPU job name: '+value['JobName'])
    ...
    need(rows, 'Scheduler returned no V13 GPU allocations')
    return rows
...
def accounting_metrics(rows):
    ...
    feature=sum(r['allocated_gpu_seconds'] for r in rows if r['JobName'] in ('v13_null_profile','v13_null_harvest'))
    software=sum(r['allocated_gpu_seconds'] for r in rows if r['JobName']=='v13_native_profile')
    per_job=all(r['per_job_cap_passed'] for r in rows)
    subcaps=feature<=2700 and software<=600
    return dict(allocated_gpu_seconds=total,allocated_gpu_hours=total/3600,
        ...
        maximum_concurrent_v13_gpus=peak,concurrency_passed=peak<=4,
        per_job_caps_passed=per_job,feature_allocated_gpu_seconds=feature,native_software_allocated_gpu_seconds=software,
        feature_gpu_seconds_cap=2700,native_software_gpu_seconds_cap=600,subcaps_passed=subcaps,
        ...
        scope='All user v13_ GPU allocations since 2026-09-01, including failed and zero-allocation attempts. Concurrency is campaign-wide; other project campaigns remain the root scheduler responsibility.')
...
def scheduler_accounting(analysis,post,out):
    ...
    add(cache['profile_directory'],'cache_profile','v13_null_profile',cache['profile_summary_sha256'])
    need(len(cache['shards'])==4 and {s['shard'] for s in cache['shards']}==set(range(4)), 'Four new union shards required')
    for shard in cache['shards']: add(shard['directory'],'cache_shard_'+str(shard['shard']),'v13_null_harvest',shard['summary_sha256'])
    release_ref=analysis['release']; bind(release_ref['file'],release_ref['sha256']); release=read(release_ref['file'])
    need(release['protocol']=='v13_learned_null_main_release' and release['passed'] and set(release['profiles'])==set(CONDITIONS),
    ...
    add(Path(software['file']).parent,'native_software','v13_native_profile',software['sha256'])
    for condition,item in release['profiles'].items(): add(item['directory'],'training_profile_'+condition,'v13_train_profile',item['summary_sha256'])
    for name,run in analysis['runs'].items(): add(run['run_directory'],'main_'+name,'v13_main',run['summary_sha256'])
    job=str(post['slurm_job_id']); need(job.isdigit() and job not in required, 'Post GPU allocation missing or duplicated')
    required[job]=dict(phase='selected_checkpoint_native_audit',job_name='v13_selected',seconds_cap=300)
    need(len(required)==13, 'Require one software, one cache profile, four shards, two training profiles, four mains and one selected audit')
    ...
    save(out/'accounting_request.json',dict(command=command,scope='Independent all-user discovery; retain every v13_ GPU allocation'))
    ...
    need(len({i['accounted_job_id_raw'] for i in required.values()})==13, 'Successful artifact roles reused one allocation')
...
def audit_post(post,plan):
    need(post['protocol']==plan['protocol']=='all_selected_v13_native_checkpoint_audit'
...
def self_test():
    ...
    for condition in CONDITIONS:
        for seed in SEEDS:
            ...
            analysis['runs'][name]=dict(condition=condition,seed=seed,run_directory='/fixture/'+name,
                ...
                verified_pair_presentations=36720,verified_auxiliary_presentations=73440,verified_optimizer_log_rows=4590,metrics=metrics)
    ...
    for edit in ('denominator','partition'):
        changed=copy.deepcopy(analysis);m=changed['runs']['offset_s16']['metrics']
    ...
    changed=copy.deepcopy(analysis);changed['decisions']['16']['primary']=True
    ...
    def record(job,name='v13_main',elapsed=10,start='2026-09-11T01:00:00',end='2026-09-11T01:00:10',state='COMPLETED',tres='gres/gpu=1',exitcode='0:0'):
    ...
    need(measured['allocated_gpu_seconds']==30 and measured['maximum_concurrent_v13_gpus']==2
    ...
    failed_software=parse_accounting(record(1,name='v13_native_profile',elapsed=300,state='FAILED',exitcode='1:0')+
        record(2,name='v13_native_profile',elapsed=300)+record(3,name='v13_native_profile',elapsed=1))
    need(not accounting_metrics(failed_software)['subcaps_passed'], 'Failed software attempts omitted from subcap')
    for malformed in (record(1,state='RUNNING'),record(1,name='v13_unknown'),record(1,tres='gres/gpu=2'),record(1)+record(1)):
    ...
    return dict(passed=True,tests=['mandatory_report_post_and_resource_gates','descriptive_cache_failure_retained',
        ...
        'registered_per_seed_decisions_rederived','four_GPU_concurrency_limit','actual_per_job_caps','software_retry_subcap',
...
def finalize(args, out):
    ...
    text = ['# V13 final artifact', '',
        ...
        f"V13 allocated compute: {accounting['allocated_gpu_seconds']} GPU-seconds / {GPU_SECONDS_CAP}; all failed attempts remain in the accounting.",
