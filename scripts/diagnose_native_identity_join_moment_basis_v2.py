"""Matched within/cross moment coordinates under fixed native visual CE."""
...
from scripts import diagnose_native_identity_join_moment_basis as frozen_driver
...
PEOPLE=original.PEOPLE;ARMS=('within','cross')
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_moment_basis_v2'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_moment_basis_v2')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_moment_basis_v2')
PROTOCOL='identity_join_moment_basis_v2'
RUN_JOBS={arm:'identity_join_moment_basis_v2_'+arm for arm in ARMS}
...
PROPOSAL='docs/paper/NATIVE_AGGREGATION_MOMENT_BASIS_V2_PROPOSAL.md'
PROPOSAL_SHA='ba0fa39b28ad4996b6a7868d44f627fb83921e4370bfd179114210f6b5d3b985'
DIAGNOSTIC_STEPS=(1,2,32,128,300,600,2000,6000)
EVALUATION_STEPS=(6000,);PROGRESS_STEPS=(600,2000,4000,6000)
POLICY={k:v for k,v in paired_visual.POLICY.items() if k not in (
    'frozen_selector_parameters','mode','fixed_selector_weight','fixed_selector_bias','factor_rule','paired_only',
    'shared_study_maximum_concurrent_gpus','shared_study_gpu_seconds_cap')}
POLICY.update(protocol=PROTOCOL,arms=list(ARMS),retained_state_parameters=1404192,effective_trainable_parameters=1404192,
    fresh_shared_initialization=True,reused_unfitted_initialization=False,paired_minibatch_only=False,
    bases=list(ARMS),fixed_weight=.5,query_placement='pre_silu',moment_basis_comparison=True,
    readout_input_width=288,no_selector_parameters=True,both_moment_bases_computed=True,
    max_training_head_rows=48,maximum_training_head_rows=48,
    norm_calls={a:6014 for a in ARMS},head_calls={a:6014 for a in ARMS},core_calls={a:6014 for a in ARMS},
    total_head_rows={a:213546 for a in ARMS},gpu_seconds_cap=120,campaign_gpu_seconds_cap=240,
    cpu_check_seconds=90,cpu_report_seconds=300,maximum_concurrent_gpus=2)
OWN=('scripts/diagnose_native_identity_join_moment_basis_v2.py',
    'scripts/report_native_identity_join_moment_basis_v2.py','slurm/native_identity_join_moment_basis_v2_check.sbatch',
    'slurm/native_identity_join_moment_basis_v2_run.sbatch','slurm/native_identity_join_moment_basis_v2_report.sbatch',PROPOSAL)
REPORTS={
 'paired_visual_report':(paired_visual.OUT/'report_443743/summary.json','551a03afc57d343653f53f966d79fbcb2957837f16df50a84413ff0fe71775a8'),
 'post_query_code_report':(REPO/'outputs/native_aggregation_vlm/identity_join_orientation_post_query_code/report_443794/summary.json','9da20cc3754e734ee62dc55f551c9fe41b58e2947898c9c11d9253d334b0c3f3'),
 'post_query_visual_report':(REPO/'outputs/native_aggregation_vlm/identity_join_orientation_post_query_visual/report_443795/summary.json','dd56720c974fcbb44e45b55c9d14472f91d6b122af0522d92e60093ce7ee9489')}

REPORT_PROTOCOLS={
    'paired_visual_report':'identity_join_orientation_paired_visual',
    'post_query_code_report':'identity_join_orientation_post_query_local_joint_code_oracle',
    'post_query_visual_report':'identity_join_orientation_post_query_visual'}
ORIGINAL_FAILURE=REPO/'outputs/native_aggregation_vlm/identity_join_moment_basis/check_443829/failure.json'
ORIGINAL_FAILURE_SHA='d72ac218d5bf9a8d6cd83751442772bc7b5c87d2d1dc8fe3d61a50ff284db33e'
ORIGINAL_RELEASE=REPO/'outputs/native_aggregation_vlm/identity_join_moment_basis/source_release.json'
ORIGINAL_RELEASE_SHA='cee44559ab199588d8ac9ed1393012a862cd61a5c4343cd0ef83bc29c0149fab'


def verify_original_failure(bindings=None):
    bindings={} if bindings is None else bindings
    bind(ORIGINAL_FAILURE,bindings,ORIGINAL_FAILURE_SHA);bind(ORIGINAL_RELEASE,bindings,ORIGINAL_RELEASE_SHA)
    failure=read(ORIGINAL_FAILURE);release=read(ORIGINAL_RELEASE);directory=ORIGINAL_FAILURE.parent
    need(failure['type']=='ValueError' and failure['message']=='Completed numerical negative evidence required'
         and failure['partial_outputs_retained'] is True and 0<=failure['elapsed_seconds']<=90
         and not (directory/'plan.json').exists() and not (directory/'summary.json').exists(),
         'Preserved pre-fixture CPU443829 failure differs')
    need(release['status']=='frozen_before_execution' and release['source_sha256']==failure['source_sha256']==frozen_driver.sources()
         and release['inherited_source_sha256']==frozen_driver.inherited_sources()
         and release['policy']==frozen_driver.POLICY,'Original frozen seven-source release differs')
    bind(directory/'request.json',bindings)
    need(read(directory/'request.json')==dict(phase='check',arm=None,plan=None,source_sha256=release['source_sha256']),
         'Original failed CPU request differs')
    bind(directory/'source_hashes.json',bindings);bind(directory/'inherited_sources.json',bindings)
    need(read(directory/'source_hashes.json')==release['source_sha256']
         and read(directory/'inherited_sources.json')==dict(source_sha256=release['inherited_source_sha256']),
         'Preserved failed source descriptors differ')
    for name,digest in release['source_sha256'].items():bind(directory/'source'/name.replace('/','_'),bindings,digest)
    return dict(file=str(ORIGINAL_FAILURE),sha256=ORIGINAL_FAILURE_SHA,
        original_source_release_file=str(ORIGINAL_RELEASE),original_source_release_sha256=ORIGINAL_RELEASE_SHA)


def make_core(torch,arm,hidden_size=3584,rank=96):
    from gnnformer.parallel_local_moment_basis import ParallelLocalMomentBasis
    return ParallelLocalMomentBasis(hidden_size,rank=rank,basis=arm)


def check_core(core,arm):
    need(type(core).__module__=='gnnformer.parallel_local_moment_basis' and type(core).__name__=='ParallelLocalMomentBasis'
         and core.basis==arm and arm in ARMS and core.merge=='sum' and core.post_activation=='silu', 'Exact fixed moment core required')
    ...
    need({n:list(p.shape) for n,p in core.named_parameters()}==shapes and not list(core.buffers())
         and all(str(p.dtype)=='torch.float32' for p in core.parameters()),'Six-tensor moment architecture differs')


def trainability(core):
    check_core(core,core.basis)
    need(all(p.requires_grad and p.grad is None for p in core.parameters()),'All six fresh parameters must be trainable')
    result=dict(trainable_parameter_names=[n for n,p in core.named_parameters()],
        effective_trainable_parameters=sum(p.numel() for p in core.parameters()),retained_state_parameters=sum(p.numel() for p in core.parameters()))
    need(result['effective_trainable_parameters']==result['retained_state_parameters']==1404192,'Moment parameter count differs')
    return result


def query_means(torch,stats,scenes,sids,layout=None,*,arm):
    need(arm in ARMS,'Unknown moment basis')
    return original.original.query_means(torch,stats,scenes,sids,layout,arm='global')


def inherited_sources():
    result={}
    for group in (frozen_driver.sources(),frozen_driver.inherited_sources()):
        for name,digest in group.items():
            need(name not in result or result[name]==digest,'Inherited source conflict');result[name]=digest
    need(len(result)==223,'Original seven plus216 inherited sources required')
...
def evidence(bindings):
    result={}
    for key,(file,digest) in REPORTS.items():
        ...
        need(proof['passed'] is proof['completed'] is analysis['passed'] is analysis['completed'] is analysis['all_numerical_audits_collected'] is True
             and proof['phase']==analysis['phase']=='report' and proof['protocol']==analysis['protocol']==REPORT_PROTOCOLS[key],
             'Completed numerical negative evidence required')
        need(proof['source_sha256']==analysis['source_sha256'] and proof['inherited_source_sha256']==analysis['inherited_source_sha256'],'Evidence source ownership differs')
        for name,source_sha in proof['source_sha256'].items():bind(file.parent/'source'/name.replace('/','_'),bindings,source_sha)
        scores=analysis['first_token_screen']
        expected={'product':77,'additive':73} if key=='paired_visual_report' else ({'product':84,'additive':74} if key=='post_query_visual_report' else {'joint_code':84})
        need(set(scores)==set(expected) and all(scores[a]['first_correct']==n and scores[a]['complete_families']==0 and not scores[a]['passed'] for a,n in expected.items()),'Fixed failed216 outcomes differ')
        if key=='paired_visual_report':need(analysis['plan_file']==str(PARENT) and analysis['plan_sha256']==PARENT_SHA,'Matched input preparation differs')
...
def self_test(torch):
    ...
    from gnnformer.parallel_local_moment_basis import self_test as core_self_test
    return dict(passed=True,criterion_groups=4,call_inventory=True,moment_core=core_self_test(torch))


def check(args,out,frozen):
    import torch
    torch.set_num_threads(4);bindings={};repair=verify_original_failure(bindings);bind(PARENT,bindings,PARENT_SHA);parent=paired_visual.verify_plan(PARENT,ancestors=True)
    ...
    ckpt=CKPT/out.name;ckpt.mkdir(parents=True,exist_ok=False);initial_file=ckpt/'initial.pt'
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(24);core=make_core(torch,'within');mask=trainability(core);initial_state=base.v7.state_info(core)
        need(bool((core.up.weight==0).all()),'Fresh U must be exactly zero')
        torch.save(dict(branch=cpu_tree(torch,core.state_dict()),step=0,seed=24,
            initialization='query,local,aggregate,up default Linear draws; zero local/aggregate biases and U',
            bases=list(ARMS),fresh_shared_initialization=True),initial_file)
    packet=torch.load(initial_file,map_location='cpu',weights_only=True)
    for arm in ARMS:
        core=make_core(torch,arm);core.load_state_dict(packet['branch']);check_core(core,arm)
        need(base.v7.state_info(core)==initial_state and trainability(core)==mask,'Shared fresh initialization/mask differs')
    tests=self_test(torch)
    from scripts.report_native_identity_join_moment_basis_v2 import self_test as reporter_self_test
    ...
    for file in (initial_file,out/'order.json',out/'head_rows.json',PARENT,ORIGINAL_FAILURE,ORIGINAL_RELEASE):bind(file,runtime)
    ...
    plan=dict(parent,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),tests=tests,
        input_bindings=bindings,runtime_bindings=runtime,original_failure=repair,paired_visual_plan=dict(file=str(PARENT),sha256=PARENT_SHA),**dependencies,
        ...
        query_placement='pre_silu',bases=list(ARMS),fixed_weight=.5,moment_basis_comparison=True,paired_minibatch_only=False,
        initial_state=initial_state,trainability_mask=mask,fresh_shared_initialization=True,reused_unfitted_initialization=False,
    ...
    return dict(passed=True,completed=True,phase='check',protocol=PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        original_failure=repair,orientation_plan=plan['orientation_plan'],paired_order_stage=stage_ref,paired_visual_plan=plan['paired_visual_plan'],**dependencies,
        query_placement='pre_silu',bases=list(ARMS),fixed_weight=.5,moment_basis_comparison=True)


def verify_plan(path,ancestors=False):
    path=Path(path).resolve();plan=read(path)
    need(plan['original_failure']==verify_original_failure(),'Original failed check evidence changed')
...
def run(args,out,frozen,started):
    ...
    need(base.v7.state_info(core)==plan['initial_state'] and mask==plan['trainability_mask']
         and core.basis==args.arm,'Exact shared fresh core/mask/basis required')
    ...
    config=dict(protocol=PROTOCOL,original_failure=plan['original_failure'],run_id=out.name,slurm_job_id=os.environ['SLURM_JOB_ID'],arm=args.arm,seed=24,policy=POLICY,
        ...
        conditioning_plan=plan['conditioning_plan'],conditioning_report=plan['conditioning_report'],fresh_shared_initialization=True,reused_unfitted_initialization=False,
        ...
        paired_minibatch_only=False,basis=core.basis,query_placement='pre_silu',fixed_weight=.5,moment_basis_comparison=True,
        paired_visual_plan=plan['paired_visual_plan'],paired_visual_report=plan['paired_visual_report'],post_query_code_report=plan['post_query_code_report'],post_query_visual_report=plan['post_query_visual_report'],
        unchanged_presentation_multiset=True,no_automatic_native_or_fresh_release=True,
    ...
    def save_capture(local,g,valid,means,cap,layout,sids,weights_file,weights_step):
        need(core.basis==args.arm,'Only the fixed moment basis is authorized')
        ...
        blob=dict(schema_version=1,arm=args.arm,phase=current_phase,step=current_step,endpoint_step=current_endpoint,basis=core.basis,query_placement='pre_silu',sids=sids,layout=layout,
        ...
        record=dict(arm=args.arm,phase=current_phase,step=current_step,endpoint_step=current_endpoint,basis=core.basis,query_placement='pre_silu',sids=sids,file=str(path),sha256=sha(path),
    ...
    def evaluate(update):
        ...
        save(endpoint_file,dict(passed=True,endpoint_step=update,before_evaluation=table,after_evaluation=base.v7.state_info(core),parameter_sha256=object_sha(table),
            ...
            statistics_versions_unchanged=True,basis=core.basis,query_placement='pre_silu',hooks_removed=True,
    ...
    try:
        for step in range(1,6001):
            ...
            if capture_current:
                ...
                torch.save(dict(arm=args.arm,branch=cpu_tree(torch,core.state_dict()),step=step-1,optimizer_step=step,position='before_update',basis=core.basis,query_placement='pre_silu',source_sha256=frozen),weights_file)
            ...
            counters['conditioning']+=hook.calls
            check_core(core,args.arm)
            need(core.basis==args.arm
                 and torch.equal(cap['gates'],valid.float()*.5) and bool((cap['scores']==.5).all()),'Fixed moment basis/half weights changed')
            ...
            gradnorm=torch.nn.utils.clip_grad_norm_(parameters,1.);optimizer.step();check_core(core,args.arm)
            ...
            logs.append(dict(step=step,sids=sids,pair_ids=[p['pair_id'] for p in batch],cycles=[p['cycle'] for p in batch],target_ids=layout['targets'],
                ...
                gradient_norm=float(gradnorm),clipped=float(gradnorm)>1.,native_frozen=True,statistics_frozen=True,trainable_parameters_finite=True,
        ...
        result=dict(**config,passed=True,completed=True,phase='run',computational_integrity_passed=True,steps=6000,
            **{key:value for key,value in last.items() if key not in ('endpoint_step','descriptive_only')},selected_parameter_sha256=object_sha(final),checkpoint_roundtrip_passed=True,
            ...
            statistics_frozen=True,statistics_versions_unchanged=True,
            input_versions_unchanged=True,cached_first_query_only=True,no_pretrained_backbone_loaded=True,no_dev_or_test=True,no_native_or_whole_answer_claim=True)
        need(time.perf_counter()-started<=120,'Single moment-basis job exceeded120 seconds');return result
    finally:
...
def main():
    ...
    if args.check:
        need(args.plan is None and args.arm is None and os.environ.get('SLURM_JOB_PARTITION')=='cpu'
             and not os.environ.get('SLURM_JOB_GPUS'),'CPU-only fixed moment preparation required')
