"""Frozen V10 pair metadata and persistent complete-epoch presentation order."""
...
def presentation_order(pairs, seed):
    need(seed in (14, 15) and len(pairs) == 918, 'Unregistered seed or pair count')
...
def load_pairs(manifest_file=MANIFEST, cache_file=CACHE, schedule_file=SCHEDULE):
    ...
    need((manifest_file, cache_file, schedule_file) == (MANIFEST, CACHE, SCHEDULE), 'Require the exact V10 corpus/cache')
    ...
    return dict(schema_version=1, protocol='v10_equal_question_count_pairing', pairs=pairs,
        ...
        target_positions_per_epoch=4428, epochs=40, seeds=[14,15])
...
def self_test():
    ...
    a = presentation_order(pairs, 14)
    need(a == presentation_order(pairs,14) and a != presentation_order(pairs,15), 'Seed/order determinism differs')
