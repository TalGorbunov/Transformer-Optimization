...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_moment_basis'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_moment_basis')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_moment_basis')
PROTOCOL='identity_join_moment_basis'
RUN_JOBS={arm:'identity_join_moment_basis_'+arm for arm in ARMS}
...
PROPOSAL='docs/paper/NATIVE_AGGREGATION_MOMENT_BASIS_PROPOSAL.md'
PROPOSAL_SHA='30c74851a76f03c8e542cf0bb9531a76e92918f1736eb1611e5099362555333e'
...
OWN=('gnnformer/parallel_local_moment_basis.py','scripts/diagnose_native_identity_join_moment_basis.py',
    'scripts/report_native_identity_join_moment_basis.py','slurm/native_identity_join_moment_basis_check.sbatch',
    'slurm/native_identity_join_moment_basis_run.sbatch','slurm/native_identity_join_moment_basis_report.sbatch',PROPOSAL)
REPORTS={
...
def inherited_sources():
    ...
    for file,digest in REPORTS.values():
        need(sha(file)==digest,'Fixed negative evidence changed');proof=read(file)
...
def evidence(bindings):
    ...
    for key,(file,digest) in REPORTS.items():
        ...
        need(proof['passed'] is proof['completed'] is analysis['passed'] is analysis['completed'] is analysis['all_numerical_audits_collected'] is True
             and proof['phase']==analysis['phase']=='report' and proof['protocol']==analysis['protocol']==file.parent.parent.name,
...
def check(args,out,frozen):
    ...
    from scripts.report_native_identity_join_moment_basis import self_test as reporter_self_test
