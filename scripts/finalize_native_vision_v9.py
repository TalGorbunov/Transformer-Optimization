"""Plot existing V9 metrics and bind final acceptance to both completed audits.
...
OUT = REPO/'outputs/native_aggregation_vlm/v9/finalization'
OWN = ('scripts/finalize_native_vision_v9.py', 'slurm/native_vision_v9_finalize.sbatch')
...
def plotted_rows(analysis):
    rows = []
    need(set(analysis['runs']) == {f'{condition}_s{seed}' for condition in ('residual', 'path') for seed in (12, 13)},
         'Require exactly the four registered reported runs')
    for seed in (12, 13):
        for condition in ('residual', 'path'):
...
def match_selected(analysis, post):
    ...
    need(set(rows) == {(condition, seed) for condition in ('residual', 'path') for seed in (12, 13)},
...
def plot(rows, directory):
    ...
    colors = dict(residual='#66717e', path='#1465ac')
    with plt.rc_context({'font.family': 'DejaVu Sans', 'font.size': 10, 'axes.spines.top': False,
        ...
        figure.suptitle('MMReD Vision: V9 native evaluation', fontsize=15, x=.06, ha='left')
        figure.legend(handles=[Patch(facecolor=colors[a], label=label) for a, label in
                      (('residual', 'CE + residual consistency'), ('path', 'CE + path bound'))],
        ...
        for extension in ('png', 'pdf'):
            path = directory/f'native_vision_v9.{extension}'
...
def self_test():
    ...
    changed = copy.deepcopy(analysis); changed['runs']['residual_s12']['metrics']['cells']['main_test_N16']['n'] = 107
...
def finalize(args, out):
    ...
    text = ['# V9 final artifact', '',
        ...
        '![Existing per-cell native accuracy](native_vision_v9.png)', '',
        '[PDF figure](native_vision_v9.pdf) · [Acceptance and provenance](final_acceptance.json)', '',
