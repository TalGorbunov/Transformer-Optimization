"""Independent CPU release and report for the fixed trainability diagnostic.
...
def pilot():
    from scripts import diagnose_native_identity_join_optimization_v3 as module
    return module
...
def config_and_endpoint(torch,directory,plan_path,profile):
    ...
    need(directory.parent==p.OUT and directory.name==config['run_id'] and config['profile'] is profile and config['seed']==24
         and config['condition']==config['selection_mode']==mode and config['consistency_coefficient']==coefficient
...
def audit_training(torch,directory,config,summary,plan):
    ...
    need([x['step'] for x in grads]==[1,2] and all(set(x['pre_clip'])==set(config['initialized']) for x in grads),'Initial gradient coverage differs')
...
def audit_gradients(torch,diagnostics,logs,config):
    names=list(config['initialized']);expected_groups=dict(all=names,selector=[n for n in names if n.startswith('selection_')],
...
# Exact preserved failures caused by the immutable progress-file collision.
PRIOR_FAILED_RUNS = {'run_clip_ce_s24_443208': 'bf391c8c915778b2ecfaa7929135e8d921b5d7f7680c25b846b544d526cc214b', 'run_clip_consistency_s24_443209': '02e749f1e11815ad78ab1232fad5eb48d88afbe6ad7aff8d0d626c6e822683b0', 'run_sigmoid_ce_s24_443210': '8460f494f9d0eaff308fcf5b3fc2b242b6b2451747d16660faf0e6f3b2e59cd6', 'run_sigmoid_consistency_s24_443207': '5e48465ef358510d4e7db267328c3b13db1b6e9ab1318f0165a58ea78157973a'}

PRIOR_FAILED_ARTIFACTS = {'run_clip_ce_s24_443208': {'config.json': 'e4103b34d9278a61a6412907628279bb39d3e023aa774827746f5f3cad093d66', 'training.json': '13c2d3d49f544140134b83980e41eadbeb0ccb678b7b3ad932cb28727e599263', 'gradient_diagnostics.json': '9fdf0b4a8249c7506a51e9bed586bfe869c4b410f48af8862338882c9da5857e'}, 'run_clip_consistency_s24_443209': {'config.json': '9c9343a35485efb2b5ade3d329c733fe28edba90eb4b09c7461eafe1bd274319', 'training.json': '1ba31de21f28b24093e5eff257874567109e455824baf335d8d920bb4c6959b2', 'gradient_diagnostics.json': '7a45656ce5f3c6293eb147d378176d9499d0c00a378231213c02e6aefa21081f'}, 'run_sigmoid_ce_s24_443210': {'config.json': '9cf7bc150dfdd992765923e029f511fdb73eddcf5536732aff7e0f208385f830', 'training.json': '5442fe2ddbf93f5ef939dda0c2a3044c998e47d8d35b81a3322ede1b25ff2131', 'gradient_diagnostics.json': '607d9acdae7b09f6dae25a7885eca8a3401c8b873549f573575c798d146462a9'}, 'run_sigmoid_consistency_s24_443207': {'config.json': '593e7f8a843b299a61fbdcbc7ec5d03c20c3b4fc77a463ca666f3ddd38e84aa7', 'training.json': 'aaacece8b6c93d43f3b0d80ad87f7b5629665dedd261cc488f0458c24ec8c720', 'gradient_diagnostics.json': '8b2f1aca9822ccba45c683081e0abf4c53e0b69d7eb94b3fde92368b00b37146'}}

def verify_prior_failed_mains(budget, allowed_completed=()):
    p=pilot();root=p.REPO/'outputs/native_aggregation_vlm/identity_join_optimization_v2'
    prior={name.rsplit('_',1)[1] for name in PRIOR_FAILED_RUNS}
    mains={r['job_id']:r for r in budget['jobs'] if r['name']==p.MAIN_JOB}
    need(set(mains)==prior|set(allowed_completed),'Unexpected prior/current pilot main allocation')
    evidence=[]
    for name,digest in PRIOR_FAILED_RUNS.items():
        directory=root/name;failure=directory/'failure.json';bind(failure,digest);f=read(failure)
        for n,h in PRIOR_FAILED_ARTIFACTS[name].items():bind(directory/n,h)
        need(f['type']=='FileExistsError' and f['message']=="[Errno 17] File exists: '"+str(directory/'training.json')+"'"
             and f['partial_outputs_retained'] is True,'Different preserved V2 failure')
        config=read(directory/'config.json');job=mains[config['slurm_job_id']]
        need(config['source_sha256']==f['source_sha256'] and config['arm']==name[4:].rsplit('_s24_',1)[0]
             and config['seed']==24 and config['profile'] is False,'Prior failed config identity differs')
        need(config['slurm_job_id']==name.rsplit('_',1)[1] and job['state']=='FAILED' and job['exit_code']=='1:0'
             and job['gpus']==1 and job['seconds']=={'443208':49,'443209':50,'443210':50,'443207':48}[job['job_id']],
             'Failed allocation identity/cost differs')
        need(not any((directory/n).exists() for n in ('summary.json','final_endpoint.json','train_final.json','dev_final.json'))
             and not (Path(config['checkpoint_directory'])/'final.pt').exists(),'Failed V2 main reached an endpoint/evaluation')
        need(len(read(directory/'training.json'))==100,'Preserved step100 progress inventory differs')
        for n,h in f['source_sha256'].items():bind(p.REPO/n,h);bind(directory/'source'/n.replace('/','_'),h)
        evidence.append(dict(directory=str(directory),failure_sha256=digest,job_id=job['job_id'],gpu_seconds=job['gpu_seconds']))
    return evidence
...
def release(args,out,frozen):
    ...
    need(len(args.profiles)==4,'Exactly four fixed profiles required');profiles={};records=[];initialized=None;first=None
    ...
    need(set(profiles)==set(p.ARMS),'Four-arm profile matrix incomplete')
    ...
    prior_failures=verify_prior_failed_mains(budget)
    budget.update(reserved_main_gpu_seconds=3360,projected_total_with_main_reservations=budget['allocated_gpu_seconds']+3360)
    passed=seconds<=840 and budget['projected_total_with_main_reservations']<=5500;budget['passed']=passed
    ...
    value=dict(passed=passed,completed=True,protocol='identity_join_optimization_main_release',source_sha256=frozen,
        plan_file=str(Path(args.plan).resolve()),plan_sha256=sha(args.plan),profiles=profiles,profile_audits=records,
        pooled_projection=projection,campaign_budget=budget,per_main_seconds_cap=840,campaign_gpu_seconds_cap=5500,
        no_outcome_dependent_release=True,all_failed_allocations_retained=True,preserved_v2_logging_failures=prior_failures)
...
def report(args,out,frozen):
    ...
    need(len(args.runs)==4,'All four fixed final runs required');records=[];outcomes={};gate_diagnostics={};seen=set();release_binding=None;initial=None;first=None
    for directory in args.runs:
        ...
        for label in ('train','dev'):
            ...
            evaluated[label]=old.audit_eval(torch,summary[label+'_file'],expected,config['condition'],24,tokenizer,
    ...
    prior_failures=verify_prior_failed_mains(budget,[r['job_id'] for r in records])
    ...
    analysis=dict(passed=True,completed=True,protocol=p.PROTOCOL,source_sha256=frozen,plan_file=str(Path(args.plan).resolve()),plan_sha256=sha(args.plan),
        ...
        preserved_v2_logging_failures=prior_failures,all_arms_retained=True,one_fixed_seed=True,no_test_evaluation=True,no_extrapolation_or_reasoning_claim=True)
    save(out/'analysis.json',analysis)
    lines=['# Fixed training-only optimization pilot','','All four fixed seed24 endpoints; no test evaluation or confirmed efficacy.','',
