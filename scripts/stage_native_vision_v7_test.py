"""Stage fresh V7 evaluation only; run generation/rendering/audits on Slurm CPU.
...
DESTINATION = DATA_BASE / 'v7_fresh'
OUTPUT_ROOT = REPO / 'outputs/native_aggregation_vlm/v7/data_staging'
SEED = 20260924
SOURCE_FILES = ('scripts/stage_native_vision_v7_test.py',
...
def load_inventory():
    ...
    inventory = dict(schema_version=1, entries=entries, excluded_content_sha256=ordered,
        ...
        scope='All19 available canonical V1/V2/V3-binding/V4/reasoning/recoverability/V6-fresh/V7-balanced manifests found in the staging inventory',
    ...
    for purpose, (lengths, golds, repeats, axis) in SPEC.items():
        ...
        for gold in golds:
            for replicate in range(repeats):
                label = f'v7_{purpose}_K{gold}_{replicate:03d}'
...
def main():
    ...
    plan = dict(schema_version=1, purpose='v7_fresh_evaluation_only', slurm_job_id=job,
        ...
        for purpose, rows in groups.items():
            ...
            for n in SPEC[purpose][0]:
                ...
                for record, sample in zip(records, selected):
                    record.update(question=sample['question'], origin='generated_v7_fresh', fresh_test_seed=SEED)
            ...
            manifests[purpose] = dict(schema_version=1, purpose=purpose, dataset_root=str(DESTINATION),
                ...
                main_count_content_disjoint=True, evaluation_status='Fresh complete context/question content, fixed before V7 outcomes',
    ...
    (output / 'REPORT.md').write_text('# V7 fresh evaluation staging\n\n'
    ...
    if not index.exists():
        index.write_text('# V7 fresh evaluation staging\n\n')
