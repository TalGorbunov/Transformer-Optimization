"""Deterministic V9 paired schedules from the unchanged balanced V7 scenes.
...
EPOCHS=40
SEEDS=(12,13)
SOURCE='scripts/native_vision_v9_pairs.py'
    ...
    for q in questions:
        for k in range(9):
            ...
            for replica,(a,b) in enumerate(zip(small,large)):
                ...
                pairs.append(dict(slot=len(pairs),pair_id=objsha(['v9_pair',q,k,replica]),question=q,gold=k,
...
def load_pairs(manifest_file=MANIFEST,cache_file=CACHE,schedule_file=SCHEDULE):
    ...
    need((manifest_file,cache_file,schedule_file)==(MANIFEST,CACHE,SCHEDULE),'V9 pairing requires literal unchanged V7 artifacts')
    ...
    return dict(schema_version=1,protocol='v9_equal_question_count_pairing',pairs=pairs,pairs_sha256=objsha(pairs),
...
def self_test():
    ...
    pairs=build_pairs(manifest,cache,schedule,plan)
    a=presentation_order(pairs,12);b=presentation_order(pairs,12)
    need(a==b and a!=presentation_order(pairs,13),'Paired order must be deterministic and seed-dependent')
