"""V5: independent visual memory with extensive versus averaged messages.

Unchanged native VLM forward, cached generation and final-answer/EOS CE. This
copies the frozen V4 training/evaluation procedure and imports its audited data
utilities; V4 itself is never modified. No established V5 result is claimed.
...
SOURCES = (
    "scripts/native_aggregation_vlm_v5.py", "scripts/profile_native_vision_v5_checks.py", "gnnformer/independent_vision_aggregation.py",
    ...
    "slurm/native_aggregation_vision_v5_check.sbatch",
    "slurm/native_aggregation_vision_v5_profile.sbatch",
    "slurm/native_aggregation_vision_v5_main.sbatch",
...
def validate_args(a):
    ...
    if a.arm != "independent" or a.seed not in (4, 5):
        raise SystemExit("V5 fixes independent memory and training seeds4/5")
    ...
    if any(getattr(a, key) != value for key, value in expected.items()):
        raise SystemExit("V5 model/training/decoding differs from registration")
    if a.dataset_root.resolve() != DATASET or a.center_messages:
        raise SystemExit("V5 fixes dataset and internal centered operator, no legacy centering flag")
    if a.max_steps or a.gradient_checkpointing or a.test_epochs or a.eval_only or a.checkpoint:
        raise SystemExit("V5 requires complete training with ordinary non-checkpointed forwards")
    if a.epochs != (2 if a.profile else 9) or a.train_ns != [8,16] or a.dev_ns != [8,16] or a.eval_ns != [16,32,64]:
        raise SystemExit("V5 complete main/profile schedule differs")
    if (a.limit_dev,a.limit_eval,a.limit_count) != ((2,2,2) if a.profile else (36,108,64)):
        raise SystemExit("V5 evaluation cell counts differ")
    ...
    if any(getattr(a,field).resolve() != DATASET / name for field,name in zip(("manifest","count_manifest","schedule"),names)):
        raise SystemExit("V5 requires the exact immutable V4 manifests/schedule")
...
def parser():
    ...
    p.add_argument("--condition",choices=("sum","mean"),required=True)
    ...
    p.set_defaults(model="Qwen/Qwen2.5-VL-7B-Instruct",arm="independent",seed=4,
                   ...
                   data_seed=20260914,eval_modes="all",output=REPO_ROOT/"outputs/native_aggregation_vlm/v5")
    return p
...
def run(a) -> int:
    ...
    if not a.source_ledger or json.loads(a.source_ledger.read_text()) != source_hashes():
        raise ValueError("V5 CPU-frozen source ledger differs")
    ...
    checkpoint_dir = a.checkpoint_root / "native_aggregation_vlm_v5" / run_id
    ...
    config.update(run_id=run_id, slurm_job_id=os.environ["SLURM_JOB_ID"],
                  ...
                  training_schedule_condition="refresh", test_status="exploratory_reused_V2_test",
                  interpretation="SUM=N_visible*MEAN at identical weights/query; tests extensive scaling and optimization, not information capacity")
    ...
    branch = attach_independent_vision_aggregation(
        model, layer_index=layer_index, rank=a.rank, merge=a.condition)
    branch.mode = "all"
    torch.manual_seed(a.seed)
    torch.cuda.manual_seed_all(a.seed)
    ...
    operator = dict(variant="independent_visual_memory", merge=a.condition,
    ...
    architecture = dict(protocol="vision_v5_extensive", model=a.model, arm=a.arm,
                        ...
                        quantization="nf4", operator=operator)
    ...
    diagnostic_dir = DATA_BASE / "v5_diagnostics" / run_id
    ...
    if a.profile:
        from scripts.profile_native_vision_v5_checks import nonzero_cached_and_reset
        profile_checks['nonzero_cache_and_reset'] = nonzero_cached_and_reset(
            model,branch,prepare,[test_sets['length_N16'][0],test_sets['length_N64'][0],
                                 test_sets['length_N16'][1],test_sets['length_N64'][1]])
