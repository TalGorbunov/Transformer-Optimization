"""Independent paired-only, fixed-6000-update factor optimization audit.

Both arms retain the original native cached features, global conditioning and
factor/query/readout architecture. All three endpoints are audited; only the
fixed final endpoint controls the training first-query screen.
"""
...
OWN=('scripts/report_native_identity_join_factor_long.py',)
...
def driver():
    from scripts import diagnose_native_identity_join_factor_long as module
    return module
...
def audit_inputs(torch,plan):
    ...
    rows,scenes,pairs,short_order,states,index,stats=old.audit_inputs(torch,parent)
    need(read(plan['rows_file'])==rows and read(plan['scenes_file'])==scenes and read(plan['pairs_file'])==pairs,
         'Original training scenes, labels or pair identities changed')
    order=read(plan['order_file']);expected=expected_order(pairs)
    need(order==expected and len(order)==48000 and order[:4800]==short_order,'Extended shuffle or original4800 prefix differs')
    for key in ('features_sha256','feature_tensor','stats_sha256','stats_tensors',
                'initial_sha256','initial_state','native_identity','native_identity_sha256','native_module_identity','native_model_sha256',
                'uniform_plan','oracle_report','conditioning_plan','conditioning_report','trainability_mask'):
        need(plan[key]==parent[key],'Original factor/native/statistics input differs: '+key)
    for field,digest_field in (('features_file','features_sha256'),('stats_file','stats_sha256'),('initial_file','initial_sha256'),
                              ('native_model_file','native_model_sha256')):
        bind(plan[field],plan[digest_field]);need(sha(plan[field])==sha(parent[field]),'Consumed original bytes differ: '+field)
    bind(plan['stats_inventory_file'],parent['runtime_bindings'][parent['stats_inventory_file']])
    need(read(plan['stats_inventory_file'])==read(parent['stats_inventory_file']),'Original statistic inventory changed')
    need(read(plan['stats_inventory_file'])==dict(questions=stats['questions'],occurrences=stats['occurrences'],training_sids=[r['sid'] for r in rows],
        original_cached_empty_prefix_only=True,no_fitted_captures=True,no_local_labels=True),'Statistic provenance inventory differs')
    ...
    need(set(packet)=={'seed','branch'} and packet['seed']==24 and {k:p.tensor_info(v) for k,v in initial.items()}==plan['initial_state']
         and bool((initial['up.weight']==0).all()),'Reused original unfitted factor tensor table differs')
    ...
    need(len(counts)==6000 and counts==plan['head_rows_by_update'] and counts[:600]==parent['head_rows_by_update']
         and sum(counts)==plan['training_head_rows']==213330 and sum(counts)+324==plan['total_head_rows']==213654
         and read(plan['head_rows_file'])==dict(by_update=counts,total=sum(counts),maximum=max(counts),endpoint_rows=324,all_head_rows=sum(counts)+324),
         'Full extended training/endpoint row inventory differs')
...
def audit_capture(torch,out,record,arm,plan,rows,scenes,states,index,stats,initial,endpoints,logs,norm,head):
    ...
    if training:
        ...
        need(1<=step<=7 and wp['step']==record['weights_step']==endpoint_step and all(torch.equal(w[k],endpoints[endpoint_step][k]) for k in initial), 'Final capture does not use fixed endpoint')
...
def training_loss_descriptions(logs,scenes):
    ...
    return dict(descriptive_only=True,token_roles=metrics(roles),cycles=[dict(cycle=k,pair_presentations=v['pair_presentations'],
        ...
        for k,v in sorted(cycles.items())],no_checkpoint_selection=True,horizon_and_annealing_both_changed=True,
...
def self_test(torch):
    ...
    need(order[:4800]==old.expected_order(pairs) and counts[889]==48 and all(counts[i]==54 for i in range(1,889)),
         'Persistent shuffled-order/original-prefix fixture failed')
    need(close(learning_rate(1),.00002,atol=1e-15,rtol=1e-15) and learning_rate(50)==.001
         and learning_rate(6000)==.00001 and learning_rate(600)>.0009,'Warmup/cosine horizon fixture failed')
    ...
    need(not primary_screen({600:rows,2000:rows,6000:failed})['passed']
         and primary_screen({600:failed,2000:failed,6000:rows})['passed'],'Earlier endpoints must not select acceptance')
    ...
    return dict(passed=True,groups=5,persistent_order_original_prefix=True,extended_cosine_schedule=True,
...
def audit_run(torch,out,directory,plan_path,plan,frozen,rows,scenes,order,states,index,stats,initial,norm,head):
    ...
    for key in ('native_identity','native_identity_sha256','native_module_identity','native_model_file','native_model_sha256',
                ...
                'uniform_plan','oracle_report','conditioning_plan','conditioning_report','factor_plan','long_oracle_report','long_oracle_plan',
    ...
    counters=dict(core=6021,conditioning=6021,norm=6021,head=6021,vlm=0,vision=0);calls=read(summary['calls_file'])
    ...
    need([(r['phase'],r['step'],r['endpoint_step']) for r in records]==expected and all(r['arm']==arm and r['pairing']=='paired' for r in records),'Chronological29-capture inventory differs')
...
def resources(out,runs):
    ...
    for row in records:
        ...
        need(row['job_id']==runs[arm]['slurm_job_id'] and row['partition']=='gpu' and row['state']=='COMPLETED'
             and row['exit_code']=='0:0' and row['gpus']==1 and row['seconds']<=240,'Factor-binding allocation identity/cap differs')
    ...
    total=sum(r['gpu_seconds'] for r in records);need(current==0 and peak<=2 and total<=480,'Factor-binding campaign cap exceeded')
...
def report(args,out,frozen):
    ...
    need(frozen==plan['source_sha256']==p.sources() and len(frozen)==6 and len(args.runs)==2
         and plan['inherited_source_sha256']==p.inherited_sources() and len(plan['inherited_source_sha256'])==99,
         'New/inherited source or two-run inventory differs')
    inherited=dict(plan_file=plan['factor_plan']['file'],plan_sha256=plan['factor_plan']['sha256'],source_sha256=plan['inherited_source_sha256'])
    ...
    need(plan['decision_step']==6000 and plan['evaluation_steps']==list(ENDPOINTS) and plan['original_order_prefix_equal'] is True
         ...
         and plan['no_new_statistics_or_features'] is True and plan['policy']['paired_only'] is True,'Registered horizon/architecture scope differs')
    trigger=plan['long_oracle_report'];bind(trigger['file'],trigger['sha256']);proof=read(trigger['file'])
    bind(trigger['analysis_file'],trigger['analysis_sha256']);evidence=read(trigger['analysis_file'])
    need(proof['passed'] is proof['completed'] is evidence['passed'] is evidence['completed'] is True
         and proof['phase']==evidence['phase']=='report' and proof['analysis_file']==trigger['analysis_file']
         and proof['analysis_sha256']==trigger['analysis_sha256'] and proof['first_token_screen']==evidence['first_token_screen']
         and proof['first_token_screen']['joint_code']['passed'] is True,'Bound completed long-oracle calibration evidence differs')
    bind(plan['long_oracle_plan']['file'],plan['long_oracle_plan']['sha256']);oracle_plan=read(plan['long_oracle_plan']['file'])
    need(evidence['plan_file']==plan['long_oracle_plan']['file'] and evidence['plan_sha256']==plan['long_oracle_plan']['sha256'],
         'Evidence must bind its exact CPU preparation')
    rows,scenes,order,states,index,stats,initial=audit_inputs(torch,plan)
    need(order==read(oracle_plan['order_file']) and plan['head_rows_by_update']==oracle_plan['head_rows_by_update'],
         'Long calibration must retain the already fixed extended schedule')
    save(out/'input_reconstruction_audit.json',dict(passed=True,factor_plan=plan['factor_plan'],contexts=108,local_occurrences=1296,
        pair_presentations=48000,original4800_prefix_exact=True,training_target_positions=213330,endpoint_rows_per_arm=324,
        original_cached_native_features_exact=True,global_conditioning_independently_reconstructed=True,
        original_shared_unfitted_factor_bytes_exact=True,consumed_input_bytes_unchanged=True,no_new_data_or_representation=True))
    ...
    need(set(runs)==set(ARMS) and before==dict(norm=p.tensor_info(norm.weight),head=p.tensor_info(head.weight))
         ...
         'Two-arm fixed native audit incomplete')
    need(all(r['cpu_head_calls']==r['cpu_norm_calls']==21 and r['cpu_head_rows']==324 and len(r['capture_audits'])==29 for r in runs.values()),
         'Both arms require29 captures/21 CPU head batches/324 rows')
    budget=resources(out,runs);comparisons=[]
    for e in ENDPOINTS:
        outcomes={arm:read(runs[arm]['endpoint_audits'][str(e)]['outcomes_file']) for arm in ARMS}
        need([r['sid'] for r in outcomes['product']]==[r['sid'] for r in outcomes['additive']],'Matched endpoint scene order differs')
        transitions=Counter((a['first_token_correct'],b['first_token_correct']) for a,b in zip(outcomes['additive'],outcomes['product']))
        comparisons.append(dict(endpoint_step=e,contexts=108,all_comparisons_descriptive=True,
            ...
            mean_first_token_nll={arm:runs[arm]['endpoint_audits'][str(e)]['mean_first_token_nll'] for arm in ARMS}))
    result=dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        plan_file=str(Path(args.plan).resolve()),plan_sha256=sha(args.plan),factor_plan=plan['factor_plan'],long_oracle_report=trigger,long_oracle_plan=plan['long_oracle_plan'],
        stats_file=plan['stats_file'],stats_sha256=plan['stats_sha256'],stats_tensors=plan['stats_tensors'],runs=runs,comparisons=comparisons,resources=budget,
        primary_endpoint_step=6000,first_token_screen={arm:runs[arm]['first_token_fit'] for arm in ARMS},
        cpu_head_calls=42,cpu_norm_calls=42,cpu_head_rows=648,gpu_head_calls=12042,gpu_norm_calls=12042,gpu_head_rows=427308,vlm_calls=0,vision_calls=0,
        paired_only=True,no_new_permutation=True,horizon_and_annealing_both_changed=True,earlier_endpoints_descriptive_only=True,
    ...
    lines=['# Extended optimization of the locked factor pair','',
        'Independent computation and provenance audits passed. Both original interactions used the exact shared unfitted factor tensors, native cached features and global conditioning statistics. '
        'The query remains in both factor preactivations and in the readout. Both optimization and cosine annealing horizons changed from 600 to 6,000 updates.','',
        '| Interaction | Endpoint | Correct /108 | Complete families /18 | Mean first-token NLL | Decision role |',
        '|---|---:|---:|---:|---:|---|']
    for arm in ARMS:
        for e in ENDPOINTS:
            value=runs[arm]['endpoint_audits'][str(e)];c=value['first_token_fit'];role='Descriptive only' if e!=6000 else ('PASS' if c['passed'] else 'FAIL')
            lines.append(f"| {arm} | {e} | {c['correct']} | {c['complete_families']} | {value['mean_first_token_nll']:.9f} | {role} |")
    lines+=['','Only each arm\'s fixed step-6,000 endpoint controls its unchanged 103/108 and 16/18-family screen. All six endpoint archives and all per-name/question/length/family strata remain available. '
        'Intermediate evaluations preserved optimizer state, parameter objects/versions, native weights, cached features, conditioning statistics and training mode. '
        'No score selected a checkpoint, restart or schedule change. No cyclic factor intervention was run in this calibration.','',
        'These are cached training first queries. A pass calibrates trainability of that fixed interaction under the expanded horizon/schedule; a failure remains a bounded incomplete fit. '
        'Neither outcome establishes generalization, native whole-answer performance, semantic factor identities, a general bandwidth advantage or reasoning benefit. '
        'Product/additive comparisons are descriptive, with one shared seed and the same data and parameter counts. Prior short-run results remain unchanged.','',
        'Full-name-plus-EOS CE retains the original mean-scene weighting and all 48,000 pair presentations per arm. Loss descriptions separate first-token, continuation and EOS losses in every complete presentation cycle and the partial tail. '
        'Those curves are not convergence certificates. Any later native or fresh-data validation requires a separate prospective release.','',
        f"Allocated GPU cost: {budget['allocated_gpu_seconds']} seconds. Each arm used 6,021 core/conditioning/norm/head calls and 213,654 head rows; the combined total is 12,042 calls per module and 427,308 head rows. "
        'VLM and vision calls were zero. Independent CPU audit covered 58 captured computations and 42 head batches / 648 rows, with fixed TV <= 0.02 and exact argmax. '
    ...
    return dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        first_token_screen=result['first_token_screen'],resources=budget,paired_only=True,cached_first_query_only=True,no_native_or_whole_answer_claim=True)
