"""V13 learned-null native software profile; no training or efficacy scoring.

Fixed-prefix structural audits copy complete states/KV. Natural timings use a
separate minimal observer: full-batch LAST-query fused hidden plus global raw
logits only, no complete hidden or KV copies. Every actual model invocation has
...
OUT=REPO/'outputs/native_aggregation_vlm/v13/null_software'
DATA=Path('/mnt/data/gabriele/gnn_transformer/v13_null_software')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/v13_null_software')
ANCESTOR=REPO/'outputs/native_aggregation_vlm/v7/runtime/profile_441845/summary.json'
FORCED=('Therefore',':');MODES=('offset','centered');PREDICTOR_SEED=20261104
...
OWN=tuple(dict.fromkeys(('scripts/profile_native_vision_v13_null.py','scripts/native_vision_v13_runtime.py',
    'gnnformer/conditional_null_mean.py','tests/test_conditional_null_mean.py','gnnformer/parallel_local_learned_null.py',
    'slurm/native_vision_v13_null_check.sbatch','slurm/native_vision_v13_null_profile.sbatch',*ancestor.OWN)))
...
def snapshot(out):
    ...
    (out/'INDEX.md').write_text('# V13 learned-null software\n\n[Summary](summary.json) · [Plan](plan.json) · [Sources](source_hashes.json).\n')
    index=OUT/'INDEX.md'
    if not index.exists():index.write_text('# V13 learned-null software\n\n')
    with index.open('a') as stream:stream.write(f'- [{out.name}]({out.name}/INDEX.md)\n')
    return frozen


def predictor_state(torch):
    from gnnformer.conditional_null_mean import ConditionalNullMean
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(PREDICTOR_SEED);module=ConditionalNullMean()
        with torch.no_grad():module.fc2.weight.normal_(0,.001)
        value={k:v.detach().clone() for k,v in module.state_dict().items()}
    need(bool(value['fc2.weight'].ne(0).any()) and not bool(value['fc2.bias'].ne(0).any()),'Fixed software predictor differs')
    return value


def controller_tests(torch):
    ...
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(20261104);core=ParallelLocalAggregation(8,rank=4);predictor=ConditionalNullMean(4)
        norm=torch.nn.Identity();h=torch.randn(4,5,8)
        with torch.no_grad():core.up.weight.normal_(0,.1);predictor.fc2.weight.normal_(0,.001)
        original=h.clone();outputs=[]
        for mode in MODES:
            control=ParallelLocalLearnedNull(norm,core,predictor,n_local_rows=3,mode=mode,
                query_indices=[3,4],stream_positions=[3,4],capture=True,detach_captures=False)
            with control:
                out=norm(h);c=control.last_capture
                need(torch.equal(out[:-1],h[:-1]) and torch.equal(out[-1,:3],h[-1,:3]) and torch.equal(h,original),
                     'Controller mutated a native input/local/non-query row')
                q=core.query(core.rms(h[-1,3:]));mean=predictor(q)
                expected=projected_null_readout(core,core.aggregate(core.encode(h[:-1,3:],h[-1,3:])),h[-1,3:],mean,
                    n_elements=3,mode=mode)
                need(torch.equal(out[-1,3:],h[-1,3:]+expected['delta']),'Controller projection differs')
                grad=torch.autograd.grad(out[-1].square().sum(),(core.local.weight,core.query.weight,predictor.fc2.weight),retain_graph=True)
                need(all(bool(torch.isfinite(g).all()) and bool(g.ne(0).any()) for g in grad),'Main loss gradient was detached')
                control.configure_queries([0],[4]);decoded=norm(h[:,4:])
                torch.testing.assert_close(decoded[:,-1],out[:,-1],atol=1e-6,rtol=1e-6)
                outputs.append(out.detach())
            need(not control.active and not norm._forward_pre_hooks,'Controller hook leaked')
        need(not torch.equal(outputs[0][-1],outputs[1][-1]),'Active deployed modes unexpectedly identical')
        try:
            with ParallelLocalLearnedNull(norm,core,predictor,n_local_rows=3,mode='centered',query_indices=[4],stream_positions=[4]):
                raise RuntimeError('fixture')
        except RuntimeError:pass
        need(not norm._forward_pre_hooks,'Exception cleanup failed')
    return dict(passed=True,checks=['actual_and_history_unchanged','reference_formula','live_main_gradients',
        'cached_physical_query_mapping','deployed_modes_distinct','exception_cleanup'])
...
def check():
    ...
    command=[sys.executable,'tests/test_conditional_null_mean.py']
    ...
    need(unit.returncode==0,'Conditional-mean unit tests failed; logs retained')
    hooks=controller_tests(torch)
    ...
    for sample in scenes:
        base=runtime.prepare_scene(processor,sample,verify_processor_parity=True);case_id=f'parallel_N{sample["n_frames"]}';variants=[]
        for step in range(3):
            bundle=old.prefixed_bundle(base,forced[:step]);runtime.validate_bundle(bundle);layout=native.audit_layout(rope,bundle)
        ...
        cases.append(dict(case_id=case_id,sid=sample['sid'],n_frames=sample['n_frames'],question=sample['question'],
            prompt_width=base['metadata']['original_prompt_width'],sample=sample,metadata=base['metadata'],variants=variants))
    ...
    plan=dict(schema_version=1,protocol='v13_learned_null_software_only',source_sha256=frozen,
        ...
        parameters=dict(core=1041600,predictor=18624,total=1060224),no_reference_streams=True,
        natural_timing_observer='full-batch last-query fused h plus global logits; no full hidden or KV copying',
...
def verify(path):
    ...
    need(plan['schema_version']==1 and plan['protocol']=='v13_learned_null_software_only' and plan['source_sha256']==sources()
    ...
    for name in ('ancestor','parent_plan','initial','prepared'):need(sha(plan[name+'_file'])==plan[name+'_sha256'],'Bound artifact changed')
    need(sha(plan['source_manifest']['path'])==plan['source_manifest']['sha256'],'Original software data manifest changed')
    need([c['n_frames'] for c in plan['cases']]==[16,64] and plan['no_reference_streams'],'Software rows differ')
    return plan
...
def verify_fusion(raw,controller,query,stream):
    ...
    need(c is not None and c['query_indices']==query and c['stream_positions']==stream
        and c['n_local_rows']==n and c['mode']==controller.mode,'Explicit query or local-row partition differs')
    need(torch.equal(c['local_states'],h[:-1,query]) and torch.equal(c['global_states'],h[-1,query]),'Read states differ')
    expected=h.clone();expected[-1,query]=h[-1,query]+c['delta'].half()
    need(c['delta'].dtype==torch.float32 and torch.equal(expected,raw['fused_norm_input'])
        and torch.equal(expected[-1,query],c['fused_global']),'Only declared native global positions may change')
    coefficient=n if controller.mode=='centered' else 1
    need(c['coefficient']==coefficient and torch.equal(c['corrected_preactivation'],c['preactivation']-coefficient*c['projected_null']),
        'Deployed projected correction differs')
    return dict(passed=True,query_indices=query,stream_positions=stream,mode=controller.mode,n_frames=n,
        coefficient=coefficient,delta_nonzero=bool(c['delta'].ne(0).any()),local_rows_untouched=True)


class MinimalTimingAudit:
    """No native KV snapshot or full-hidden copy during natural timing."""
    def __init__(self,model,data):
        self.model,self.data=model,data;self.handles=[];self.per_call=[];self.records=[];self.context={}
        self.counts=dict(model=0,visual=0,language=0,norm=0,last_block=0)
    def __enter__(self):
        from gnnformer.runtime import get_layers
        def increment(name):
            def call(*_):self.counts[name]+=1
            return call
        self.handles=[self.model.register_forward_pre_hook(self.before,with_kwargs=True),self.model.register_forward_hook(self.after),
            ...
            self.model.model.language_model.register_forward_pre_hook(increment('language')),
            get_layers(self.model)[-1].register_forward_pre_hook(increment('last_block'))]
        return self
    def __exit__(self,*_):
        for h in self.per_call+self.handles:h.remove()
        return False
    def before(self,module,args,kwargs):
        self.counts['model']+=1;self.query_shape=list(kwargs['input_ids'].shape)
        self.visual=kwargs.get('pixel_values') is not None
        # Registered here after the runtime's controller: observe the fused h.
        self.per_call=[self.model.model.language_model.norm.register_forward_pre_hook(self.norm)]
    def norm(self,module,args):
        self.counts['norm']+=1
        self.fused=cpu_copy(args[0][:,-1:,:])
        need(self.fused.shape[1]==1 and self.fused.dtype.__str__()=='torch.float16','Minimal capture shape/dtype differs')
    def after(self,module,args,output):
        import torch
        for h in self.per_call:h.remove()
        self.per_call=[];torch.cuda.synchronize()
        raw=dict(fused_query_hidden=self.fused,native_global_logits=cpu_copy(output.logits[-1,-1]),context=dict(self.context))
        need(raw['native_global_logits'].dtype==torch.float16 and bool(torch.isfinite(raw['native_global_logits']).all()),'Native raw logits differ')
        label=self.context['label']+f'__t{len(self.records):03d}';path=self.data/(label+'.pt');torch.save(raw,path)
        self.records.append(dict(label=label,path=str(path),sha256=sha(path),capture_kind='last_query_only',
            context=dict(self.context),model=1,visual=int(self.visual),input_shape=self.query_shape,
            retained_hidden_shape=list(self.fused.shape),kv_snapshots=0,full_hidden_copies=0))


def replay_heads(torch,model,records):
    metrics=[];raw_metrics=[]
    for row in records:
        raw=torch.load(row['path'],map_location='cpu',weights_only=True)
        if row.get('capture_kind')=='last_query_only':hidden=raw['fused_query_hidden'];expected=raw['native_global_logits']
        else:hidden=raw['fused_norm_input'][:,-1:,:];expected=raw['native_logits'][-1,-1]
        with torch.no_grad():value=model.lm_head.forward(model.model.language_model.norm.forward(hidden.to(model.device)))
        actual=value[-1,-1].detach().cpu();metric=old.metric(torch,actual,expected)
        metric.update(label=row['label'],binding=True,batch_shape=list(hidden.shape));metrics.append(metric)
        raw_metrics.append(dict(label=row['label'],replayed_global=actual,native_global=expected))
...
def run(args):
    ...
    (data/'INDEX.md').write_text('# V13 software raw evidence\n\n[Head replays](head_replays.pt).\n')
    ...
    fixed_counts={};natural_counts={};all_records=[]
    try:
        with ancestor.NativeAudit(model,data) as audit:
            try:
                for case in plan['cases']:
                    ...
                    del cache,output
                    for condition,mode in [('zero','centered'),('active','offset'),('active','centered')]:
                        ...
                        cache=None
                        controller=ParallelLocalLearnedNull(norm,core,predictor,n_local_rows=n,mode=mode,
                            ...
                            for step,bundle in enumerate(bundles):
                                ...
                                if condition=='zero':
                                    ...
                                    if step>0:
                                        ...
                                        if mode=='offset':native_full[step]=full['native_norm_input']
        ...
        need(len(zero)==6 and len(checks)==26 and len(controller_counts)==6 and len(comparisons)==328
        ...
        with MinimalTimingAudit(model,data) as timing_audit:
            try:
                for case in plan['cases']:
                    for mode in MODES:
                        tick=time.perf_counter();bundle=runtime.prepare_scene(loaded.processor,case['sample'])
                        ...
                        generated=runtime.generate_native(model,loaded.processor,core,predictor,bundle,mode=mode,capture=False)
                        ...
                        need(len(records)==steps and 1<=steps<=4 and generated['captures']==[],
                            'Natural timing retained branch captures or changed call coverage')
                        ...
                        timings.append(dict(sid=case['sid'],n_frames=case['n_frames'],mode=mode,generated_tokens=steps,
                            ...
                            counters=generated['counters'],no_efficacy_scoring=True,no_full_hidden_or_kv_copies=True))
            finally:
    ...
    timing=dict(rows=timings,by_mode={mode:{str(t['n_frames']):t['four_token_seconds_bound'] for t in timings if t['mode']==mode} for mode in MODES},
        ...
        observer='only full-batch last-query fused h and global native logits; no full hidden/KV copies',maximum_new_tokens=4)
    ...
    summary=dict(schema_version=1,protocol=plan['protocol'],completed=True,passed=not failures,computational_integrity_passed=True,
        ...
        extra_last_block_forwards=0,fixed_prefix_kv_checks=18,fixed_prefix_fusion_checks=26,zero_identity_checks=6,cached_full_row_comparisons=328,
...
def verify_profile(path):
    ...
    need(path.is_relative_to(OUT) and path.name=='summary.json' and summary.get('schema_version')==1
        and summary.get('protocol')=='v13_learned_null_software_only' and all(summary.get(k) is True for k in
        ('completed','passed','computational_integrity_passed','zero_identity_passed','fixed_prefix_native_kv_unchanged',
         ...
         'predictor_unfitted_unchanged','no_fit','no_efficacy_scoring')),'Require completed successful native V13 profile')
    ...
    for row in rows:
        ...
        need(row['sid']==case['sid'] and 1<=row['generated_tokens']<=4 and row['preparation_seconds']>0 and row['generation_seconds']>0
            ...
            and row['no_full_hidden_or_kv_copies'] and sha(row['raw_file'])==row['raw_sha256'],'Natural timing binding differs')
    ...
    need(total<=48 and summary['calls']==dict(model=total,visual=20,language=total,norm=total,last_block=total)
        ...
        and summary['cached_full_row_comparisons']==328 and not summary['numerical_failures'],'Software call/structural coverage differs')
