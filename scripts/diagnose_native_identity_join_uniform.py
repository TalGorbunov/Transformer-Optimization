"""Conditional fixed-uniform native identity-join control.

Frozen sigmoid selector0/.5; native CE only. Fixed seed24,
...
One32-update profile precedes independent CPU resource release. No fitted-parent
...
from scripts import diagnose_native_identity_join_optimization_v3 as pilot
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_uniform'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_uniform')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_uniform')
PARENT=REPO/'outputs/native_aggregation_vlm/identity_join_optimization_v3/check_443214/plan.json'
PARENT_SHA='0a3cb1b364e69e8cfaf534eaa5118508d7353610e3c9cf01aba0cbb9e4e28326'
CONDITIONAL=REPO/'outputs/native_aggregation_vlm/identity_join_optimization_v3/report_443233/summary.json'
CONDITIONAL_SHA='6c0232cd70908d13872d8b59f22cae3ab1ef7e4ca79b5373e6c030f39d12dcb6'
CONDITIONAL_ANALYSIS_SHA='999028973907dab197f46ca5177ecddf89849b33c740bf7998fbaa8971a22fc9'
FROZEN_SELECTOR=('selection_weight','selection_bias')
ARMS=('uniform_half_ce',)
PROTOCOL='identity_join_fixed_uniform_control'
PROFILE_JOB='identity_join_uniform_profile';MAIN_JOB='identity_join_uniform_main'
POLICY=dict(protocol=PROTOCOL,arms=list(ARMS),seed=24,steps=600,profile_steps=32,pairs=54,training_contexts=108,
    dev_contexts=54,families=18,batch_pairs=8,batch_scenes=16,parameters=1041697,effective_trainable_parameters=1041600,frozen_selector_coordinates=97,rank=96,
    ...
    coefficients=[0.],epsilon=1e-6,all_native_name_tokens_and_eos=True,maximum_new_tokens=4,
    ...
    main_generations=162,main_max_model_calls=648,main_vision_calls=162,profile_seconds=150,main_seconds=840,
    campaign_gpu_seconds=990,maximum_project_gpus=1,final_checkpoint_only=True,dev_evaluated_once=True,
...
OWN=('scripts/diagnose_native_identity_join_uniform.py','scripts/report_native_identity_join_uniform.py',
     ...
     'slurm/native_identity_join_uniform_check.sbatch','slurm/native_identity_join_uniform_profile.sbatch',
     'slurm/native_identity_join_uniform_main.sbatch','slurm/native_identity_join_uniform_report.sbatch')


def sources():return {**pilot.sources(),**{name:sha(REPO/name) for name in OWN}}
...
def snapshot(out):
    ...
    (out/'INDEX.md').write_text('# Fixed-uniform training-only join control\n\n[Summary](summary.json) · [Sources](source_hashes.json). No test evaluation or efficacy confirmation.\n')
    return frozen


def arm_config(arm):
    need(arm=='uniform_half_ce','Unknown fixed-uniform arm');return 'sigmoid',0.
...
def parse_resources(raw):
    ...
    for line in raw.splitlines():
        ...
        gpus=int(values['gres/gpu']) if 'gres/gpu' in values else sum(typed);seconds=int(seconds);cap=150 if name==PROFILE_JOB else 840
    ...
    need(now==0 and peak<=1 and total<=990,'Pilot cumulative budget/concurrency exceeded')
...
def save_progress(out,step,training,diagnostics):
    paths=(Path(out)/f'training_step_{step:04d}.json',Path(out)/f'gradient_diagnostics_step_{step:04d}.json')
    for path,value in zip(paths,(training,diagnostics)):save(path,value)
    return paths


def progress_io_self_test():
    """Exercise the real exclusive writer through every progress and final save."""
    import tempfile
    training=[dict(step=i) for i in range(1,601)]
    diagnostics=[dict(step=i) for i in POLICY['gradient_steps']]
    with tempfile.TemporaryDirectory(prefix='join_optimization_progress_') as directory:
        out=Path(directory);paths=[]
        for step in range(100,601,100):
            values=(training[:step],[r for r in diagnostics if r['step']<=step])
            written=save_progress(out,step,*values);paths.extend(written)
            need(all(read(path)==value for path,value in zip(written,values)),'Exclusive progress content differs')
        for name,value in (('training.json',training),('gradient_diagnostics.json',diagnostics)):
            path=out/name;save(path,value);paths.append(path)
            need(read(path)==value,'Exclusive final content differs')
        need(len(paths)==len(set(paths))==len(list(out.iterdir()))==14,'Progress/final filenames collide')
        previous=sha(out/'training.json')
        try:save(out/'training.json',[])
        except FileExistsError:pass
        else:raise ValueError('Exclusive writer unexpectedly permitted overwrite')
        need(sha(out/'training.json')==previous,'Rejected overwrite changed final evidence')
    return dict(passed=True,progress_steps=list(range(100,601,100)),unique_files=14,exclusive_overwrite_rejected=True)
...
def self_test():
    ...
    return dict(passed=True,schedule_threshold_resource_checks=True,progress_io=progress_io_self_test(),no_model_loaded=True)


def conditional_gate(bindings):
    bind(PARENT,bindings,PARENT_SHA);bind(PARENT.parent/'summary.json',bindings)
    parent=pilot.verify_plan(PARENT,ancestors=True)
    bind(CONDITIONAL,bindings,CONDITIONAL_SHA);summary=read(CONDITIONAL)
    need(summary['passed'] is True and summary['completed'] is True and summary['phase']=='report','Completed independent pilot report required')
    bind(summary['analysis_file'],bindings,CONDITIONAL_ANALYSIS_SHA)
    need(summary['analysis_sha256']==CONDITIONAL_ANALYSIS_SHA,'Conditional analysis identity differs')
    analysis=read(summary['analysis_file']);need(analysis['passed'] and analysis['completed']
        and analysis['plan_sha256']==PARENT_SHA and analysis['source_sha256']==pilot.sources()
        and len(analysis['runs'])==4 and {r['arm'] for r in analysis['runs']}==set(pilot.ARMS),
        'Conditional four-arm pilot/source coverage differs')
    rows=[];reference=None
    for row in analysis['runs']:
        path=Path(row['directory'])/'summary.json';bind(path,bindings,row['summary_sha256']);result=read(path)
        need(result['passed'] and result['completed'] and result['computational_integrity_passed'] and result['steps']==600
            and result['profile'] is False and result['seed']==24 and result['arm']==row['arm']
            and result['source_sha256']==pilot.sources() and result['trainability']==row['trainability']
            and row['trainability']['passed'] is False,'Every original arm must be a valid completed training-fit failure')
        rows.append(dict(arm=row['arm'],summary_file=str(path),summary_sha256=sha(path),trainability=row['trainability']))
        if row['arm']=='clip_ce':
            bind(result['first_gradients_file'],bindings,result['first_gradients_sha256'])
            reference=dict(file=result['first_gradients_file'],sha256=result['first_gradients_sha256'],arm='clip_ce')
    need(reference is not None,'Missing fixed common-gradient comparator')
    return parent,dict(summary_file=str(CONDITIONAL),summary_sha256=CONDITIONAL_SHA,
        analysis_file=summary['analysis_file'],analysis_sha256=CONDITIONAL_ANALYSIS_SHA,
        all_four_valid_training_fit_failures=True,runs=rows),reference


def check(out,frozen):
    import torch
    from gnnformer.parallel_local_learned_selection import ParallelLocalLearnedSelection
    torch.set_num_threads(4);bindings={};parent,conditional,reference=conditional_gate(bindings)
    tests=dict(self_test(),tensor_algebra=tensor_self_test(torch),uniform_algebra=uniform_self_test(torch))
    with torch.random.fork_rng(devices=[]):torch.manual_seed(24);core=ParallelLocalLearnedSelection(mode='sigmoid')
    initial=torch.load(parent['initial_file'],map_location='cpu',weights_only=True)
    need(initial['seed']==24 and base.v7.state_info(core)==parent['initial_state'],'Exact original initial state differs')
    core.load_state_dict(initial['branch']);mask=freeze_selector(torch,core)
    need(base.v7.state_info(core)==parent['initial_state'],'Freezing selector changed initial tensor bytes')
    copied=dict(parent)
    copied.update(protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,
        artifact_bindings={**parent['artifact_bindings'],**bindings},
        runtime_bindings={**parent['runtime_bindings'],**bindings},parent_plan=dict(file=str(PARENT),sha256=PARENT_SHA),
        conditional_report=conditional,reference_first_gradients=reference,trainability_mask=mask,**mask,
        selector_initial_state={n:parent['initial_state'][n] for n in FROZEN_SELECTOR},
        tests=tests,no_model_loaded=True,reused_prepared_contexts=162,no_render_or_repreparation=True)
    save(out/'plan.json',copied);(out/'plan.sha256').write_text(sha(out/'plan.json')+'\n')
    return dict(passed=True,completed=True,phase='check',protocol=PROTOCOL,source_sha256=frozen,
        plan_file=str(out/'plan.json'),plan_sha256=sha(out/'plan.json'),subset=parent['subset'],tests=tests,
        trainability_mask=mask,conditional_report=conditional,no_model_loaded=True,
        reused_prepared_contexts=162,no_render_or_repreparation=True)
...
def gradient_geometry(torch,core,ce,residual,coefficient,cap):
    ...
    names,parameters=zip(*((n,p) for n,p in core.named_parameters() if p.requires_grad))
...
def verify_release(path,plan_path,plan):
    ...
    need(release['protocol']=='identity_join_uniform_main_release' and release['passed'] is True
         and release['completed'] is True and release['plan_sha256']==sha(plan_path)
         and release['source_sha256']==sources() and release['per_main_seconds_cap']==840
         and release['campaign_gpu_seconds_cap']==990 and set(release['profiles'])==set(ARMS)
         and release['pooled_projection']['passed'] and 0<release['pooled_projection']['projected_seconds']<=840
    ...
    for arm,record in release['profiles'].items():
        ...
        need(result['passed'] and result['completed'] and result['profile'] and result['arm']==arm
             and result['condition']==arm and result['selection_mode']==mode and result['consistency_coefficient']==coef and result['seed']==24
             ...
             and result['no_dev_or_test_evaluation'],'Released uniform profile is invalid')
...
def run(args,out,frozen,started):
    ...
    mask=freeze_selector(torch,core);need(mask==plan['trainability_mask'],'Uniform trainable mask differs')
    ...
    config=dict(protocol=PROTOCOL,run_id=out.name,arm=args.arm,condition=args.arm,selection_mode=mode,
        **mask,selector_initial_state={n:base.v7.tensor_info(p) for n,p in core.named_parameters() if n in FROZEN_SELECTOR},
        conditional_report=plan['conditional_report'],reference_first_gradients=plan['reference_first_gradients'],
    ...
    optimizer=torch.optim.AdamW(trainable,lr=.001,weight_decay=0.)
    setup=time.perf_counter()-started;steps=32 if args.profile else 600;log=[];diagnostics=[];first_gradients=[];io_seconds=0.
    exercised={n:False for n,p in core.named_parameters() if p.requires_grad};zero=None
    for step in range(1,steps+1):
        ...
        check_selector(torch,core)
        need(torch.equal(cap['gates'],valid.float()*.5) and bool((cap['scores']==.5).all()),'Uniform gate must remain exactly .5 on every actual item')
        if step==1:zero=uniform_zero_gradients(torch,core,residual)
        ...
        total.backward();need(all(p.grad is not None and bool(torch.isfinite(p.grad).all()) for p in trainable),
            'Missing/nonfinite uniform trainable gradient')
        for name,p in core.named_parameters():
            if p.requires_grad:exercised[name]|=bool((p.grad!=0).any())
        if step<=2:first_gradients.append(dict(step=step,pre_clip={n:base.v7.tensor_info(p.grad) for n,p in core.named_parameters() if p.requires_grad}))
        gradient_norm=torch.nn.utils.clip_grad_norm_(trainable,1.);need(bool(torch.isfinite(gradient_norm)),'Nonfinite clipping norm')
        optimizer.step();check_selector(torch,core);need(all(bool(torch.isfinite(p).all()) for p in core.parameters())
        ...
        log.append(dict(step=step,arm=args.arm,condition=args.arm,lr=rate,loss=float(total),ce_loss=float(ce),consistency_loss=float(residual),
            ...
            query_diagnostics=queries,frozen_selector_exact=True,**parts))
        if step%100==0:
            tick=time.perf_counter();save_progress(out,step,log,diagnostics)
    ...
    check_selector(torch,core);core.eval().requires_grad_(False);final=base.v7.state_info(core);checkpoint=ckpt/('profile.pt' if args.profile else 'final.pt')
    ...
    check_selector(torch,core);after=base.v7.state_info(core);after_weights=dict(norm=base.v7.tensor_info(norm.weight),head=base.v7.tensor_info(head.weight))
...
def check_selector(torch,core):
    need(core.mode=='sigmoid' and core.selection_weight.dtype==core.selection_bias.dtype==torch.float32
         and bool((core.selection_weight==0).all()) and bool((core.selection_bias==.5).all())
         and not core.selection_weight.requires_grad and not core.selection_bias.requires_grad
         and core.selection_weight.grad is None and core.selection_bias.grad is None,
         'Fixed uniform selector bytes, mode, or gradient mask changed')


def freeze_selector(torch,core):
    need(core.mode=='sigmoid' and bool((core.selection_weight==0).all()) and bool((core.selection_bias==.5).all()),
         'Only the exact original initial sigmoid selector may be frozen')
    for name,p in core.named_parameters():
        need(p.grad is None,'Freeze selector before gradient computation')
        p.requires_grad_(name not in FROZEN_SELECTOR)
    check_selector(torch,core)
    result=dict(trainable_parameter_names=[n for n,p in core.named_parameters() if p.requires_grad],
        frozen_selector_parameter_names=list(FROZEN_SELECTOR),
    ...
    need(result['effective_trainable_parameters']==1041600 and result['retained_state_parameters']==1041697,
         'Uniform effective trainable/state parameter counts differ')
    return result


def uniform_zero_gradients(torch,core,residual):
    check_selector(torch,core);parameters=tuple(p for p in core.parameters() if p.requires_grad)
    need(bool((core.up.weight==0).all()) and float(residual)==0.,'Initial U or residual is nonzero')
    gradients=torch.autograd.grad(residual,parameters,retain_graph=True,allow_unused=True)
    need(all(v is None or bool(torch.isfinite(v).all() and (v==0).all()) for v in gradients),
         'Initial residual consistency gradient must be zero on every trainable parameter')
    return dict(passed=True,up_exactly_zero=True,residual_loss=0.,all_residual_gradients_zero=True,
        frozen_selector_excluded_from_autograd=True)


def uniform_self_test(torch):
    """Exact fixed gate and initial U-gradient algebra, with a small frozen FP16 head."""
    from gnnformer.parallel_local_learned_selection import ParallelLocalLearnedSelection
    import torch.nn.functional as F
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(24);ordinary=ParallelLocalLearnedSelection(mode='sigmoid')
        uniform=ParallelLocalLearnedSelection(mode='sigmoid');uniform.load_state_dict(ordinary.state_dict())
        mask=freeze_selector(torch,uniform)
        need(len(mask['trainable_parameter_names'])==6 and len(mask['frozen_selector_parameter_names'])==2,
             'Uniform named tensor mask differs')
        h=torch.randn(3,2,3584).half();g=torch.randn(2,3584).half()
        valid=torch.tensor([[True,True],[True,False],[False,False]])
        head=torch.nn.Linear(3584,9,bias=False,dtype=torch.float16).requires_grad_(False)
        da,ca=ordinary(h,g,valid_mask=valid,output_dtype=torch.float32,capture=True)
        db,cb=uniform(h,g,valid_mask=valid,output_dtype=torch.float32,capture=True)
        need(torch.equal(cb['gates'],valid.float()*.5) and torch.equal(ca['aggregate'],cb['aggregate'])
             and torch.equal(ca['preactivation'],cb['preactivation']) and torch.equal(da,db),
             'Uniform initial algebra differs from learned sigmoid initialization')
        la=F.cross_entropy(head(ordinary.rms(g+da.half()).half()).float(),torch.tensor([1,2]))
        lb=F.cross_entropy(head(uniform.rms(g+db.half()).half()).float(),torch.tensor([1,2]))
        uniform_zero_gradients(torch,uniform,(db[1]-db[0]).square().sum())
        ga=torch.autograd.grad(la,ordinary.up.weight)[0];gb=torch.autograd.grad(lb,uniform.up.weight)[0]
        need(torch.equal(ga,gb) and bool((gb!=0).any()),'Initial native-cast U gradient differs or fixture is vacuous')
        with torch.no_grad():uniform.up.weight.normal_(std=.001)
        before={n:p.detach().clone() for n,p in uniform.named_parameters() if n in FROZEN_SELECTOR}
        trainable=[p for p in uniform.parameters() if p.requires_grad];optimizer=torch.optim.AdamW(trainable,lr=.001,weight_decay=0.)
        need({id(p) for group in optimizer.param_groups for p in group['params']}=={id(p) for p in trainable},
             'Optimizer parameter mask differs')
        delta,cap=uniform(h,g,valid_mask=valid,output_dtype=torch.float32,capture=True)
        loss=F.cross_entropy(head(uniform.rms(g+delta.half()).half()).float(),torch.tensor([1,2]))
        residual=(delta[1]-delta[0]).square().mean();raw,diagnostic=gradient_geometry(torch,uniform,loss,residual,0.,cap)
        need(set(raw['ce'])==set(mask['trainable_parameter_names']) and not diagnostic['groups']['selector']['parameter_names']
             and diagnostic['groups']['selector']['ce_norm']==diagnostic['groups']['selector']['consistency_norm']==0.,
             'Frozen selectors entered parameter-gradient diagnostics')
        loss.backward();torch.nn.utils.clip_grad_norm_(trainable,1.);optimizer.step();check_selector(torch,uniform)
        need(all(torch.equal(dict(uniform.named_parameters())[n],v) for n,v in before.items()),'Optimizer changed frozen selector')
        _,after=uniform(h,g,valid_mask=valid,output_dtype=torch.float32,capture=True)
        need(torch.equal(after['gates'],valid.float()*.5),'Fixed gate changed after real trainable update')
        uniform.selection_weight.requires_grad_(True)
        try:check_selector(torch,uniform)
        except ValueError:pass
        else:raise ValueError('Accidentally trainable selector accepted')
        uniform.selection_weight.requires_grad_(False)
    return dict(passed=True,fixed_half_and_padding=True,initial_aggregate_and_preactivation_exact=True,
        initial_frozen_fp16_head_U_gradient_exact_nonzero=True,actual_trainable_parameters=1041600,
        frozen_selector_coordinates=97,optimizer_and_autograd_exclude_frozen_selector=True,no_pretrained_model_loaded=True)
...
def main():
    ...
    if args.release:need(args.plan is not None and args.profiles and len(args.profiles)==1,'Release needs the single fixed-uniform profile')
    if args.report:need(args.plan is not None and args.runs and len(args.runs)==1 and args.main_release is not None,'Report needs the single fixed-uniform main and release')
    ...
    try:
        if args.check:result=check(out,frozen)
            ...
            from scripts import report_native_identity_join_uniform as reporter
