"""Independent single-product readout-only-query audit; cached training screen.

The completed parent product is a hash-bound comparator, never rerun.
Full-name CE, initial tensors, conditioning and final-only criteria are fixed.
"""
...
OWN=('scripts/report_native_identity_join_readout_query.py','slurm/native_identity_join_readout_query_report.sbatch')
...
def driver():
    from scripts import diagnose_native_identity_join_readout_query as module
    return module
...
def functional_core(torch,h,g,valid,mean,scale,w,arm,cyclic=False):
    ...
    local=f.linear(conditioned,w['local.weight']);a=local[...,:rank]+w['local_bias'][:rank]
    b=local[...,rank:]+w['local_bias'][rank:];u=torch.tanh(a);v=torch.tanh(b)
    indices=permutation_indices(torch,valid,cyclic);used=v.gather(0,indices.unsqueeze(-1).expand_as(v))
    need(arm=='product','Only product is registered');payload=u*used
...
def audit_capture(torch,out,record,arm,plan,rows,scenes,states,index,stats,initial,final,logs,norm,head):
    ...
    payload=observed['factor_a']*observed['factor_b_used']
    ...
    result=dict(arm=arm,phase=phase,step=step,sids=sids,capture_file=record['file'],capture_sha256=record['sha256'],
        ...
        factor_pairing='cyclic' if cyclic else 'paired',query_placement='readout_only',captured_rows=len(g),
...
def self_test(torch):
    ...
    need(torch.equal(indices,torch.tensor([[2,1],[1,0],[0,2]])), 'Ragged cyclic ownership fixture failed')
    ...
    other={**w,'query.weight':2*w['query.weight']};changed=functional_core(torch,h,g,valid,mean,scale,other,'product')
    need(torch.equal(cap['query'],rms(torch,g)) and torch.equal(cap['conditioned_local_rms_input'],(rms(torch,h)-mean)/scale)
         and bool((cap['messages'][1]==0).all()) and torch.equal(cap['factor_b_used'],cap['factor_b']), 'Fixed local conditioning/padding fixture failed')
    need(all(torch.equal(cap[k],changed[k]) for k in ('factor_a','factor_b','payload','aggregate'))
         and not torch.equal(cap['query'],changed['query']) and not torch.equal(cap['delta'],changed['delta'])
         and torch.equal(cap['preactivation'],cap['aggregate']+cap['query'])
         and torch.equal(changed['preactivation'],cap['aggregate']+changed['query']), 'Query must affect only the post-SUM readout')
    from scripts.report_native_identity_join_factor_binding import functional_core as parent_core
    zero={**w,'up.weight':torch.zeros_like(w['up.weight'])}
    current=functional_core(torch,h,g,valid,mean,scale,zero,'product');parent=parent_core(torch,h,g,valid,mean,scale,zero,'product')
    need(bool((current['delta']==0).all()) and torch.equal(current['delta'],parent['delta'])
         and torch.equal(g+current['delta'].half(),g+parent['delta'].half())
         and not torch.equal(current['payload'],parent['payload']), 'Same zero-U native function despite different internal factors')
    ...
    return dict(passed=True,groups=5,ragged_cyclic_indices=True,conditioned_global_and_padding=True,
        readout_only_query=True,zero_U_parent_native_identity=True,complete_family_screen=True)
...
def audit_run(torch,out,directory,plan_path,plan,frozen,rows,scenes,order,states,index,stats,initial,norm,head):
    ...
    need(arm=='product' and directory.parent==p.OUT and directory.name==config['run_id']==f'run_{arm}_{config["slurm_job_id"]}'
    ...
    need(summary['phase']=='run' and summary['steps']==600 and summary['training_head_rows']==21334
         and summary['final_head_rows']==108 and summary['total_head_rows']==21550,'Run endpoint/call rows differ')
    ...
    for key in ('native_identity','native_identity_sha256','native_module_identity','native_model_file','native_model_sha256',
                ...
                'factor_plan','factor_report','query_geometry'):
        need(config[key]==plan[key],'Fixed plan/native/statistics binding differs: '+key)
    need(config['fresh_shared_initialization'] is plan['fresh_shared_initialization'] is False
         and config['reused_unfitted_initialization'] is plan['reused_unfitted_initialization'] is True
         and config['query_placement']==plan['query_placement']=='readout_only' and config['initialized']==plan['initial_state'] and config['initial_checkpoint_sha256']==plan['initial_sha256'],'Shared original initialization differs')
    ...
    need(endpoint['before_evaluation']==endpoint['after_evaluation']==table and endpoint['parameter_sha256']==summary['selected_parameter_sha256']==object_sha(table)
         ...
         and endpoint['query_placement']==summary['query_placement']==config['query_placement']=='readout_only'
    ...
    training['permutation_head_rows']=108;training['total_head_rows']=21550
    ...
    expected_positions += [('permutation',step) for step in range(1,8)]
    ...
    phases=('evaluation','permutation')
    need(summary['permutation_diagnostic']==dict(rule=p.POLICY['permutation'],contexts=108,head_calls=7,head_rows=108,
    ...
    return dict(passed=True,completed=True,arm=arm,slurm_job_id=config['slurm_job_id'],run_directory=str(directory),run_summary_sha256=sha(directory/'summary.json'),
        ...
        cpu_head_calls=14,cpu_norm_calls=14,cpu_head_rows=sum(r['head_rows'] for r in capture_audits),
        ...
        permutation_diagnostic=permutation,query_placement='readout_only')
...
def resources(out,runs):
    ...
    need(len(records)==1 and {r['name'] for r in records}==names,'Exactly one product attempt required; failed/zero allocations retained')
    for row in records:
    ...
    total=sum(r['gpu_seconds'] for r in records);need(current==0 and peak<=1 and total<=90,'Readout-query campaign cap exceeded')
...
def audit_parent(plan,rows,order):
    """Hash/JSON joins only: no parent tensor load, head replay or refitting."""
    from scripts import diagnose_native_identity_join_factor_binding as factor_driver
    ledger={}
    def bound(file,digest):
        bind(file,digest);ledger[str(Path(file).resolve())]=digest;return Path(file)
    descriptor=plan['factor_plan'];parent=read(bound(descriptor['file'],descriptor['sha256']))
    proof=plan['factor_report'];summary=read(bound(proof['summary_file'],proof['summary_sha256']))
    analysis=read(bound(proof['analysis_file'],proof['analysis_sha256']))
    need(summary['passed'] is summary['completed'] is analysis['passed'] is analysis['completed'] is True
         and summary['phase']==analysis['phase']=='report' and summary['analysis_file']==proof['analysis_file']
         and summary['analysis_sha256']==proof['analysis_sha256']
         and summary['source_sha256']==analysis['source_sha256']==parent['source_sha256']==factor_driver.sources()
         and analysis['plan_file']==descriptor['file'] and analysis['plan_sha256']==descriptor['sha256']
         and summary['first_token_screen']==analysis['first_token_screen']==proof['first_token_screen'], 'Passed parent report/source/plan join differs')
    for name,digest in parent['source_sha256'].items():
        bound(Path(proof['summary_file']).parent/'source'/name.replace('/','_'),digest)
    for key in ('initial_sha256','initial_state','features_file','features_sha256','feature_tensor','stats_file','stats_sha256','stats_tensors',
                'native_identity','native_identity_sha256','native_module_identity','native_model_file','native_model_sha256','head_rows_by_update','trainability_mask'):
        need(plan[key]==parent[key], 'Parent-unfitted initialization/data/native protocol differs: '+key)
    bound(plan['initial_file'],plan['initial_sha256']);bound(parent['initial_file'],parent['initial_sha256'])
    for key in ('rows_file','scenes_file','pairs_file','order_file','stats_inventory_file'):
        bound(parent[key],parent['runtime_bindings'][str(Path(parent[key]).resolve())])
        need(read(plan[key])==read(parent[key]), 'Original parent data/order bytes differ: '+key)
    need(read(parent['rows_file'])==rows and read(parent['order_file'])==order,'Parent comparison row/order mismatch')
    published=analysis['runs']['product'];directory=Path(published['run_directory'])
    run=read(bound(directory/'summary.json',published['run_summary_sha256']))
    need(published['passed'] is published['completed'] is run['passed'] is run['completed'] is True
         and str(run['slurm_job_id'])=='443375' and run['arm']==published['arm']=='product'
         and run['source_sha256']==parent['source_sha256'] and run['initial_checkpoint_sha256']==plan['initial_sha256']
         and run['initialized']==plan['initial_state'] and run['order_sha256']==object_sha(order)
         and run['steps']==600 and run['policy']==parent['policy'] and run['fresh_shared_initialization'] is True,
         'Frozen original product endpoint differs')
    for key in ('stats_file','stats_sha256','stats_tensors','native_identity','native_identity_sha256','native_module_identity'):
        need(run[key]==plan[key], 'Parent actual run fixed binding differs: '+key)
    bound(run['initial_checkpoint'],run['initial_checkpoint_sha256']);bound(run['checkpoint'],published['checkpoint_sha256'])
    need(run['checkpoint']==published['checkpoint'] and run['checkpoint_sha256']==published['checkpoint_sha256']
         and run['selected_parameter_sha256']==published['selected_parameter_sha256'], 'Parent fitted endpoint identity differs')
    outcomes={};metrics={}
    for phase in ('evaluation','permutation'):
        prefix='' if phase=='evaluation' else 'permutation_';item=published['outcomes'][phase]
        values=read(bound(item['file'],item['sha256']))
        saved=read(bound(run[prefix+'predictions_file'],run[prefix+'predictions_sha256']))
        bound(run[prefix+'raw_file'],run[prefix+'raw_sha256'])
        need(len(values)==len(saved)==len(rows)==108, 'Parent outcome denominator differs')
        for expected,value,original in zip(rows,values,saved):
            need(set(value)==set(original)==set(expected)|{'argmax_id','first_token_correct','nll','gpu_fp32_nll'}
                 and all(value[k]==original[k]==v for k,v in expected.items())
                 and type(value['argmax_id']) is int and 0<=value['argmax_id']<152064
                 and type(value['first_token_correct']) is bool and value['first_token_correct']==(value['argmax_id']==expected['first_token_id'])
                 and all(value[k]==original[k] for k in ('argmax_id','first_token_correct','gpu_fp32_nll'))
                 and close(value['nll'],original['nll']) and close(value['nll'],value['gpu_fp32_nll']), 'Parent audited prediction/raw metadata differs')
        criterion=first_token_criterion(values)
        measured={k:criterion[k] for k in ('correct','contexts','complete_families','families')}
        measured['mean_nll']=sum(r['nll'] for r in values)/108
        need(all(close(value,published['phase_metrics'][phase][key]) for key,value in measured.items()), 'Parent published phase metrics differ')
        outcomes[phase]=values;metrics[phase]=measured
    screen=first_token_criterion(outcomes['evaluation'])
    need(screen==run['first_token_fit']==published['first_token_fit']==summary['first_token_screen']['product']
         and screen['passed'] is False, 'Original fixed product screen must remain failed')
    geometry=plan['query_geometry'];bound(geometry['file'],geometry['sha256'])
    return dict(passed=True,run_directory=str(directory),run_summary_sha256=published['run_summary_sha256'],
        factor_plan=descriptor,factor_report=proof,query_geometry=geometry,initial_sha256=plan['initial_sha256'],
        checkpoint=published['checkpoint'],checkpoint_sha256=published['checkpoint_sha256'],
        first_token_fit=screen,phase_metrics=metrics,outcomes=published['outcomes'],bindings=ledger,
        no_parent_tensor_load=True,no_parent_head_replay=True,no_parent_refit=True),outcomes


def report(args,out,frozen):
    import torch
    torch.set_num_threads(4);p=driver();plan=p.verify_plan(args.plan,ancestors=True)
    need(frozen==plan['source_sha256']==p.sources() and len(frozen)==108 and len(args.runs)==1,
         'Source closure or single-product inventory differs')
    need(plan['query_placement']==p.POLICY['query_placement']=='readout_only'
         and plan['reused_unfitted_initialization'] is True and plan['fresh_shared_initialization'] is False,
         'Registered query placement/initialization differs')
    rows,scenes,pairs,order,states,index,stats=audit_inputs(torch,plan)
    parent,parent_outcomes=audit_parent(plan,rows,order);save(out/'parent_comparator_audit.json',parent)
    ...
    need(set(initial_packet)=={'seed','branch'} and initial_packet['seed']==24 and {k:p.tensor_info(v) for k,v in initial.items()}==plan['initial_state']
         and bool((initial['up.weight']==0).all()), 'Reused unfitted initialization tensor table differs')
    check_weights(torch,initial,initial)
    from gnnformer.parallel_local_readout_query_binding import ParallelLocalReadoutQueryBinding
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(24);unfitted=ParallelLocalReadoutQueryBinding(interaction='product')
    need(p.base.v7.state_info(unfitted)==plan['initial_state'],'Independent constructor changed original seed24 parameter bytes');del unfitted
    ...
    value=audit_run(torch,out,args.runs[0],args.plan,plan,frozen,rows,scenes,order,states,index,stats,initial,norm,head);runs={'product':value}
    need(value['arm']=='product' and native_before==dict(norm=p.tensor_info(norm.weight),head=p.tensor_info(head.weight))
         and norm.weight.grad is head.weight.grad is None and not norm.weight.requires_grad and not head.weight.requires_grad,
         'Single-product frozen native audit incomplete')
    need(value['cpu_head_calls']==value['cpu_norm_calls']==14 and value['cpu_head_rows']==216
         and len(value['capture_audits'])==20, 'Independent final replay/capture coverage differs')
    budget=resources(out,runs);phases={}
    for phase in ('evaluation','permutation'):
        current=read(value['outcomes'][phase]['file']);previous=parent_outcomes[phase]
        need([r['sid'] for r in current]==[r['sid'] for r in previous]==[r['sid'] for r in rows], 'Parent/new prediction pairing differs')
        transitions=Counter((a['first_token_correct'],b['first_token_correct']) for a,b in zip(previous,current))
        phases[phase]=dict(parent=parent['phase_metrics'][phase],readout_only=value['phase_metrics'][phase],
            parent_to_readout_only_transitions={f'{int(a)}_to_{int(b)}':transitions[a,b] for a in (False,True) for b in (False,True)},
            readout_only_minus_parent_correct=value['phase_metrics'][phase]['correct']-parent['phase_metrics'][phase]['correct'],
            readout_only_minus_parent_mean_nll=value['phase_metrics'][phase]['mean_nll']-parent['phase_metrics'][phase]['mean_nll'])
    comparison=dict(scope='one fixed query-placement ablation on cached training first queries; frozen parent product comparator',
        contexts=108,phases=phases,initial_parameter_bytes_equal=True,initial_native_output_equal_zero_U=True,
        internal_payload_and_first_U_gradient_not_required_equal=True,all_comparisons_descriptive=True,
        no_factor_semantics_or_expressiveness_claim=True,no_parent_head_replay=True,no_parent_refit=True)
    save(out/'parent_query_placement_comparison.json',comparison)
    result=dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,
        plan_file=str(Path(args.plan).resolve()),plan_sha256=sha(args.plan),query_placement='readout_only',
        factor_plan=plan['factor_plan'],factor_report=plan['factor_report'],query_geometry=plan['query_geometry'],
        conditioning_report=plan['conditioning_report'],conditioning_plan=plan['conditioning_plan'],
        ...
        stats_tensors=plan['stats_tensors'],runs=runs,parent_comparator=parent,comparison=comparison,resources=budget,
        first_token_screen={'product':value['first_token_fit']},cpu_head_calls=14,cpu_norm_calls=14,cpu_head_rows=216,
        gpu_head_calls=614,gpu_norm_calls=614,gpu_head_rows=21550,vlm_calls=0,vision_calls=0,
    ...
    c=value['first_token_fit'];old=parent['first_token_fit'];perm=value['permutation_diagnostic']['permuted']
    lines=['# Product query-placement diagnostic','',
        ...
        f"| Frozen parent: query in factors and readout | {old['correct']} | {old['complete_families']} | {'PASS' if old['passed'] else 'FAIL'} |",
        f"| New: query in readout only | {c['correct']} | {c['complete_families']} | {'PASS' if c['passed'] else 'FAIL'} |",'',
        f"The new product after the fixed cyclic factor permutation scored {perm['correct']}/108 first tokens and {perm['complete_families']}/18 complete families. "
        'The permutation is descriptive; it cannot change the paired screen.','',
        'Both runs started from the identical unfitted seed 24 parameter packet and used the same 600 full-name-plus-EOS CE updates, pair order and global conditioning statistics. '
        'The initial native output is identical because U is zero. Internal factors, aggregates and the first U gradient can differ. '
        'Only the new run was fitted or replayed in this allocation; the completed parent is joined through its passed report, saved predictions and raw file hashes.','',
        'Local hidden states still depend on the question and prefix. The explicit global query is removed from both factors and retained after the sum. '
        'This one-seed training diagnostic does not assign semantic roles to the factors or establish wider communication capacity, whole-answer accuracy, generalization or reasoning composition.','',
        ('The fixed screen passed; only preparation of separately audited native evaluation of this unchanged endpoint is permitted.' if c['passed'] else
         'The fixed screen failed. This architecture branch ends without native evaluation, continuation or another variant.'),'',
        f"Allocated GPU cost: {budget['allocated_gpu_seconds']} seconds. GPU work: 614 norm/head batches, 21,550 rows, zero VLM/vision calls. "
        'Independent CPU replay: 14 final batches, 216 rows, fixed TV <= 0.02 and exact argmax.\n']
    (out/'REPORT.md').write_text('\n'.join(lines))
    return dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,
        analysis_file=str(out/'analysis.json'),analysis_sha256=sha(out/'analysis.json'),query_placement='readout_only',
        first_token_screen=result['first_token_screen'],resources=budget,cached_first_query_only=True,no_native_or_whole_answer_claim=True)
