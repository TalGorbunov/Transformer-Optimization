#!/usr/bin/env python3
"""ARMOR Experiment A — the Hahn O(1/N) sensitivity + margin instrument.

Directly measures, inside the frozen production VLM (7B, 4-bit, sdpa), the single-frame
output sensitivity that Hahn (TACL 2020) bounds at O(1/n) for soft attention, and the
answer-logit margin vs N. Zero training; forward passes only.

PAIRED INPUTS: for a pool sample (gold k <= --max-gold), one non-evidence frame t (with
the asked character C present in a wrong room) is toggled to evidence (C moved into the
asked room R) -> gold k+1. Both members are rendered fresh from states via the
deterministic MMReD park renderer; byte-identity of all frames except t is HARD-ASSERTED
per pair. A matched answer-PRESERVING control flip (C moved to a different wrong room,
same frame t) runs on the controls subset.

ARMS (per pair member):
  plain    deployed joint baseline: build_count_prompt + frames, causal, no replicas.
           Loci: final position (answer position). + digit logits (0..9 house protocol)
           -> A2 margin = logit(gold) - max(other digits), pred = argmax digit.
  repjoint replica layout (Q-first, per-frame question replicas), BASE replica-probe mask
           (replicas invisible to the joint computation; replica rows read the JOINT
           frame supply), no posreset. Loci: final, anchor (seq-1-ANCHOR_OFFSET),
           replica of flipped frame t.
  fenced   same layout, full block fence + per-block M-RoPE posreset (A3 deployed
           supply config). Loci: final, anchor, replica of flipped frame t.

NOISE CONTROLS (first --controls pairs per run; all measured, never theoretical):
  replay   plain(base) forward twice -> determinism floor.
  ctrl     plain arm on the answer-preserving flip -> matched single-frame control curve
           (Hahn's bound applies to it too; discriminability is what separates them).
  perm     fenced(base) with non-flipped frame slots permuted -> bf16 reduction-reorder
           floor (content-preserving BY CONSTRUCTION under fence+posreset: identical
           per-block positions, identical block set).

Hidden-state indexing: hs = output_hidden_states tuple (29 entries; hs[0]=embeddings,
hs[i]=output of decoder layer i, hs[28]=post-final-norm). --hs-layers indexes this tuple.

Pre-registered bands (outputs/armor/CAMPAIGN_BRIEF.md): joint slope alpha in [0.7,1.3];
fenced |alpha| < 0.2; margin crosses the measured floor within 2x of the accuracy-chance N.

LORAMECH L1 delta (2026-08-26): --peft-adapter restores a saved LoRA adapter dir
(frozen, eval-only) so the same instrument measures the TRAINED model; --arms selects
which arms run (default all three — "plain" alone skips the replica layouts and their
perm control; replay/ctrl controls follow the plain arm). Flags default to the exact
pre-delta behavior, so the [2026-08-22] anchor runs reproduce with no flags.

Usage (one N per invocation):
  python scripts/armor/probe_hahn.py \
      --data_root data/mmred_longN_park/seq_len_32/all_uniform --limit 50 \
      --controls 12 --output outputs/armor/hahn/N32
"""
...
def build_pair(sd, rng, max_gold, gold_set=None):
    """-> dict with q0, room, char, gold, flip_t, base/evid/ctrl state lists — or None."""
...
def main() -> int:
    ...
    ap.add_argument("--gold-set", default="",
                    help="REDUX C4b: explicit comma-set of base golds to accept "
                         "(OVERRIDES --max-gold; e.g. '32' for decay-in-k cells). "
                         "Digit margins are skipped for gold>8 (single-digit protocol).")
    ap.add_argument("--attn-logn-sref", type=int, default=0,
                    help="REDUX C4b: >0 = log-N attention-logit scaling per forward "
                         "(base_scaling*ln(S)/ln(S_ref), LM decoder attn only, same "
                         "hook as the trainers). No flag = byte-identical behavior.")
    ...
    ap.add_argument("--arms", default="plain,repjoint,fenced",
                    help="comma-set of arms to run (LORAMECH L1: plain; N2: p1fence = "
                         "the P1 trained-fenced layout — [frame+q] blocks, hide_cols=[], "
                         "posreset, count-prompt tail)")
    ...
    # C4b logN hook — attn modules captured PRE-wrap (PeftModel.model trap)
    ...
    def set_logn_scaling(seq_len):
        ...
        for m_ in _attn_mods:
            m_.scaling = s
    # PEFT wrap LAST: PeftModel.model is the OUTER base model, so any model.model.*
    # access captured after wrapping resolves wrong (the 137800/137801 lesson).
    # LoRA layers are injected in-place — hooks/rope_fn captured above stay valid.
    ...
    for sd in iter_sample_dirs_shuffled(Path(args.data_root), args.seed):
        ...
        if "plain" in arms:
            ...
            if gold + 1 <= 9:  # single-digit margin protocol (C4b k>8 cells skip it)
        ...
        # ---------------- p1fence arm (the TRAINED fenced layout: N2 read-vs-supply)
        if "p1fence" in arms:
            ...
            # verdict locus: room word inside flip_t's replica (backward search stays
            # inside the block; vision tokens never decode to the room word)
            p_rep_t = locate_word_token(p_ids, tok, room, p_blocks[flip_t])
            if p_rep_t is None:
                n_skip += 1
                continue
            p_seq = len(p_ids)
            p_mask = build_block_mask(p_seq, p_blocks, hide_cols=[])
            with torch.inference_mode():
            ...
            hooks.set_mask(p_mask, model.device)
            ...
            if "plain" not in arms and gold + 1 <= 9:  # one margin row/pair; digit protocol
        ...
        if do_controls and "plain" in arms:
            # replay floor (plain, identical input twice)
            hooks.clear_mask()
            ...
            for (L, name), (dn, bn) in dnorms(cap_pb, cap_pb2).items():
                ctrl_w.writerow([pair["sample_id"], NF, "replay", "plain", L, name,
                                 f"{dn:.6g}", f"{bn:.6g}"])
            # matched answer-preserving flip (plain)
        ...
        if do_controls and "fenced" in arms:
            # bf16 reduction-reorder floor (fenced, non-flip slots permuted)
    ...
    lines = [f"=== HAHN PROBE (pairs={n_done}, skip={n_skip}, data={args.data_root}, "
             ...
             f"logn_sref={args.attn_logn_sref}) ===",
             "[gold-hist] " + format_gold_histogram(gold_hist)]
    # quick medians for the log (figure script does the real analysis)
    import collections
