"""Clean-data MMReD Vision attribution experiment (V2), using the ordinary Qwen2.5-VL forward.
...
V2_ARMS = ("base", "lora", "global", "hidden", "middle_lora")
...
def validate_args(a):
    ...
    if not 0 <= a.gold_max <= 16:
        raise SystemExit("V2 registered support is K<=16")
    if not a.manifest:
        raise SystemExit("V2 requires exact staged manifests")
...
def parser():
    ...
    next(action for action in p._actions if action.dest == "arm").choices = V2_ARMS
    ...
    p.set_defaults(model="Qwen/Qwen2.5-VL-7B-Instruct", train_ns=[8, 16],
                   ...
                   limit_eval=108, gold_max=16, epochs=9, arm="global", max_seq_tokens=16000,
                   output=REPO_ROOT / "outputs/native_aggregation_vlm/v2")
...
def run(a) -> int:
    ...
    checkpoint_dir = a.checkpoint_root / "native_aggregation_vlm_v2" / run_id
    code_dir = outdir / "code"
    code_dir.mkdir()
    code_hashes = {}
    for name in ("scripts/native_aggregation_vlm_v2.py", "gnnformer/aggregation_controls.py", "scripts/native_aggregation.py",
    ...
    architecture = dict(protocol="vision_v2_clean", hidden_rank=a.hidden_rank,
