...
OWN = ('scripts/train_native_identity_join_learned_v2.py',
    'tests/test_native_identity_join_learned_training_v2.py',
    'slurm/native_identity_join_learned_v2_train_check.sbatch',
    'slurm/native_identity_join_learned_v2_train_profile.sbatch',
    'slurm/native_identity_join_learned_v2_train.sbatch',
...
def test_helpers():
    result=[]
    for name in ('tests/test_native_identity_join_learned_training_v2.py',
                 'tests/test_paired_sequence_objectives.py'):
...
def verify_release(path,plan_path,plan):
    ...
    need(release['protocol']=='identity_join_learned_selection_main_release' and release['passed'] is True
         ...
         and release['per_main_seconds_cap']==3300 and release['campaign_gpu_seconds_cap']==24000,
    ...
    for condition,item in release['profiles'].items():
        ...
        need(projection['passed'] is True and 0<projection['projected_seconds']<=3300,'Main projection exceeds fixed cap')
...
def run(args,out,frozen,started):
    ...
    hardware=json.loads(json.dumps(software.live_identity(torch,loaded,software_plan)))
