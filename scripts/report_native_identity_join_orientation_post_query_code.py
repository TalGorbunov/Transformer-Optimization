"""Independent post-SiLU query audit of the privileged first-query control.
...
from scripts import report_native_identity_join_orientation_first_query_code as previous
from scripts import report_native_identity_join_factor_orientation_training as orientation
OWN=('scripts/report_native_identity_join_orientation_post_query_code.py',)
CORE_CLASS='gnnformer.parallel_local_joint_code_post_query.ParallelLocalJointCodePostQuery'
...
def driver():
    from scripts import diagnose_native_identity_join_orientation_post_query_code as module
    return module


def functional_core(torch,codes,g,valid,w):
    f=torch.nn.functional;q=f.linear(old.rms(torch,g),w['query.weight'])
    gates=valid.float()*.5;messages=codes*gates.unsqueeze(-1);aggregate=messages.sum(0)
    pre=f.linear(aggregate,w['aggregate_projection.weight'],w['aggregate_projection.bias'])
    readout=f.silu(pre)+q;delta=f.linear(readout,w['up.weight'])
    return dict(query=q,payload=codes,scores=torch.full(valid.shape,.5),gates=gates,messages=messages,
        aggregate=aggregate,preactivation=pre,readout=readout,delta=delta)
...
def placement_fixture(torch):
    ...
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(24);parent=ParallelLocalJointCodeOracle(4,rank=18)
        torch.manual_seed(24);core=p.make_core(torch,4,rank=18)
    need(type(core).__module__+'.'+type(core).__name__==CORE_CLASS and core.query_placement=='post_silu'
         ...
         and all(torch.equal(v,parent.state_dict()[k]) for k,v in core.state_dict().items()),'Core class or same unfitted parameter bytes differ')
    codes=torch.zeros(2,3,18);codes[0,:,0]=1;codes[1,:,1]=1
    valid=torch.tensor([[True,True,True],[True,False,True]])
    g=torch.tensor([[.2,-.3,.4,.1],[.1,.5,-.2,.3],[-.4,.2,.3,.1]],dtype=torch.float16)
    zero,cap=core(codes,g,valid_mask=valid,capture=True);old_zero,old_cap=parent(codes,g,valid_mask=valid,capture=True)
    need(bool((zero==0).all()) and torch.equal(zero,old_zero),'Identical initial zero-U output required')
    for key in ('query','payload','scores','gates','messages','aggregate'):
        need(torch.equal(cap[key],old_cap[key]),'Readout placement changed upstream code aggregation: '+key)
    with torch.no_grad():
        core.up.weight.copy_(.03*torch.arange(72,dtype=torch.float32).cos().reshape(4,18));parent.load_state_dict(core.state_dict())
    delta,cap=core(codes,g,valid_mask=valid,output_dtype=torch.float32,capture=True)
    old_delta,_=parent(codes,g,valid_mask=valid,output_dtype=torch.float32,capture=True)
    reference=functional_core(torch,codes,g,valid,core.state_dict())
    need(set(cap)==set(reference) and all(torch.allclose(cap[k],v,rtol=1e-6,atol=1e-6) for k,v in reference.items())
         and not torch.equal(delta,old_delta),'Independent post-SiLU formula must distinguish the old placement')
    delta2,cap2=core(codes,g.roll(1,1),valid_mask=valid,output_dtype=torch.float32,capture=True)
    dq=cap2['query']-cap['query']
    need(torch.equal(cap2['preactivation'],cap['preactivation']) and not torch.equal(cap2['query'],cap['query'])
         and torch.allclose(delta2-delta,torch.nn.functional.linear(dq,core.up.weight),rtol=1e-5,atol=1e-6),
         'Fixed-code query change must enter only the additive readout route')
    gradients=torch.autograd.grad(delta.square().sum(),tuple(core.parameters()),allow_unused=True)
    need(all(x is not None and bool(torch.isfinite(x).all()) and bool((x!=0).any()) for x in gradients),
         'All four actual post-query readout tensors need live gradients')
    return dict(passed=True,actual_core=True,same_initial_state_and_zero_U_output=True,upstream_half_SUM_unchanged=True,
        independent_post_SiLU_formula=True,distinguishes_pre_SiLU_query=True,fixed_code_query_route_additive=True,all_four_gradients_live=True)
...
def self_test(torch):
    return dict(passed=True,parent=previous.self_test(torch),first_query_objective=objective_fixture(torch),post_query_placement=placement_fixture(torch))


def audit_inputs(torch,plan):
    p=driver();ref=plan['first_query_code_plan'];bind(ref['file'],ref['sha256'])
    need(ref==dict(file=str(p.PARENT),sha256=p.PARENT_SHA),'Exact passed first-query-code preparation required')
    ...
    for key in ('codes_sha256','code_tensor','features_sha256','feature_tensor','global_feature_ids','initial_sha256','initial_state',
                ...
                'joint_code_orientation_plan','paired_code_plan','paired_code_report','paired_order_stage','negative_evidence','capacity_witness','training_stage','trainability_mask',
    ...
    need(plan['paired_minibatch_only'] is False and plan['unchanged_presentation_multiset'] is True
         ...
         'First-query supervision and all exposure must remain fixed')
    need(plan['query_placement']=='post_silu' and plan['core_class']==CORE_CLASS,'Only registered post-SiLU query core permitted')
    control=plan['first_query_code_report'];bind(control['file'],control['sha256']);bind(control['analysis_file'],control['analysis_sha256'])
    need(control['file']==str(p.CONTROL) and control['sha256']==p.CONTROL_SHA
         and control['analysis_sha256']==p.CONTROL_ANALYSIS_SHA,'Exact passed first-query comparison required')
    ...
    need(len(outcomes)==216 and all({k:a[k] for k in b}==b for a,b in zip(outcomes,rows))
         and all(r['first_token_correct']==(r['argmax_id']==r['first_token_id']) for r in outcomes),'Parent first-query outcome ownership differs')
    criterion=first_token_criterion(outcomes)
    need(criterion==run['first_token_fit']==proof['first_token_screen'][p.ARM]
         and criterion['first_correct']==74 and criterion['complete_families']==0 and criterion['passed'] is False,
         'Registered negative first-query control differs')
...
def audit_capture(torch,out,record,plan,scenes,states,index,by_sid,rows,initial,endpoints,logs,norm,head):
    ...
    if training:
        need(packet['arm']==p.ARM and packet['query_placement']=='post_silu' and packet['core_class']==CORE_CLASS
             and step in DIAGNOSTIC_STEPS and packet['step']==record['weights_step']==step-1
    ...
    need(cap['schema_version']==1 and cap['arm']==p.ARM and cap['phase']==phase and cap['step']==step
         and cap['sids']==record['sids']==sids and cap['endpoint_step']==endpoint_step
         and cap['query_placement']==record['query_placement']==plan['query_placement']=='post_silu'
         and cap['core_class']==record['core_class']==plan['core_class']==CORE_CLASS, 'Capture scene/phase ownership differs')
...
def audit_run(torch,out,directory,plan_path,plan,frozen,rows,scenes,order,states,index,by_sid,initial,norm,head):
    ...
    inherited=dict(plan_file=plan['first_query_code_plan']['file'],plan_sha256=plan['first_query_code_plan']['sha256'],source_sha256=plan['inherited_source_sha256'])
    ...
    for key in ('native_identity','native_identity_sha256','native_module_identity','native_model_file','native_model_sha256',
                ...
                'parent_initial_file','parent_initial_sha256','first_query_code_plan','first_query_code_report','query_placement','core_class','paired_code_plan','paired_code_report','first_query_supervision_only','full_target_forward_unchanged','joint_code_plan','joint_code_orientation_plan','paired_order_stage','negative_evidence','orientation_plan','capacity_witness','training_stage','factor_plan','uniform_plan','architecture_stop_report','decision_step','evaluation_steps'):
    ...
    for e in ENDPOINTS:
        ...
        need(endpoint['passed'] is True and endpoint['query_placement']=='post_silu' and endpoint['core_class']==CORE_CLASS and endpoint['endpoint_step']==e and endpoint['before_evaluation']==endpoint['after_evaluation']==table
...
def report(args,out,frozen):
    ...
    inherited=dict(plan_file=plan['first_query_code_plan']['file'],plan_sha256=plan['first_query_code_plan']['sha256'],source_sha256=plan['inherited_source_sha256'])
    need(read(out/'inherited_sources.json')==inherited and plan['first_query_code_plan']==dict(file=str(p.PARENT),sha256=p.PARENT_SHA)
    ...
    save(out/'input_reconstruction_audit.json',dict(passed=True,orientation_plan=plan['orientation_plan'],joint_code_plan=plan['joint_code_plan'],capacity_witness=gate,
        ...
        oracle_local_semantics=True,no_answer_or_intersection_code=True,no_new_input_data_or_codes=True))
    ...
    result=dict(passed=run['passed'],completed=True,all_numerical_audits_collected=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        capacity_witness=gate,first_query_code_plan=plan['first_query_code_plan'],first_query_code_report=plan['first_query_code_report'],
        query_placement='post_silu',core_class=CORE_CLASS,paired_code_plan=plan['paired_code_plan'],paired_code_report=plan['paired_code_report'],
        ...
        first_query_supervision_only=True,original_first_query_weights_unchanged=True,full_target_forward_shapes_unchanged=True,only_final_query_placement_changed=True,oracle_local_semantics=True,no_answer_or_intersection_code=True,
    ...
    lines=['# Post-SiLU query local joint-code diagnostic','',
        ...
        ('This post-SiLU query placement fits the privileged first-query screen under the fixed recipe. It does not identify a unique saturation mechanism, establish matched expressivity, or prove visual learning, whole-answer generation or generalization.' if run['passed'] and criterion['passed'] else
    ...
    return dict(passed=True,completed=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        first_query_code_plan=plan['first_query_code_plan'],first_query_code_report=plan['first_query_code_report'],
        query_placement='post_silu',core_class=CORE_CLASS,paired_code_plan=plan['paired_code_plan'],paired_code_report=plan['paired_code_report'],
        first_query_supervision_only=True,full_target_forward_shapes_unchanged=True,
