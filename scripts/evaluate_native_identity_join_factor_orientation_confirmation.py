...
from scripts import diagnose_native_identity_join_factor_orientation_native as previous
from scripts import report_native_identity_join_factor_orientation_native as previous_report
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_factor_orientation_confirmation/evaluation'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_factor_orientation_confirmation/evaluation')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_factor_orientation_confirmation/evaluation')
PROTOCOL='native_identity_join_factor_orientation_confirmation_N16';ARMS=('product','additive')
JOBS={arm:f'identity_join_factor_orientation_confirmation_eval_{arm}' for arm in ARMS}
PROPOSAL='docs/paper/NATIVE_AGGREGATION_FACTOR_ORIENTATION_CONFIRMATION_INFERENCE_PROPOSAL.md'
PROPOSAL_SHA='1f3cbdc8de9b1c838af442a7d34414205942989d2ee95b9bda83373ce216dde3'
POLICY=dict(protocol=PROTOCOL,arms=list(ARMS),checkpoint_step=6000,maximum_new_tokens=4,n_frames=16,contexts=270,
    ...
    model_cap=1080,vision_calls=270,native_head_rows_cap=18360,cpu_check_seconds=300,cpu_report_seconds=300,
    ...
    timing_release_in_same_CPU_check=True,validated_N16_envelope_required=True,confirmation_seed=91726342,reference_orientation_is_not_unseen=True)
OWN=('scripts/evaluate_native_identity_join_factor_orientation_confirmation.py','scripts/report_native_identity_join_factor_orientation_confirmation.py',
    'slurm/native_identity_join_factor_orientation_confirmation_eval_check.sbatch','slurm/native_identity_join_factor_orientation_confirmation_eval_run.sbatch',
    'slurm/native_identity_join_factor_orientation_confirmation_eval_report.sbatch',PROPOSAL)
...
def inherited_sources():
    values={}
    for group in (previous.inherited_sources(),previous.sources(),stage.source_hashes()):
        for name,digest in group.items():
            need(name not in values or values[name]==digest,'Conflicting inherited source: '+name);values[name]=digest
...
def selected_rows(manifest):
    ...
    need(Counter(r['matches_reference_orientation'] for r in rows if r['panel']=='A')=={True:54,False:54}
         ...
         'Confirmation reference-orientation metadata differs')
...
def timing_certificate(analysis):
    observations=[]
    for phase,group,count in (('profile','profiles',8),('main','runs',216)):
...
def project_timings(observations):
    need([(v['phase'],v['arm'],len(v['four_token_seconds'])) for v in observations]==[(phase,arm,count) for phase,count in (('profile',8),('main',216)) for arm in ARMS],
    ...
    need(len(setups)==4 and len(times)==448 and all(math.isfinite(x) and x>0 for x in setups+times),'All prior positive timings required')
    ...
    return dict(passed=seconds<=1200,observations=observations,setup_observations=4,trajectory_observations=448,
...
def envelope_row(torch,bundle,layout):
    ...
    need(n==16 and m['row_count']==17 and m['global_row']==16 and m['row_kinds']==['local']*16+['global'] and m['resize']==392
         ...
         'Only N16 mixed native layouts define this envelope')
...
def envelope_limits(rows):
    need(len(rows)==108,'All108 current native N16 training inputs required');first=rows[0]
...
def check(args,out,frozen):
    ...
    need(stage_ref==parent['confirmation_report'] and manifest['training_stage']==parent['training_stage']
         ...
         and manifest['native_reference']['purpose']=='historical_pipeline_identity_only'
         ...
         'Confirmation data and current trained endpoints must share exact training-stage/native identity; historical success is not new-model evidence')
    ...
    envelope=dict(passed=True,original=old_envelopes,fresh=envelopes,limits=limits,old_contexts=108,fresh_contexts=270,
    ...
    need({k:tensor_info(stats[k]) for k in ('global_mean','scale')}==parent['conditioning_identity'],'Frozen original global statistics changed')
    tests=dict(driver=self_test(torch))
    from scripts.report_native_identity_join_factor_orientation_confirmation import self_test as report_self_test
    ...
    plan=dict(schema_version=1,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),tests=tests,
        native_report=native_ref,stage_report=stage_ref,training_stage=parent['training_stage'],timing_certificate=timing,envelope_file=str(out/'envelope.json'),envelope_sha256=sha(out/'envelope.json'),
...
def single_attempt_guard(out,arm):
    ...
    allocations=factor_audit.allocation_rows(raw,set(JOBS.values())|{'identity_join_factor_orientation_confirmation_eval_run'});seen=Counter()
...
def self_test(torch):
    ...
    observations=[dict(phase=phase,arm=arm,setup_seconds=30.,four_token_seconds=[1.]*count)
        for phase,count in (('profile',8),('main',216)) for arm in ARMS]
    ...
    limits=envelope_limits([sample]*108);within_envelope(sample,limits)
...
def run(args,out,frozen,started):
    ...
    config=dict(protocol=PROTOCOL,policy=POLICY,phase=phase,arm=args.arm,run_id=out.name,slurm_job_id=os.environ['SLURM_JOB_ID'],
        ...
        native_report=plan['native_report'],stage_report=plan['stage_report'],training_stage=plan['training_stage'],timing_certificate=plan['timing_certificate'],
    ...
    for number,case in enumerate(cases):
        ...
        need(sha(item['file'])==item['sha256'],'Prepared confirmation image bundle changed');bundle=torch.load(item['file'],map_location='cpu',weights_only=True)
        ...
        need(result['metadata']['layout']==item['layout'],'Actual native positions differ from CPU-bound confirmation layout')
