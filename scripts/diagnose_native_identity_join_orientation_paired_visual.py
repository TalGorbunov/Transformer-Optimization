"""Paired-minibatch control of the unchanged visual product/additive models.

Only the permutation of existing orientation-specific presentations changes.
No new architecture, objective, feature, parameter initialization or inference.
"""
...
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_orientation_paired_visual'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_orientation_paired_visual')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_orientation_paired_visual')
PROTOCOL='identity_join_orientation_paired_visual'
RUN_JOBS={arm:'identity_join_orientation_paired_visual_'+arm for arm in ARMS}
PARENT=prior.OUT/'check_443676/plan.json'
PARENT_SHA='d299d42a4444520f148f86252e2931ad599e03afac9edadbe9201836229bf097'
PROPOSAL='docs/paper/NATIVE_AGGREGATION_ORIENTATION_PAIRED_VISUAL_PROPOSAL.md'
PROPOSAL_SHA='e75a6cfbb270ef9e10003dbcd2709eabfde9e94a241e7590acda3fedbae37132'
...
POLICY.update(protocol=PROTOCOL,
    ...
    orientation_coverage_only=False,paired_minibatch_only=True,maximum_training_head_rows=48,
...
OWN=('scripts/diagnose_native_identity_join_orientation_paired_visual.py',
     'scripts/report_native_identity_join_orientation_paired_visual.py',
     'slurm/native_identity_join_orientation_paired_visual_check.sbatch',
     'slurm/native_identity_join_orientation_paired_visual_run.sbatch',
     'slurm/native_identity_join_orientation_paired_visual_report.sbatch',PROPOSAL)


def inherited_sources():
    result={}
    for group in (prior.sources(),prior.inherited_sources(),paired_order.sources(),paired_order.inherited_sources()):
...
def check(args,out,frozen):
    ...
    torch.set_num_threads(4);bindings={};bind(PARENT,bindings,PARENT_SHA);parent=prior.verify_plan(PARENT,ancestors=True)
    stage_path=args.order_stage.resolve();stage_path=stage_path/'summary.json' if stage_path.is_dir() else stage_path
    stage=paired_order.verify_stage(stage_path);proof=read(stage_path)
    stage_ref=dict(file=str(stage_path),sha256=sha(stage_path),plan_file=proof['plan_file'],plan_sha256=proof['plan_sha256'])
    bind(stage_path,bindings);bind(stage_ref['plan_file'],bindings,stage_ref['plan_sha256']);bind(REPO/PROPOSAL,bindings,PROPOSAL_SHA)
    need(stage['orientation_plan']==dict(file=str(PARENT),sha256=PARENT_SHA),'Paired order must bind exact original216 orientation preparation')
    ...
    order=read(stage['order_file']);bind(stage['order_file'],bindings,stage['order_sha256'])
    need(len(rows)==len(scenes)==216 and len(pairs)==108 and len(order)==len(old_order)==48000,
         'The216 contexts and48000 paired presentations must be unchanged')
    need(Counter(object_sha(e) for e in order)==Counter(object_sha(e) for e in old_order)
         and order[-48:]==old_order[-48:],'Reordering must preserve every orientation-specific entry and exact final tail')
    counts=[sum(len(scenes[sid]['target_ids']) for pair in order[i:i+8] for sid in pair['sids']) for i in range(0,48000,8)]
    need(counts==stage['head_rows_by_update'] and len(counts)==6000 and sum(counts)==213330 and max(counts)<=48,
         'Reordered full-sequence target-position inventory differs')
    ...
    need({k:tensor_info(v) for k,v in stats.items() if isinstance(v,torch.Tensor)}==parent['stats_tensors']
         ...
         'Original conditioning statistics must remain unchanged')
    packet=torch.load(parent['features_file'],map_location='cpu',weights_only=True)
    need(tensor_info(packet['states'])==parent['feature_tensor'] and packet['states'].dtype==torch.float16
         and packet['states'].shape==(2940,3584) and bool(torch.isfinite(packet['states']).all()),'Original native feature bank differs');del packet,stats
    ...
    from scripts.report_native_identity_join_orientation_paired_visual import self_test as reporter_self_test
    ...
    for file in (initial_file,out/'order.json',out/'head_rows.json',stage_path,Path(stage_ref['plan_file'])):bind(file,runtime)
    plan=dict(parent,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),tests=tests,
        input_bindings=bindings,runtime_bindings=runtime,orientation_plan=dict(file=str(PARENT),sha256=PARENT_SHA),paired_order_stage=stage_ref,
        negative_evidence=stage['negative_evidence'],order_file=str(out/'order.json'),order_object_sha256=object_sha(order),
        head_rows_by_update=counts,head_rows_file=str(out/'head_rows.json'),training_head_rows=213330,total_head_rows=213546,
        initial_file=str(initial_file),initial_sha256=sha(initial_file),paired_minibatch_only=True,orientation_coverage_only=False,
        unchanged_presentation_multiset=True,no_automatic_native_or_fresh_release=True)
    ...
    return dict(passed=True,completed=True,phase='check',protocol=PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        training_head_rows=213330,maximum_training_head_rows=max(counts),evaluation_head_rows=216,total_head_rows=213546,no_pretrained_head_forward=True,
        orientation_plan=plan['orientation_plan'],paired_order_stage=stage_ref,negative_evidence=plan['negative_evidence'])
...
def run(args,out,frozen,started):
    ...
    config=dict(protocol=PROTOCOL,run_id=out.name,slurm_job_id=os.environ['SLURM_JOB_ID'],arm=args.arm,seed=24,policy=POLICY,
        ...
        paired_minibatch_only=True,unchanged_presentation_multiset=True,no_automatic_native_or_fresh_release=True,
    ...
    try:
        ...
        need(time.perf_counter()-started<=120,'Single paired visual job exceeded120 seconds');return result
    finally:
...
def main():
    ...
    parser.add_argument('--plan',type=Path);parser.add_argument('--arm',choices=ARMS);parser.add_argument('--order-stage',type=Path)
    args=parser.parse_args();native.require_slurm(gpu=args.run)
    if args.check:
        need(args.order_stage is not None and args.plan is None and args.arm is None and os.environ.get('SLURM_JOB_PARTITION')=='cpu'
             and not os.environ.get('SLURM_JOB_GPUS'),'CPU-only preparation with paired order stage required')
    else:need(args.plan is not None and args.arm is not None and args.order_stage is None,'GPU consumes only passed CPU plan and fixed arm')
