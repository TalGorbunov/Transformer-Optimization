"""Plot verified V16 cells; no new estimates, fitting or selection. CPU Slurm only."""
...
def main():
    ...
    assert data['records']==288 and data['wrapped_records']==272 and data['sentinel_records']==16 and data['all_sentinels_passed'] is True and data['no_practical_milestone_claim'] is True
    out=ROOT/'outputs/native_aggregation_vlm/v16'/('figure_'+os.environ['SLURM_JOB_ID']);out.mkdir(exist_ok=False)
    ...
    for row,n in enumerate((32,64)):
        for col,(mode,label,total) in enumerate((('learned','Learned mean',17),('bank','Empirical bank mean',17))):
            ax=axes[row,col]
            for variant,shift,color,title in [('original',-.19,'#718096','Original Step labels'),('wrapped',.19,'#1565b3','Cyclic Step labels 1–16')]:
                values=[data['cells'][f'{name}/N{n}/{mode}'][variant]['all'] for name in names]
                ...
                for name,bar,v in zip(names,bars,values):
                    ...
                    rows.append(dict(model=name,n_frames=n,mode=mode,variant=variant,scope='all',correct=v['correct'],n=v['n']))
    ...
    fig.suptitle('MMReD Vision: sensitivity to rendered Step labels',x=.06,ha='left',fontsize=18)
    fig.legend(*axes[0,0].get_legend_handles_labels(),loc='upper center',bbox_to_anchor=(.53,.945),ncol=2,frameon=False)
    fig.text(.06,.02,'Exploratory reuse: 17 paired families, four frozen models, maximum training length 16.\nOnly actual-image Step footers change. Cyclic labels repeat ordinals; references and N+25 streams remain fixed.',fontsize=10,color='#505965')
    fig.subplots_adjust(left=.07,right=.985,top=.84,bottom=.13,hspace=.32,wspace=.17)
    files=[]
    for suffix in ('png','pdf'):
        p=out/f'native_vision_v16.{suffix}';fig.savefig(p,dpi=180);files.append(dict(path=str(p),sha256=digest(p)))
    plt.close(fig)
    ...
    (out/'INDEX.md').write_text('# V16 existing-cell figure\n\n![Native original/wrapped accuracy](native_vision_v16.png)\n\n[PDF](native_vision_v16.pdf) · [Provenance](summary.json).\n')
