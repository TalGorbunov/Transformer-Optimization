"""Independent privileged216-scene joint-code optimization audit.
...
OWN=('scripts/report_native_identity_join_orientation_joint_code.py',)
...
def driver():
    from scripts import diagnose_native_identity_join_orientation_joint_code as module
    return module
...
def self_test(torch):
    rows=[dict(sid=f'{f}_{v}_{n}_{o}',base_contrast_id=f,variant=v,n_frames=n,orientation_version=o,first_token_correct=True)
          for f in range(18) for v in range(3) for n in (8,16) for o in ('original','flipped')]
    need(primary_screen({6000:rows})['passed'],'Complete216-scene fixture must pass')
    bad=[dict(r,first_token_correct=(r['orientation_version']=='original')) for r in rows]
    need(not primary_screen({6000:bad})['passed'],'One-orientation shortcut must fail')
    people=[f'p{i}' for i in range(9)];rooms=['left','right']
    a,_=local_code('p2','left',people,rooms);b,_=local_code('p2','right',people,rooms)
    outside,_=local_code('p2','elsewhere',people,rooms)
    need(a[4]==b[5]==1 and sum(a)==sum(b)==1 and sum(outside)==0,'Raw local code identity differs')
    paired=torch.tensor([a,b]);flipped=torch.tensor([b,a])
    need(torch.equal(.5*paired.sum(0),.5*flipped.sum(0)),'Swapping two equal-count room coordinates changed the symmetric fixture')
    # Marginals agree while the identity occupying both rooms differs.
    first=[('p0','left'),('p0','right'),('p1','left'),('p2','right')]
    second=[('p0','left'),('p0','left'),('p1','right'),('p2','right')]
    need(Counter(p for p,r in first)==Counter(p for p,r in second)
         and Counter(r for p,r in first)==Counter(r for p,r in second)
         and [sum(local_code(p,r,people,rooms)[0][i] for p,r in first) for i in range(18)]
             !=[sum(local_code(p,r,people,rooms)[0][i] for p,r in second) for i in range(18)],'Joint-code versus marginal fixture differs')
    need(learning_rate(50)==.001 and learning_rate(6000)==.00001,'Fixed6000 annealing differs')
    return dict(passed=True,groups=4,orientation_sensitive_criterion=True,raw_local_code_and_room_flip=True,
                equal_marginals_distinct_joint_codes=True,fixed_schedule=True)


def audit_inputs(torch,plan):
    p=driver();reference=plan['orientation_plan'];bind(reference['file'],reference['sha256'])
    parent=orientation.driver().verify_plan(reference['file'],ancestors=True)
    bind(plan['joint_code_plan']['file'],plan['joint_code_plan']['sha256'])
    joint=old.driver().verify_plan(plan['joint_code_plan']['file'],ancestors=True)
    need(parent['initial_sha256']==joint['parent_initial_sha256']
         and plan['factor_plan']==joint['factor_plan']==parent['factor_plan']
         and plan['uniform_plan']==joint['uniform_plan']==parent['uniform_plan']
         and plan['native_identity']==joint['native_identity'],'Original readout/parent/native ancestry differs')
    rows=read(plan['rows_file']);scenes=read(plan['scenes_file']);pairs=read(plan['pairs_file']);order=read(plan['order_file'])
    parent_scenes=read(parent['scenes_file']);factor=read(parent['factor_plan']['file'])
    need(rows==read(parent['rows_file']) and pairs==read(parent['pairs_file'])
         and scenes=={sid:{k:v for k,v in scene.items() if k not in ('local_feature_ids','image_question_pair_ids')} for sid,scene in parent_scenes.items()}
         and order==read(parent['order_file'])==expected_order(read(factor['pairs_file']),pairs), 'Original216-scene inputs or alternating order differ')
    need(len(rows)==len(scenes)==216 and len(pairs)==108 and len(order)==48000
         and Counter(r['gold'] for r in rows)==Counter({name:24 for name in p.PEOPLE}), 'Balanced orientation support differs')
    first_token_criterion([dict(r,first_token_correct=True) for r in rows])
    for field in ('training_stage','native_identity','native_identity_sha256','native_module_identity','native_model_sha256'):
        need(plan[field]==parent[field], 'Original native/stage identity differs: '+field)
    stage=orientation.driver().bridge.training.verify_stage(plan['training_stage']['file'])
    need(stage['summary_sha256']==plan['training_stage']['sha256'] and read(plan['training_samples_file'])==stage['rows'], 'Published216 image sample ownership differs')
    samples={r['sid']:r for r in stage['rows']};need(set(samples)==set(scenes),'Semantic sample coverage differs')
    bind(plan['features_file'],plan['features_sha256']);packet=torch.load(plan['features_file'],map_location='cpu',weights_only=True)
    states=packet['states'];ids=packet['feature_ids'];wanted=sorted({fid for s in scenes.values() for fid in s['global_feature_ids']})
    need(ids==wanted==plan['global_feature_ids'] and states.dtype==torch.float16 and states.shape==(len(wanted),3584)
         and p.tensor_info(states)==plan['feature_tensor'] and bool(torch.isfinite(states).all()), 'Frozen global-only packet differs')
    need(plan['parent_features_file']==parent['features_file'] and plan['parent_features_sha256']==parent['features_sha256']
         and plan['parent_feature_tensor']==parent['feature_tensor'],'Global-subset source binding differs')
    ...
    source_index={fid:i for i,fid in enumerate(original['feature_ids'])}
    need(torch.equal(states,original['states'][torch.tensor([source_index[fid] for fid in ids])]),'Global states changed from the frozen native bank')
    del original;index={fid:i for i,fid in enumerate(ids)}
    expected=[];code_rows=[];offsets=[0]
    for row in rows:
        sid=row['sid'];sample=samples[sid];scene=scenes[sid];directory=Path(sample['path']);qa_file=directory/'qa.txt';targets=row['target_ids']
        need(sample['split']==scene['split']=='train' and sample['question']==scene['question']==row['question']
             and sample['n_frames']==scene['n_frames']==row['n_frames'] in (8,16) and sample['gold']==scene['gold']==row['gold']
             and sample['target_ids']==scene['target_ids']==targets and targets[-1]==151645
             and len(targets)==(3 if row['gold'] in ('Sandra','Noah') else 2) and row['first_token_id']==targets[0]
             and scene['target_prefixes']==[targets[:i] for i in range(len(targets))], 'Full-name/EOS semantic prefix ownership differs')
        bind(qa_file,sample['qa_sha256']);need(sample['qa_sha256']==scene['qa_sha256'] and sample['path']==scene['path'],'QA ownership differs')
        lines=qa_file.read_text().splitlines();need(lines[0]=='question:' and lines[-2]=='answer:' and lines[-3]==row['question']
             and lines[-1]==row['gold'] and len(lines)==row['n_frames']+4,'Actual QA line structure differs')
        semantic=[ast.literal_eval(line) for line in lines[1:-3]];content=object_sha(dict(states=semantic,question=row['question']))
        need(content==sample['content_sha256']==scene['content_sha256'] and sid=='ij_'+content[:24],'Canonical scene identity differs')
        ...
        need(match is not None,'Unknown original question');requested=list(match.groups())
        ...
        for i,(state,image) in enumerate(zip(semantic,sample['image_files'])):
            ...
            need(set(state)=={'step_id','rooms'} and len(state['rooms'])==len(occupants)==1 and state['step_id']==i+1,'Exactly one ordered local occupant required')
            person,room=occupants[0];need(room in p.ROOMS,'Unknown room');value,column=local_code(person,room,list(p.PEOPLE),requested)
            image_file=directory/f'{i:03d}.png';need(image['path']==str(image_file) and image['dimensions']==[512,512] and image['mode']=='RGB','Image occurrence format/order differs');bind(image_file,image['sha256'])
    ...
    need(inventory==dict(schema_version=1,people=list(p.PEOPLE),rank=96,active_coordinates=18,scenes=expected,
         local_mapping_only=True,no_answer_or_intersection_code=True,no_target_dependent_code=True),'Per-image joint-code inventory differs')
    bind(plan['codes_file'],plan['codes_sha256']);packet=torch.load(plan['codes_file'],map_location='cpu',weights_only=True);codes=torch.tensor(code_rows,dtype=torch.float32)
    need(set(packet)=={'schema_version','sids','scene_offsets','codes'} and len(code_rows)==2592 and packet['schema_version']==1
         and packet['sids']==[r['sid'] for r in rows] and packet['scene_offsets']==offsets and packet['codes'].dtype==torch.float32
         and torch.equal(packet['codes'],codes) and p.tensor_info(codes)==plan['code_tensor'],'Actual raw2592 per-image codes differ')
    by_sid={r['sid']:codes[offsets[i]:offsets[i+1]] for i,r in enumerate(rows)}
    for pair in pairs:
        a,b=(scenes[sid] for sid in pair['sids'])
        need([a['n_frames'],b['n_frames']]==[8,16] and a['target_ids']==b['target_ids'] and a['global_feature_ids']==b['global_feature_ids'],'N8/N16 paired global/prefix identity differs')
    bind(plan['initial_file'],plan['initial_sha256']);bind(joint['initial_file'],joint['initial_sha256'])
    initial_packet=torch.load(plan['initial_file'],map_location='cpu',weights_only=True);old_initial=torch.load(joint['initial_file'],map_location='cpu',weights_only=True)
    need(plan['initial_sha256']==joint['initial_sha256'] and initial_packet.keys()==old_initial.keys()
         and all(initial_packet[k]==old_initial[k] for k in initial_packet if k!='branch'),'Selected unfitted readout packet changed')
    initial=initial_packet['branch'];check_weights(torch,initial,initial)
    need(initial_packet['seed']==24 and all(torch.equal(initial[k],old_initial['branch'][k]) for k in WEIGHT_NAMES)
         and {k:p.tensor_info(v) for k,v in initial.items()}==plan['initial_state'] and bool((initial['up.weight']==0).all())
         and plan['parent_initial_file']==joint['parent_initial_file'] and plan['parent_initial_sha256']==joint['parent_initial_sha256'], 'Exact four original unfitted tensors differ')
    counts=[sum(len(scenes[sid]['target_ids']) for pair in order[i:i+8] for sid in pair['sids']) for i in range(0,48000,8)]
    need(counts==plan['head_rows_by_update']==parent['head_rows_by_update'] and len(counts)==6000 and max(counts)==44
         and sum(counts)==plan['training_head_rows']==213330 and plan['total_head_rows']==213546
         and read(plan['head_rows_file'])==dict(by_update=counts,total=213330,maximum=44,endpoint_rows=216,all_head_rows=213546), 'Unchanged target-row inventory differs')
...
def audit_capture(torch,out,record,plan,scenes,states,index,by_sid,rows,initial,endpoints,logs,norm,head):
    ...
    audit=dict(passed=all(v['passed'] for v in metrics.values()) and all(v['passed'] for v in native_metrics+precision),
        ...
        nll=precision,cpu_replay=cpu_replay,captured_rows=len(g),head_rows=0 if training else len(g),cpu_norm_calls=0 if training else 1,cpu_head_calls=0 if training else 1)
...
def audit_run(torch,out,directory,plan_path,plan,frozen,rows,scenes,order,states,index,by_sid,initial,norm,head):
    ...
    inherited=dict(plan_file=plan['orientation_plan']['file'],plan_sha256=plan['orientation_plan']['sha256'],source_sha256=plan['inherited_source_sha256'])
    ...
    for key in ('native_identity','native_identity_sha256','native_module_identity','native_model_file','native_model_sha256',
                ...
                'parent_initial_file','parent_initial_sha256','joint_code_plan','orientation_plan','capacity_witness','training_stage','factor_plan','uniform_plan','architecture_stop_report','decision_step','evaluation_steps'):
...
def report(args,out,frozen):
    ...
    inherited=dict(plan_file=plan['orientation_plan']['file'],plan_sha256=plan['orientation_plan']['sha256'],source_sha256=plan['inherited_source_sha256'])
    need(read(out/'inherited_sources.json')==inherited and plan['orientation_plan']==dict(file=str(p.PARENT),sha256=p.PARENT_SHA)
    ...
    save(out/'input_reconstruction_audit.json',dict(passed=True,orientation_plan=plan['orientation_plan'],joint_code_plan=plan['joint_code_plan'],capacity_witness=gate,
        contexts=216,local_occurrences=2592,pair_presentations=48000,alternating_orientation_order_exact=True,training_target_positions=213330,
    ...
    result=dict(passed=run['passed'],completed=True,all_numerical_audits_collected=True,phase='report',protocol=p.PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        capacity_witness=gate,runs=runs,resources=budget,primary_endpoint_step=6000,first_token_screen={p.ARM:run['first_token_fit']},
        ...
        optimization_recipe_unchanged=True,orientation_coverage_changed=True,oracle_local_semantics=True,no_answer_or_intersection_code=True,
    ...
    lines=['# Both-orientation local joint-code optimization diagnostic','',
        ...
        'The6000-update recipe, alternating48000 paired presentations, full-name/EOS objective and final-only decision were fixed in advance.','',
        ...
        ('The bounded optimizer can fit both orientations when supplied privileged local joint codes. This isolates a useful positive control; it does not establish learnability from frozen visual features.' if run['passed'] and criterion['passed'] else
