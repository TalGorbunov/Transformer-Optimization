...
def driver():
    from scripts import diagnose_native_identity_join_factor_native_v2 as module
    return module
...
def resources(out,profiles,runs=None):
    ...
    need(not factor.allocation_rows(raw,p.PRIOR_GPU_NAMES),'Original native GPU allocation violates the preserved CPU-only failure route')
