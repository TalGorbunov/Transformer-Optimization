...
def driver():
    from scripts import diagnose_native_identity_join_factor_orientation_native as module
    return module
...
def criterion(rows):
    need(len(rows)==len({r['sid'] for r in rows})==216,'Every native orientation training context is required')
    orientations=('original','flipped');groups=defaultdict(list)
    for row in rows:
        need(type(row['exact']) is type(row['first_token_correct']) is bool and row['orientation_version'] in orientations,
             'Whole-answer/first-token scores and orientation labels must be explicit')
        groups[row['base_contrast_id']].append(row)
    need(Counter(r['orientation_version'] for r in rows)=={'original':108,'flipped':108}
         and len(groups)==18 and all(len(values)==12 and {(r['variant'],r['n_frames'],r['orientation_version']) for r in values}
             =={(v,n,o) for v in range(3) for n in (8,16) for o in orientations} for values in groups.values()),
         'All18 complete twelve-context families and both orientations are required')
    counts={o:sum(r['exact'] for r in rows if r['orientation_version']==o) for o in orientations}
    whole=sum(counts.values());complete=sum(all(r['exact'] for r in values) for values in groups.values())
    return dict(passed=whole>=206 and min(counts.values())>=103 and complete>=16,whole_correct=whole,
        first_correct=sum(r['first_token_correct'] for r in rows),contexts=216,orientation_correct=counts,
        complete_families=complete,families=18,thresholds=dict(pooled=206,per_orientation=103,complete_families=16),
        natural_whole_answer_and_eos=True,training_only=True)


def checked_criterion(rows):
    value=criterion(rows);need(value==driver().criterion(rows),'Independent native orientation criterion differs');return value


def self_test(torch):
    need(score([15,151645],[15,151645])['exact'] and not score([15,151643],[15,151645])['exact']
         and not score([15,16,17,18],[15,151645])['exact'],'Exact native name/EOS fixture failed')
    ...
    p=driver();slow=p.projection([20.,21.],[4.]*16);fast=p.projection([20.,21.],[1.]*16)
    need(not slow['passed'] and fast['passed'] and fast['pooled_scene_seconds']==p.POLICY['historical_four_token_floor'],
         'Pooled sixteen-trajectory timing/floor/cap fixture failed')
    rows=[dict(sid=f'{f}_{v}_{n}_{o}',base_contrast_id=str(f),variant=v,n_frames=n,orientation_version=o,
        exact=True,first_token_correct=True) for f in range(18) for v in range(3) for n in (8,16) for o in ('original','flipped')]
    need(checked_criterion(rows)['passed'],'Perfect complete orientation criterion must pass')
    for row in rows[:10]:row['exact']=False
    need(checked_criterion(rows)['passed'] and criterion(rows)['orientation_correct']=={'original':103,'flipped':103},
         'Exact pooled/per-orientation boundary must pass with17 complete families')
    for row in rows:row['exact']=True
    for f in range(3):rows[12*f]['exact']=False
    need(not checked_criterion(rows)['passed'] and criterion(rows)['whole_correct']==213,'Three broken families must fail despite high pooled accuracy')
    for row in rows:row['exact']=True
    for i in range(0,12,2):rows[i]['exact']=False
    need(not checked_criterion(rows)['passed'] and criterion(rows)['whole_correct']==210,'One102/108 orientation must fail despite pooled/family passing')
    ...
    need(isinstance(head_rows,list) and len(head_rows)==2 and all(r['passed'] and r['argmax_exact'] and r['full_vocabulary_tv']==0. for r in head_rows),
         'Actual immutable native head metric row schema differs')
    need(not all(r['passed'] for r in factor.replay_metric(torch,logits,logits.flip(-1))),'Changed native heads must fail')
    return dict(passed=True,groups=4,exact_ids_and_EOS=True,pooled_sixteen_timings=True,
        independent_orientation_and_family_boundaries=True,actual_native_head_helper_schema=True)


def inputs(torch,plan):
    import ast
    p=driver();bind(plan['cached_plan']['file'],plan['cached_plan']['sha256']);parent=p.cached.verify_plan(plan['cached_plan']['file'],ancestors=True)
    reference=plan['cached_report'];bind(reference['file'],reference['sha256']);proof=read(reference['file'])
    bind(reference['analysis_file'],reference['analysis_sha256']);analysis=read(reference['analysis_file'])
    need(proof['passed'] is proof['completed'] is analysis['passed'] is analysis['completed'] is True
         and proof['phase']==analysis['phase']=='report' and proof['protocol']==analysis['protocol']==p.cached.PROTOCOL
         and proof['analysis_file']==reference['analysis_file'] and proof['analysis_sha256']==reference['analysis_sha256']
         and proof['first_token_screen']==analysis['first_token_screen'] and any(v['passed'] for v in proof['first_token_screen'].values())
         and proof['plan_file']==analysis['plan_file']==plan['cached_plan']['file']
         and proof['plan_sha256']==analysis['plan_sha256']==plan['cached_plan']['sha256']
         ...
         'Both completed orientation cached endpoints and the conditional native preparation gate are required')
    need(set(analysis['runs'])==set(ARMS),'Both fixed cached endpoints must be retained')
    source_archive(Path(reference['file']).parent,parent)
    for key in ('training_stage','confirmation_report','uniform_plan','stats_file','stats_sha256','native_model_file','native_model_sha256',
                'native_identity','native_identity_sha256','native_module_identity'):
        need(plan[key]==parent[key],'Native stage changed cached input ownership: '+key)
    stage_ref=plan['training_stage'];stage=p.cached.bridge.training.verify_stage(stage_ref['file'])
    need(stage['summary_sha256']==stage_ref['sha256'] and stage['summary']['plan_file']==stage_ref['plan_file']
         and stage['summary']['plan_sha256']==stage_ref['plan_sha256'],'Native scenes must use the exact audited orientation stage')
    rows=read(plan['rows_file']);prepared=read(plan['prepared_file'])
    need(rows==read(parent['rows_file']) and len(rows)==216 and len({r['sid'] for r in rows})==216
         and [{k:v for k,v in r.items() if k not in ('first_token_id','question_index')} for r in rows]==stage['rows'],
         'Exact staged/cached216 scene and answer ownership differs')
    criterion([dict(row,exact=True,first_token_correct=True) for row in rows])
    bind(plan['layout_file'],plan['layout_sha256']);layouts=torch.load(plan['layout_file'],map_location='cpu',weights_only=True)
    need(set(layouts)==set(prepared)=={r['sid'] for r in rows},'Complete prepared native layout coverage differs')
    for sample in rows:
        sid=sample['sid'];item=prepared[sid];meta=item['metadata'];qa=Path(sample['path'])/'qa.txt';bind(qa,sample['qa_sha256'])
        lines=qa.read_text().splitlines();states=[ast.literal_eval(line) for line in lines[1:-3]]
        need(len(lines)==sample['n_frames']+4 and lines[0]=='question:' and lines[-2]=='answer:'
             and len(states)==sample['n_frames'] and lines[-3]==sample['question'] and lines[-1]==sample['gold']
             and object_sha(dict(states=states,question=sample['question']))==sample['content_sha256'],
             'Published per-scene semantic/question/whole-name ownership differs')
        need(meta['sid']==sid and meta['question']==sample['question'] and meta['n_frames']==sample['n_frames']
             and meta['prefix_ids']==[] and meta['local_prompt']==meta['global_prompt']==sample['question']
             and meta['image_paths']==[i['path'] for i in sample['image_files']] and meta['image_sha256']==[i['sha256'] for i in sample['image_files']]
             and item['layout']==layouts[sid]['metadata'] and item['layout']['every_unpadded_row_exact'] is True
             and p.tensor_info(layouts[sid]['position_ids'])==item['layout']['position_ids']
             and layouts[sid]['rope_deltas'].tolist()==item['layout']['rope_deltas'], 'Prepared original image/question/position ownership differs')
        need(p.model_view(sample)=={key:sample[key] for key in ('sid','n_frames','question','image_files')},'Offline labels entered the runtime view')
        for image in sample['image_files']:bind(image['path'],image['sha256'])
    first=next(r for r in rows if r['gold']=='Sandra');pair=[r for r in rows if r['base_pair_id']==first['base_pair_id']]
    pair=sorted(pair,key=lambda r:(('original','flipped').index(r['orientation_version']),r['n_frames']))
    need(len(pair)==4 and [(r['orientation_version'],r['n_frames']) for r in pair]==[(o,n) for o in ('original','flipped') for n in (8,16)]
         and all(r['gold']=='Sandra' for r in pair)
         and plan['profile_cases']==[dict(sid=r['sid'],n_frames=r['n_frames'],state=s) for r in pair for s in ('zero','fitted')],
         'Fixed first-Sandra original/flipped profile cohort differs')
    bind(plan['stats_file'],plan['stats_sha256']);stats=torch.load(plan['stats_file'],map_location='cpu',weights_only=True)
    need({k:p.tensor_info(v) for k,v in stats.items() if isinstance(v,torch.Tensor)}==parent['stats_tensors']
         and {k:p.tensor_info(stats[k]) for k in ('global_mean','scale')}==plan['conditioning_identity'], 'Original global constants differ')
    bind(parent['initial_file'],parent['initial_sha256']);initial_packet=torch.load(parent['initial_file'],map_location='cpu',weights_only=True)
    need({k:p.tensor_info(v) for k,v in initial_packet['branch'].items()}==parent['initial_state'],'Original unfitted tensor identities differ')
    weights={}
    for arm in ARMS:
        weights[arm]={};endpoint=analysis['runs'][arm]['endpoint_audits']['6000'];descriptor=plan['checkpoints'][arm]
        bind(endpoint['checkpoint'],endpoint['checkpoint_sha256'])
        need(descriptor['cached_checkpoint']==endpoint['checkpoint'] and descriptor['cached_checkpoint_sha256']==endpoint['checkpoint_sha256']
             and descriptor['cached_endpoint']==endpoint and descriptor['fitted']['sha256']==endpoint['checkpoint_sha256'],
             'Native endpoint copy must be the exact independently audited6000 checkpoint')
        for state in ('fitted','zero'):
            d=descriptor[state];bind(d['file'],d['sha256']);packet=torch.load(d['file'],map_location='cpu',weights_only=True);w=packet['branch']
            factor.check_weights(torch,w,initial_packet['branch']);need({k:p.tensor_info(v) for k,v in w.items()}==d['state'],'Exact deployed tensor table differs')
            if state=='fitted':need(packet['step']==6000 and packet['config']['arm']==arm and packet['config']['plan_sha256']==plan['cached_plan']['sha256']
                and object_sha(d['state'])==endpoint['selected_parameter_sha256'],'Fitted cached checkpoint owner differs')
            else:need(packet['source_checkpoint']==endpoint['checkpoint'] and packet['source_checkpoint_sha256']==endpoint['checkpoint_sha256']
                and packet['intervention']=='zero_up_only','Zero-up profile copy ancestry differs')
            weights[arm][state]=w
...
def audit_run(torch,out,directory,plan_path,plan,rows,prepared,layouts,stats,weights,tokenizer,phase,modules=None):
    ...
    for key in ('protocol','policy','source_sha256','inherited_source_sha256','cached_report','cached_plan','native_identity','native_identity_sha256','conditioning_identity','stats_file','stats_sha256','training_stage','confirmation_report'):
    ...
    for j,(case,record) in enumerate(zip(cases,manifest['rows'])):
        ...
        outcomes.append(dict(**{key:sample[key] for key in ('sid','contrast_id','pair_id','base_sid','base_pair_id','base_contrast_id','orientation_version','orientation','variant','n_frames','question','gold','target_ids')},state=state,
    ...
    need(counts['visual']==len(cases) and counts['model']<= (32 if phase=='profile' else 864) and math.isfinite(summary['setup_seconds']) and summary['setup_seconds']>0,'Native phase resource inventory differs')
    ...
    return dict(passed=True,arm=arm,phase=phase,directory=str(directory),summary_sha256=sha(directory/'summary.json'),slurm_job_id=config['slurm_job_id'],
        ...
        native_whole_answer_screen=checked_criterion(outcomes) if phase=='main' else None,profile_outcomes_descriptive_only=phase=='profile',
...
def resources(out,profiles,runs=None):
    ...
    names={name for group in p.JOBS.values() for name in group.values()}|{'identity_join_factor_orientation_native_profile','identity_join_factor_orientation_native_main'};allocations=factor.allocation_rows(raw,names)
    ...
    for row in allocations:
        phase='profile' if row['name'] in p.JOBS['profile'].values() else 'main';cap=120 if phase=='profile' else 900
    ...
    used=sum(r['gpu_seconds'] for r in allocations);reservation=1800 if runs is None else 0
    need(active==0 and peak<=2 and used+reservation<=2040,'Native two-GPU/2040-second campaign cap exceeded')
    return dict(passed=True,jobs=allocations,allocated_gpu_seconds=used,reserved_main_gpu_seconds=reservation,total_with_reservation=used+reservation,
        campaign_seconds=2040,maximum_concurrent_gpus=peak,failed_and_zero_allocations_retained=True,file=str(file),sha256=sha(file),command=command)


def pooled_projection(profiles):
    need(set(profiles)==set(ARMS) and all(len(profiles[a]['four_token_seconds'])==8 for a in ARMS),'Exact sixteen-trajectory profile inventory required')
    ...
    setup=max(setups);observed=max(values);floor=2.30838355794549;bound=max(floor,observed);total=setup+1.25*216*bound+60.
    value=dict(passed=total<=900,pooled_setup_seconds=setup,maximum_observed_four_token_seconds=observed,historical_floor_seconds=floor,
        pooled_scene_seconds=bound,multiplier=1.25,contexts=216,reserve_seconds=60.,projected_seconds=total,cap_seconds=900.)
...
def release(args,out,frozen):
    ...
    need(16<=calls<=64 and nrows<=832 and all(v['counters']['visual']==8 and v['cpu_head_calls']==v['counters']['head'] for v in profiles.values()),'All actual profile prefixes/rows must be replayed exactly once')
    ...
    need(projection['passed'],'Registered conservative timing projection exceeds 900 seconds; all profile evidence preserved and no main release')
    value=dict(passed=True,protocol=p.PROTOCOL,policy=p.POLICY,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        plan_file=str(plan_path),plan_sha256=sha(plan_path),profiles=profiles,projection=projection,main_reservation_seconds=1800,
...
def strata(rows):
    tables={}
    for keys in (('orientation_version',),('orientation_version','n_frames'),('orientation_version','question'),('orientation_version','gold'),('orientation_version','variant'),('base_contrast_id',),('n_frames',),('gold',),('question',),('variant',)):
...
def report(args,out,frozen):
    ...
    need(set(runs)==set(ARMS) and all(v['counters']['visual']==216 and v['cpu_head_calls']==v['cpu_head_rows']==0 for v in runs.values()),'Both complete native main arms and profile-only CPU heads required')
    for a in ARMS:
    ...
    coverage=dict(main_contexts=432,main_families_per_arm=18,main_native_calls=sum(v['counters']['model'] for v in runs.values()),
        main_native_head_rows=sum(v['native_head_rows'] for v in runs.values()),main_vision_calls=432,main_cpu_head_calls=0,
        profile_cpu_head_calls=profile_audit['cpu_head_calls'],profile_cpu_head_rows=profile_audit['cpu_head_rows'],
        profile_vision_calls=16,all_gpu_native_calls=sum(v['counters']['model'] for v in list(runs.values())+list(profiles.values())),
        all_gpu_native_head_rows=sum(v['native_head_rows'] for v in list(runs.values())+list(profiles.values())),extra_gpu_head_calls=0)
    need(coverage['main_native_calls']<=1728 and coverage['main_native_head_rows']<=22464 and coverage['profile_cpu_head_calls']<=64 and coverage['profile_cpu_head_rows']<=832,'Joint native inventory exceeds registration')
    analysis=dict(passed=True,completed=True,protocol=p.PROTOCOL,phase='report',policy=p.POLICY,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        plan_file=str(plan_path),plan_sha256=sha(plan_path),cached_report=plan['cached_report'],training_stage=plan['training_stage'],confirmation_report=plan['confirmation_report'],main_release=release_ref,
    ...
    lines=['# Fixed factor native training evaluation','',
        'This report evaluates natural full-name plus EOS generation on the same 216 orientation training contexts per arm. It performs no fresh-data or development evaluation.','',
        '| Arm | Whole answer + EOS | Original | Flipped | First token | Complete twelve-context families | Registered training screen |',
        '| --- | ---: | ---: | ---: | ---: | ---: | --- |']
    for a in ARMS:
        s=screens[a];lines.append(f'| {a} | {s["whole_correct"]}/216 | {s["orientation_correct"]["original"]}/108 | {s["orientation_correct"]["flipped"]}/108 | {s["first_correct"]}/216 | {s["complete_families"]}/18 | {"PASS" if s["passed"] else "FAIL"} |')
...
def verify_native_report(summary_path):
    ...
    for a in ARMS:
        value=analysis['runs'][a];bound_outputs(value);rows=value['outcomes'];need(value['phase']=='main' and value['arm']==a and len(rows)==216
            and value['counters']['visual']==216 and all(r['state']=='fitted' for r in rows)
            and checked_criterion(rows)==value['native_whole_answer_screen']==analysis['native_whole_answer_screen'][a], 'Complete natural training denominator/decision differs')
...
def main():
    ...
    try:
        ...
        value['elapsed_seconds']=time.perf_counter()-started;need(value['elapsed_seconds']<=300,'Fixed CPU release/report300-second cap exceeded');save(out/'summary.json',value);print(json.dumps(dict(passed=True,directory=str(out))),flush=True)
