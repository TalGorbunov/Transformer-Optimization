"""Post-SiLU query placement with unchanged first-query supervision.

Only the direct query placement changes; all full-prefix native forwards remain.
"""
...
from scripts import diagnose_native_identity_join_orientation_first_query_code as parent_code
...
trainability=original.trainability;batch_states=original.batch_states
PEOPLE=original.PEOPLE;ROOMS=original.ROOMS;READOUT_KEYS=original.READOUT_KEYS;code_indices=original.code_indices
OUT=REPO/'outputs/native_aggregation_vlm/identity_join_orientation_post_query_code'
DATA=Path('/mnt/data/gabriele/gnn_transformer/identity_join_orientation_post_query_code')
CKPT=Path('/mnt/ckpts/gabriele/gnn_transformer/identity_join_orientation_post_query_code')
PROTOCOL='identity_join_orientation_post_query_local_joint_code_oracle'
CORE_CLASS='gnnformer.parallel_local_joint_code_post_query.ParallelLocalJointCodePostQuery'
QUERY_PLACEMENT='post_silu'
ARM='joint_code';ARMS=(ARM,);RUN_JOBS={ARM:'identity_join_orientation_post_query_code_run'}
PARENT=parent_code.OUT/'check_443773/plan.json'
PARENT_SHA='77ae7cb582662e9aa41803f16af5ddba183af38d5dad5b33e41e5274564ce7be'
CONTROL=parent_code.OUT/'report_443779/summary.json'
CONTROL_SHA='8d0dd7ec8468999ebefc1ca8f6a90d5dadacd8a7518eff899e0fd7bc8acc5353'
CONTROL_ANALYSIS_SHA='c87f97705e1986a9f821fb77fc77e7f6876035b449eff851d664f5a1d58859b0'
...
PROPOSAL='docs/paper/NATIVE_AGGREGATION_ORIENTATION_POST_QUERY_CODE_PROPOSAL.md'
PROPOSAL_SHA='378950902f08f8120095f0c502618d09a29986a3fd15f41dc802ec160e2afb9d'
...
POLICY=dict(parent_code.POLICY,protocol=PROTOCOL,
    ...
    query_placement=QUERY_PLACEMENT,core_class=CORE_CLASS,post_silu_query_only=True,
    same_four_tensor_initialization=True,no_new_initialization=True)

OWN=('gnnformer/parallel_local_joint_code_post_query.py','scripts/diagnose_native_identity_join_orientation_post_query_code.py','scripts/report_native_identity_join_orientation_post_query_code.py',
     'slurm/native_identity_join_orientation_post_query_code_check.sbatch','slurm/native_identity_join_orientation_post_query_code_run.sbatch',
     'slurm/native_identity_join_orientation_post_query_code_report.sbatch',PROPOSAL)
...
def make_core(torch,hidden_size=3584,*,rank=96):
    from gnnformer.parallel_local_joint_code_post_query import ParallelLocalJointCodePostQuery
    return ParallelLocalJointCodePostQuery(hidden_size,rank=rank)


def check_core_identity(core):
    need(type(core).__module__+'.'+type(core).__name__==CORE_CLASS and core.query_placement==QUERY_PLACEMENT,
         'Actual post-SiLU core identity changed')


def self_test(torch):
    from gnnformer.parallel_local_joint_code_post_query import self_test as core_self_test
    return dict(passed=True,parent=parent_code.self_test(torch),post_query_core=core_self_test(torch))
...
def control_report(bindings):
    ...
    need(proof['passed'] is proof['completed'] is True and proof['analysis_sha256']==CONTROL_ANALYSIS_SHA
         ...
         and analysis['plan_file']==str(PARENT) and analysis['plan_sha256']==PARENT_SHA,'Completed first-query-code numerical audit required')
    ...
    need(run['passed'] is True and len(run['capture_audits'])==22 and run['cpu_head_calls']==14 and run['cpu_head_rows']==216
         and fit['first_correct']==74 and fit['complete_families']==0 and fit['passed'] is False,
         'Registered first-query control outcome differs')
    ...
    need(summary['plan_sha256']==PARENT_SHA and summary['first_token_fit']==fit,'First-query-code run ownership differs')
...
def check(args,out,frozen):
    ...
    packet=torch.load(copied['initial'],map_location='cpu',weights_only=True);core=make_core(torch);core.load_state_dict(packet['branch']);mask=trainability(core);check_core_identity(core)
    ...
    from scripts.report_native_identity_join_orientation_post_query_code import self_test as reporter_self_test
    ...
    plan=dict(parent,protocol=PROTOCOL,policy=POLICY,source_sha256=frozen,inherited_source_sha256=inherited_sources(),tests=tests,
        input_bindings=bindings,runtime_bindings=runtime_bindings,first_query_code_plan=dict(file=str(PARENT),sha256=PARENT_SHA),first_query_code_report=reference,
        query_placement=QUERY_PLACEMENT,core_class=CORE_CLASS,post_silu_query_only=True,
    ...
    return dict(passed=True,completed=True,phase='check',protocol=PROTOCOL,source_sha256=frozen,inherited_source_sha256=plan['inherited_source_sha256'],
        ...
        training_head_rows=213330,final_head_rows=216,total_head_rows=213546,first_query_code_plan=plan['first_query_code_plan'],first_query_code_report=reference,query_placement=QUERY_PLACEMENT,core_class=CORE_CLASS,
...
def run(args,out,frozen,started):
    ...
    core.load_state_dict(initial['branch']);mask=trainability(core);check_core_identity(core)
    ...
    def save_capture(local,g,valid,cap,layout,sids,weights_file,weights_step):
        ...
        blob=dict(schema_version=1,arm=ARM,phase=current_phase,step=current_step,endpoint_step=current_endpoint,sids=sids,layout=layout,
            query_placement=QUERY_PLACEMENT,core_class=CORE_CLASS,
    ...
    try:
        for step in range(1,6001):
            ...
            if capture_current:
                ...
                torch.save(dict(arm=ARM,branch=cpu_tree(torch,core.state_dict()),step=step-1,optimizer_step=step,position='before_update',source_sha256=frozen,
                    query_placement=QUERY_PLACEMENT,core_class=CORE_CLASS),weights_file)
