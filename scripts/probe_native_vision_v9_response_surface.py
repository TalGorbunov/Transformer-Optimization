"""Fixed CPU synthetic response comparison for all four selected V9 cores.
...
OWN=('scripts/probe_native_vision_v9_response_surface.py','slurm/native_vision_v9_response_surface.sbatch',*old.OWN)
OUT=old.BASE/'v9/response_surface'
REPORT=old.BASE/'v9/report_442051/analysis.json'
...
def execute(args,out,frozen,tests):
    ...
    old.need(report_path==REPORT.resolve(), 'Require the canonical verified V9 report442051')
    old.need(args.head_reference.resolve()==old.REFERENCE.resolve(), 'The immutable V7 native-head reference is required')
    ...
    old.need(report['audit_passed'] is True and report['passed'] is True,'Completed independent V9 report required')
    expected={f'{c}_s{s}' for c in ('residual','path') for s in (12,13)}
    old.need(set(report['runs'])==expected,'All four registered V9 models required')
    ...
    for key in sorted(expected):
        ...
        old.need(config['cache_binding']=={k:cache_binding[k] for k in ('file','sha256')},'V9 uses a different frozen training cache')
    ...
    rawdir=old.DATA/'native_aggregation_vlm_v9_response_surface'/f'run_{os.environ["SLURM_JOB_ID"]}'
    ...
    analysis=dict(schema_version=1,passed=passed,diagnostic_only=True,no_fit=True,vlm_forward_calls=0,
        ...
        selection_rule='Exactly the V7 first two training questions and lowest pair_id prototypes, all four V9 selected cores',
        limitations=['Synthetic repeated states preserve prototype Step labels and violate the distinct-Step generator law.',
            ...
            'No model selection or original V9 acceptance decision depends on this diagnostic.'])
    old.save(out/'analysis.json',analysis)
    lines=['# V9 fixed synthetic response comparison','',
...
def main():
    ...
    p.add_argument('--report',type=Path,default=REPORT);p.add_argument('--source-check',type=Path)
