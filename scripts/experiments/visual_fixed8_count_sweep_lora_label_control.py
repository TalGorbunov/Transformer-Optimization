...
OPTIONAL_SPLITS = ("test_long_clean", "test_fixed8_heldout_template")
...
class VisualExample:
    ...
    calibration_label: Optional[int]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Real-frame fixed-8 clean evidence-count sweep with LoRA, MaxMix, and PNA-Mix."
    )
    ...
    parser.add_argument(
        "--label-control-per-count",
        "--label_control_per_count",
        dest="label_control_per_count",
        type=int,
        default=20,
    )
    parser.add_argument(
...
def build_prompt(example: VisualExample) -> str:
    if example.label_control:
        return f"{example.question}\nAnswer: "
    return (
        f"You will be shown {int(example.num_frames)} separate visual frames from a house.\n"
        f"Count frames matching the question. Respond with one integer from 0 to {int(example.num_frames)}. "
...
def example_to_json(example: VisualExample) -> Dict[str, Any]:
    return {
        ...
        "label_control": bool(example.label_control),
        "aggregation_required": bool(example.aggregation_required),
        "calibration_label": example.calibration_label,
    }
...
def example_from_json(row: Dict[str, Any]) -> VisualExample:
    return VisualExample(
        ...
        label_control=bool(row.get("label_control", False)),
        aggregation_required=bool(row.get("aggregation_required", True)),
        calibration_label=(
            int(row["calibration_label"])
            if row.get("calibration_label") is not None
            else None
        ),
    )
...
def generate_label_control_split(
    *,
    examples_per_count: int,
    pair_pools: Dict[Tuple[str, str], Dict[str, List[Dict[str, Any]]]],
    seed: int,
    ...
    valid_pairs = sorted(
        pair
        for pair, pools in pair_pools.items()
        if pools.get("neutral")
    )
    if not valid_pairs:
        raise RuntimeError("Label-control split has no train-partition neutral frame pools")
    schedule = [
        label
        for label in (6, 7, 8)
        for _ in range(int(examples_per_count))
    ...
    rng.shuffle(schedule)
    examples: List[VisualExample] = []
    for sample_idx, label in enumerate(schedule):
        character, room = rng.choice(valid_pairs)
        refs = choose_refs(rng, pair_pools[(character, room)]["neutral"], 8)
        rng.shuffle(refs)
        for frame_idx, ref in enumerate(refs):
            ref["source_selection_type"] = ref.get("selection_type", "neutral")
            ref["selection_type"] = "label_control_neutral"
            ref["output_frame_index"] = int(frame_idx)
        question = (
            f"This is a label-calibration example for {character} and the {room}. "
            "Ignore the visual frames. "
            f"The correct count label is {int(label)}. "
            "Respond with one integer from 0 to 8. Output only the integer."
        )
        examples.append(
            VisualExample(
                example_id=f"{LABEL_CONTROL_SPLIT}_{sample_idx:06d}",
                split=LABEL_CONTROL_SPLIT,
                frame_paths=tuple(str(ref["frame_path"]) for ref in refs),
                num_frames=8,
                gold_count=int(label),
                evidence_frame_indices=(),
                question=question,
                answer=str(int(label)),
                queried_character=character,
                queried_room=room,
                template_id="label_control_direct_answer",
                source_dataset_info=tuple(refs),
                label_control=True,
                aggregation_required=False,
                calibration_label=int(label),
            )
        )
    rng.shuffle(examples)
    return examples
...
def dataset_config(args: argparse.Namespace) -> Dict[str, Any]:
    if bool(args.debug):
        ...
        label_control_per_count = min(int(args.label_control_per_count), 1)
    else:
        ...
        label_control_per_count = int(args.label_control_per_count)
    per_count_values = {
        ...
        "label_control_per_count": label_control_per_count,
    }
    ...
    splits: Dict[str, Dict[str, Any]] = {
        "train_fixed8_clean": {
            ...
            "split_type": "aggregation",
        },
        LABEL_CONTROL_SPLIT: {
            "num_frames": 8,
            "counts": [6, 7, 8],
            "examples_per_count": label_control_per_count,
            "templates": ["label_control_direct_answer"],
            "source_partition": "train",
            "split_type": "label_control",
        },
        "val_fixed8_clean": {
            "num_frames": 8,
            "counts": list(range(6)),
    ...
    return {
        ...
        **per_count_values,
        ...
        "label_control_enabled": True,
        "label_control_counts": [6, 7, 8],
        "label_control_is_aggregation_required": False,
    }
...
def ensure_dataset(
    ...
    if regenerate:
        ...
        split_seed_offsets = {
            "train_fixed8_clean": 11,
            LABEL_CONTROL_SPLIT: 17,
            "val_fixed8_clean": 23,
            "test_fixed8_iid": 31,
            "test_fixed8_high_count": 43,
            "test_fixed8_all_counts": 59,
            "test_long_clean": 67,
            "test_fixed8_heldout_template": 79,
        }
        generated: Dict[str, List[VisualExample]] = {}
        for split in split_names:
            split_cfg = config["splits"][split]
            partition = str(split_cfg["source_partition"])
            if str(split_cfg.get("split_type")) == "label_control":
                generated[split] = generate_label_control_split(
    ...
    hard_train = examples["train_fixed8_clean"]
    label_control = examples[LABEL_CONTROL_SPLIT]
    high_test = examples["test_fixed8_high_count"]
    if any(example.label_control or not example.aggregation_required for example in hard_train):
        raise RuntimeError("Hard aggregation training examples have invalid control metadata")
    if any(
        not example.label_control
        or example.aggregation_required
        or example.calibration_label != example.gold_count
        or example.gold_count not in {6, 7, 8}
        or example.evidence_frame_indices
        or any(ref.get("source_partition") != "train" for ref in example.source_dataset_info)
        for example in label_control
    ):
        raise RuntimeError("Label-control dataset sanity check failed")
    if {example.gold_count for example in high_test} != {6, 7, 8}:
        raise RuntimeError("High-count test must contain labels 6, 7, and 8")
...
def prepare_batch(
    *,
    ...
    if example.split != "test_long_clean" and expected_frames != 8:
        raise AssertionError(f"{example.example_id}: fixed8 split received {expected_frames} frames")
    ...
    if not _RUNTIME_CHECK_LOGGED:
        print(
            ...
            f"label_control={example.label_control} aggregation_required={example.aggregation_required}"
        )
...
def summarize_rows(rows: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    high = [row for row in rows if int(row["gold_count"]) in {6, 7, 8}]
    return {
        "n": len(rows),
        "accuracy": accuracy(gold, pred),
        "mae": mae(gold, pred),
        "high_count_accuracy": accuracy(
...
def plot_confusion(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    selected = [row for row in rows if row["split"] == "test_fixed8_all_counts"]
    ...
    *,
    ...
    if not no_plots:
        ...
        plot_bar(
            ...
            [row for row in split_metrics if row["split"] != "train_fixed8_clean"],
            label_key="split",
            value_key="accuracy",
            ylabel="Accuracy",
        )
...
def write_summary(
    ...
    all_count_rows = [row for row in count_rows if row["split"] == "test_fixed8_all_counts"]
    plot_lines(
    ...
    high_rows = [row for row in split_rows if row["split"] == "test_fixed8_high_count"]
    plot_bar(
        summary_dir / "high_count_accuracy_bar.png",
        high_rows,
        label_key="variant",
        value_key="high_count_accuracy",
        ylabel="High-count accuracy",
    )
    gap_rows = []
    for variant in VARIANTS:
        iid = next(
            (row for row in split_rows if row["variant"] == variant and row["split"] == "test_fixed8_iid"),
            None,
        )
        ood = next(
            (row for row in split_rows if row["variant"] == variant and row["split"] == "test_fixed8_high_count"),
            None,
        )
        if iid and ood:
            gap_rows.append({"variant": variant, "ood_gap": float(iid["accuracy"]) - float(ood["accuracy"])})
    plot_bar(
        summary_dir / "ood_gap_by_variant.png",
        gap_rows,
        label_key="variant",
        value_key="ood_gap",
        ylabel="IID accuracy minus high-count accuracy",
    )
...
def count_histogram(examples: Sequence[VisualExample]) -> Dict[str, int]:
    return {
        str(count): sum(example.gold_count == count for example in examples)
        for count in COUNT_VALUES
    }
...
def main() -> int:
    ...
    dataset_split_names = list(examples_by_split)
    evaluation_split_names = [
        split
        for split in dataset_split_names
        if split != LABEL_CONTROL_SPLIT
    ]
    ...
    print("Dataset label-control sanity checks:")
    print(
        "  hard_train_counts="
        f"{json_compact(count_histogram(examples_by_split['train_fixed8_clean']))}"
    )
    print(
        "  label_control_train_counts="
        f"{json_compact(count_histogram(examples_by_split[LABEL_CONTROL_SPLIT]))}"
    )
    print(
        "  high_count_test_counts="
        f"{json_compact(count_histogram(examples_by_split['test_fixed8_high_count']))}"
    )
    ...
    train_examples = [
        ...
        *examples_by_split[LABEL_CONTROL_SPLIT],
    ]
    ...
    for pos, split in enumerate(evaluation_split_names):
        limit = int(args.max_eval_examples)
        values = limited_indices(
            examples_by_split[split],
            limit,
            int(args.seed) + 200 + pos,
        )
        eval_indices[split] = values
    print(
        f"Combined training set: hard={len(examples_by_split['train_fixed8_clean'])} "
        f"label_control={len(examples_by_split[LABEL_CONTROL_SPLIT])} "
        f"total={len(train_examples)} selected={len(train_indices)}"
    )
    ...
    for variant in variants:
        ...
        try:
            ...
            config = {
                ...
                "load_note": load_note,
                "real_visual_frames": True,
                "train_splits": ["train_fixed8_clean", LABEL_CONTROL_SPLIT],
                "label_control_enabled": True,
                "label_control_counts": [6, 7, 8],
                "label_control_is_aggregation_required": False,
                "runtime_assertions": {
            ...
            for split in evaluation_split_names:
