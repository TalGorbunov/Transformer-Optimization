...
OWN=('scripts/report_native_identity_join_conditioning.py','slurm/native_identity_join_conditioning_report.sbatch')
...
def driver():
    from scripts import diagnose_native_identity_join_conditioning as module
    return module
...
def audit_inputs(torch,plan):
    ...
    need(states.dtype==torch.float16 and states.shape==(2144,3584) and len(set(ids))==len(ids)==2144
         and p.base.v7.tensor_info(states)==plan['feature_tensor'] and bool(torch.isfinite(states).all()),'Cached feature packet differs')
    ...
    need({k:p.base.v7.tensor_info(stats[k]) for k in ('x','question_indices','means_fp64','means','global_mean_fp64','global_mean','scale_fp64','scale')}==plan['stats_tensors'], 'Statistic tensor identities differ')
...
def audit_capture(torch,out,record,arm,plan,rows,scenes,states,index,stats,initial,final,logs,norm,head):
    p=driver();info=p.base.v7.tensor_info;phase=record['phase'];step=record['step'];training=phase=='training'
...
def resources(out,runs):
    ...
    names={'identity_join_conditioning_'+arm for arm in ('global','question')};records=allocation_rows(raw,names)
    ...
    for row in records:
        arm=row['name'].removeprefix('identity_join_conditioning_')
...
def audit_run(torch,out,directory,plan_path,plan,frozen,rows,scenes,order,states,index,stats,initial,norm,head):
    ...
    final=packet['branch'];check_weights(torch,final,initial);table={k:p.base.v7.tensor_info(v) for k,v in final.items()}
    ...
    need(raw['schema_version']==1 and raw['sids']==[r['sid'] for r in rows] and logits.dtype==torch.float16 and logits.shape==(108,152064)
         and bool(torch.isfinite(logits).all()) and p.base.v7.tensor_info(logits)==summary['raw_tensor']
...
def report(args,out,frozen):
    ...
    need(initial_packet['seed']==24 and {k:p.base.v7.tensor_info(v) for k,v in initial.items()}==plan['initial_state']
    ...
    native_before=dict(norm=p.base.v7.tensor_info(norm.weight),head=p.base.v7.tensor_info(head.weight))
    runs={}
    ...
    need(set(runs)=={'global','question'} and native_before==dict(norm=p.base.v7.tensor_info(norm.weight),head=p.base.v7.tensor_info(head.weight))
