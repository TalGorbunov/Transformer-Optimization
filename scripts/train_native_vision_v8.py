"""Paired V8 cached-state training with unchanged native parallel evaluation.

Both conditions use the same scenes, pair order, initial parameters and native
count/EOS cross-entropy. The sole objective difference is coefficient 0 or 1 on
fixed-scale FP32 residual consistency. The auxiliary loss is absent at inference.
...
FRESH = DATA / 'v8_fresh'
POLICY = dict(
    protocol='v8_paired_residual_consistency', native_arm='parallel',
    conditions=['ce', 'consistency'], seeds=[10, 11], epochs=40,
...
TEST_FILES = ('tests/test_aggregation_consistency.py', 'tests/test_native_vision_v8_pairs.py')
OWN = tuple(sorted(set(p for p in v7.OWN if not p.startswith('slurm/')) | {
    'scripts/train_native_vision_v8.py', 'scripts/native_vision_v8_pairs.py',
    'gnnformer/aggregation_consistency.py', *TEST_FILES,
    'slurm/native_vision_v8_train_check.sbatch',
    'slurm/native_vision_v8_train_profile.sbatch', 'slurm/native_vision_v8_train.sbatch',
}))
save = v7.save
...
def check(args):
    ...
    snapshot(out)
    index(out, 'V8 paired training CPU plan')
    import torch
    from transformers import AutoProcessor
    from scripts.native_vision_v8_pairs import load_pairs, presentation_order
    ...
    diagnostic = bind_diagnostic(bindings, args.diagnostic_summary, cache_path)
...
def verify_plan(path):
    path = Path(path).resolve()
    need(sha(path) == path.with_suffix('.sha256').read_text().strip(), 'V8 plan sidecar differs')
    plan = read(path)
    need(plan['schema_version'] == 1 and plan['policy'] == POLICY and plan['source_sha256'] == sources(),
         'Frozen V8 source/policy differs')
...
def verify_release(path, plan_path, plan):
    ...
    for condition, item in released['profiles'].items():
        ...
        need(profile['profile'] and profile['condition'] == condition and profile['seed'] == 10
...
def losses(torch, branch, local, global_states, targets, norm, head, coefficient):
    ...
    # The cast and native carry reproduce the unmodified inference controller.
    ...
    with torch.no_grad():
        ...
        position_penalties = ((paired_delta[0::2] - paired_delta[1::2]).square().sum(-1) /
            (paired_global[0::2].square().sum(-1) + POLICY['consistency_epsilon'])).mean(0)
    components = dict(ce_count_loss=float(ce_positions[0]), ce_eos_loss=float(ce_positions[1]),
        consistency_count_loss=float(position_penalties[0]), consistency_eos_loss=float(position_penalties[1]))
    total = ce + coefficient * consistency
    need(all(bool(torch.isfinite(x)) for x in (ce, consistency, total)), 'Nonfinite training objective')
    return total, ce, consistency, components
...
def bind_diagnostic(bindings, path, cache_path):
    ...
    need(summary.get('passed') is True and summary.get('diagnostic_only') is True
         ...
         and summary.get('synthetic_points') == 216, 'A completed 216-point CPU diagnostic is required')
    ...
    bind_file(bindings, analysis['training_cache_file'], analysis['training_cache_sha256'])
    bind_file(bindings, analysis['training_plan_file'], analysis['training_plan_sha256'])
    need(Path(analysis['training_cache_file']).resolve() == Path(cache_path).resolve(),
    ...
    return dict(**summary_binding, software_completion_passed=True, selection_on_diagnostic_predictions=False)
...
def run(args):
    ...
    out.mkdir(parents=True, exist_ok=False)
    snapshot(out)
    (out / 'INDEX.md').write_text('# V8 paired aggregation run\n\n'
    ...
    from scripts.native_vision_v8_pairs import presentation_order
    ...
    need(not args.profile or args.seed == 10, 'Profiles use only registered seed 10')
    ...
    dataout = DATA / 'native_aggregation_vlm_v8' / runid
    ...
    config = dict(run_id=runid, arm='parallel', condition=args.condition, seed=args.seed,
        profile=args.profile, policy=POLICY, consistency_coefficient=coefficient,
    ...
    for step in range(1, totalsteps + 1):
        ...
        for group in optimizer.param_groups:
            group['lr'] = rate
        total, ce, consistency, components = losses(torch, branch, local, g, targets, norm, head, coefficient)
        total.backward()
        ...
        trainlog.append(dict(step=step, lr=rate, loss=float(total), ce_loss=float(ce),
            consistency_loss=float(consistency), consistency_coefficient=coefficient,
            weighted_consistency_loss=float(coefficient * consistency), gradient_norm=float(gradient_norm),
        ...
        if step % 100 == 0:
            print(json.dumps(dict(step=step, ce=float(ce), consistency=float(consistency), total=float(total))), flush=True)
...
def main():
    ...
    if args.check:
        need(os.environ.get('SLURM_JOB_PARTITION') == 'cpu' and not os.environ.get('SLURM_JOB_GPUS'),
             'The V8 freeze requires a CPU-only Slurm allocation')
