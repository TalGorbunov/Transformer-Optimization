"""Proposed V11 fresh test preparation; exact CPU dry plan before rendering.
...
and exclude27 prior manifests. Fresh means complete context/question freshness,
...
DESTINATION=DATA_BASE/'v11_fresh'
OUT=REPO/'outputs/native_aggregation_vlm/v11/data_staging'
SEED=20261010
PROTOCOL='v11_fresh_test_preparation_only'
SPEC=dict(unique_samples=272,unique_content=272,families=136,image_references=13056,
          paired_extensions=136,test_N32=136,test_N64=136,prior_manifests=27)
OWN=tuple(dict.fromkeys(('scripts/stage_native_vision_v11_test.py','slurm/native_vision_v11_test_stage.sbatch',*prior.OWN)))
...
def snapshot(out):
    ...
    (out/'INDEX.md').write_text('# Proposed V11 fresh test preparation\n\n[Plan](plan.json) · [Summary](summary.json) · [Sources](source_hashes.json) · [Report](REPORT.md).\n')
    index=OUT/'INDEX.md'
    if not index.exists():index.write_text('# V11 fresh test data preparation\n\n')
    with index.open('a') as stream:stream.write(f'- [{out.name}]({out.name}/INDEX.md)\n')
    return frozen


def prior_inputs():
    inventory,manifests,excluded=prior.prior_inputs()
    need(len(inventory['entries'])==len(manifests)==25,'Require exact V10 prior25 inventory')
    prior25=dict(inventory['source_manifest_sha256'])
    for root in (DATA_BASE/'v10_balanced',DATA_BASE/'v10_fresh'):
        path=root/'main_manifest.json';need(path.is_file() and not path.is_symlink(),'Missing canonical V10 manifest')
        manifest=read(path);need(manifest['source_manifest_sha256']==prior25,'V10 prior25 ancestry changed')
        rows=list(content_rows(manifest));hashes={r['content_sha256'] for r in rows};h=digest(path)
        need(rows and str(path) not in manifests and all(isinstance(x,str) and len(x)==64
             and all(c in '0123456789abcdef' for c in x) for x in hashes),'Invalid added prior manifest')
        ...
        for field in ('audit','stage_plan','inventory'):
            target=Path(manifest[field+'_file']);expected=manifest[field+'_sha256']
            need(digest(target)==expected,'V10 data audit/plan/inventory changed')
            inventory['protected_source_sha256'][str(target)]=expected
        stage_audit=Path(manifest['stage_audit_file']);audit=read(stage_audit)
        need(audit['passed'] and audit['completed'],'V10 published data did not pass audit')
        inventory['protected_source_sha256'][str(stage_audit)]=digest(stage_audit)
    balanced=DATA_BASE/'v10_balanced';manifest_sha=digest(balanced/'main_manifest.json')
    schedule=read(balanced/'schedule.json');pairing=read(balanced/'pairing.json')
    need(schedule['manifest_file']==pairing['manifest_file']==str(balanced/'main_manifest.json')
         and schedule['manifest_sha256']==pairing['manifest_sha256']==manifest_sha
         and pairing['schedule_file']==str(balanced/'schedule.json')
         and pairing['schedule_sha256']==digest(balanced/'schedule.json'),'Literal V10 training schedule/pairing changed')
    for name in ('schedule.json','pairing.json'):
        path=balanced/name;need(path.is_file() and not path.is_symlink(),'Missing canonical V10 schedule/pairing')
        inventory['protected_source_sha256'][str(path)]=digest(path)
    need(len(inventory['entries'])==len(manifests)==len(inventory['source_manifest_sha256'])==27,'Require exactly27 prior manifests')
    ...
    inventory.update(excluded_content_sha256=ordered,excluded_content_count=len(ordered),
        ...
        scope='Exact V10 prior25 plus V10 balanced main and V10 fresh main;27 canonical manifests',
        permitted_historical_reuse='None. All proposed V11 contexts are nonsaturated N32/N64 with K0..16.',
...
def generate(excluded):
    ...
    for k in range(17):
        for replica in range(8):
            label=f'v11_test_K{k}_{replica:03d}';seed=stable_seed(SEED,label)
            family=make_family(random.Random(seed),(32,64),k,label,'test','v11_length',used,seed)
            for row in family:
...
def dry(out,frozen):
    need(not DESTINATION.exists() and not DESTINATION.is_symlink(),'Preserve existing/partial proposed V11 data')
    ...
    plan=dict(schema_version=1,protocol=PROTOCOL,dataset_root=str(DESTINATION),data_seed=SEED,fresh_test_seed=SEED,
        ...
        generator_law=LAW,sampling='Canonical V2 family law conditioned on exclusion against27 prior manifests and all new families',
    ...
    (out/'REPORT.md').write_text('# V11 prospective test dry plan\n\nPASS: 272 distinct contexts,136 N32→N64 families, eight/K0–16. All27 prior manifests and literal V10 training/dev/schedule/pairing are protected. No images rendered or training released.\n')
    return summary


def load_plan(path,frozen):
    path=Path(path).resolve();need(path.is_relative_to(OUT),'Require a canonical V11 dry plan')
...
def stage(args,out,frozen):
    ...
    need(current==inventory and current_excluded==excluded,'Prior27 inventory changed after dry gate')
    ...
    for record,sample in zip(records,rows):record.update(question=sample['question'],origin='generated_v11_fresh',fresh_test_seed=SEED)
    cells={}
    for n in (32,64):
    ...
    (DESTINATION/'INDEX.md').write_text('# V11 fresh test data\n\n[Manifest](main_manifest.json) · [Semantic audit](semantic_audit.json) · [Completed audit](stage_audit.json) · [Dry plan](stage_plan.json) · [Exclusions](exclusion_inventory.json).\n')
    (out/'REPORT.md').write_text('# V11 fresh test staging\n\nPASS:272 contexts,136 N32→N64 families, eight/K0–16. Independent QA, image, renderer-link, exclusion and extension audits passed. All27 prior manifests and literal V10 training/dev/schedule/pairing are unchanged. No model calls or training release.\n')
    return result
